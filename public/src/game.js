...
(function () {
  ...
  const VERSION = 'Raid Dungeon V46 - ACTUAL Visible Progression Market Chat';
...
/* =========================================================
   RAID DUNGEON V46 - FORCED VISIBLE PROGRESSION / MARKET / CHAT
...
========================================================= */
(function raidDungeonV46ForcedVisibleProgression(){
  const V46_VERSION = 'Raid Dungeon V46 - ACTUAL Visible Progression Market Chat';
  const MARKET_KEY = 'raid-dungeon-v46-market';
  const CHAT_KEY = 'raid-dungeon-v46-chat';
  const FIRST_BOX_ID = 'raid-v46-first-clear-only-center';
  const FIRST_STYLE_ID = 'raid-v46-first-style';
  ...
  function v46(){
    ...
    const v = state.save.v46 || (state.save.v46={});
  ...
  function addMat(name,qty){const v=v46(); name=name||'정체불명 부산물'; v.materials[name]=(v.materials[name]||0)+Math.max(0,Math.floor(qty||0));}
  function matCount(name){return Number(v46().materials[name]||0);}
  function takeMat(name,qty){const v=v46(); qty=Math.max(0,Math.floor(qty||0)); if((v.materials[name]||0)<qty) return false; v.materials[name]-=qty; return true;}
  ...
  function weaponMeta(id){const v=v46(); id=safeId(id); return {enh:v.weaponEnhance[id]||0, ench:v.weaponEnchant[id]||[]};}
  function enhanceCost(id){const w=(oldGetWeaponV46?oldGetWeaponV46(id):null)||getWeapon(id)||{}; const lv=weaponMeta(id).enh; const mat=MATERIALS[Math.abs(hashCode(id||w.name||'w'))%MATERIALS.length]; return {mat, qty:Math.max(1,1+lv*2), coins:Math.max(20,60+lv*55)};}
  ...
  try{
    var oldNormalizeSaveDataV46 = normalizeSaveData;
    normalizeSaveData = function(s){ const out=oldNormalizeSaveDataV46(s); state.save=out; v46(); return out; };
  }catch(e){}

  var oldGetWeaponV46 = null;
  try{
    oldGetWeaponV46 = getWeapon;
    getWeapon = function(id){
      const base=oldGetWeaponV46(id); if(!base) return null;
      const m=weaponMeta(id), rates=enchantRates(m.ench);
      const upMul=1+m.enh*.10+rates.attack;
      const speedMul=1+rates.speed;
      return {...base, atk:+(Number(base.atk||1)*upMul).toFixed(3), speed:+(Number(base.speed||1)/speedMul).toFixed(3), v46Enhance:m.enh, v46Enchant:m.ench, desc:String(base.desc||'')+`\n[V46] +${m.enh} 강화 · 인챈트 ${enchantSummary(m.ench)} · 공격 +${Math.round((upMul-1)*100)}% · 공속 +${Math.round(rates.speed*100)}% · 행운 +${Math.round(rates.luck*100)}% · 약탈 +${Math.round(rates.plunder*100)}%`};
    };
  }catch(e){console.warn('[V46 getWeapon patch failed]',e);}
  var oldGetSkillV46 = null;
  try{
    oldGetSkillV46 = getSkill;
    getSkill = function(id){
      const base=oldGetSkillV46(id); if(!base) return null;
      const lv=clamp(Number(v46().skillLevels[id]||1),1,7);
      const mul=1+(lv-1)*.16;
      return {...base, power:+(Number(base.power||1)*mul).toFixed(3), v46Level:lv, desc:String(base.desc||'')+`\n[V46] 스킬 Lv.${lv}/7 · 위력 +${Math.round((mul-1)*100)}%`};
    };
  }catch(e){console.warn('[V46 getSkill patch failed]',e);}

  try{
    const oldRollGachaV46=rollGacha;
    rollGacha=function(kind){
      if(kind!=='skill') return oldRollGachaV46(kind);
      if(!state.save.tickets || typeof state.save.tickets!=='object') state.save.tickets={...INITIAL_TICKETS};
  ...
  }catch(e){console.warn('[V46 gacha patch failed]',e);}
  ...
  function addBossExclusiveDrops(b, out){
    ...
    function addPatternSkill(){
      const id='boss_pattern_skill_'+idBase; const v=v46();
  ...
  function showFirstOnlyCenter(name){
    ...
    let st=document.getElementById(FIRST_STYLE_ID); if(!st){st=document.createElement('style'); st.id=FIRST_STYLE_ID; document.head.appendChild(st);} st.textContent='@keyframes v46first{0%{opacity:0;transform:translate(-50%,-50%) scale(.65)}14%{opacity:1;transform:translate(-50%,-50%) scale(1.08)}75%{opacity:1}100%{opacity:0;transform:translate(-50%,-50%) scale(.98)}}';
    const box=document.createElement('div'); box.id=FIRST_BOX_ID; box.style.cssText='position:fixed;left:50%;top:50%;transform:translate(-50%,-50%);z-index:2147483647;text-align:center;min-width:620px;max-width:92vw;padding:38px 46px;border:5px solid #facc15;border-radius:34px;background:rgba(2,6,23,.94);box-shadow:0 0 80px rgba(250,204,21,.8);pointer-events:none;animation:v46first 5.2s ease-out forwards'; box.innerHTML=`<div style="font-size:64px;font-weight:950;color:#facc15;text-shadow:0 0 30px #facc15">FIRST CLEAR!</div><div style="font-size:24px;font-weight:900;color:#fff;margin-top:8px">${esc(name)} 최초 클리어 기록 달성</div>`; document.body.appendChild(box); setTimeout(()=>box.remove(),5600);
  }
  ...
  }catch(e){console.warn('[V46 endRaid patch failed]',e);}
  ...
  function chatSend(){const input=document.getElementById('v46ChatInput'); const text=(input&&input.value||'').trim().slice(0,140); if(!text)return; const a=chatLoad(); a.push({name:state.save.playerName||'Player',text,t:Date.now()}); localStorage.setItem(CHAT_KEY,JSON.stringify(a)); input.value=''; renderMenu();}
  function marketBuy(id){const a=marketLoad(); const idx=a.findIndex(x=>x.id===id); if(idx<0)return; const it=a[idx]; if((state.save.coins||0)<it.price){toast('코인이 부족합니다.');return;} state.save.coins-=it.price; if(it.kind==='material') addMat(it.name,it.qty); if(it.kind==='weapon'&&!state.save.weapons.includes(it.itemId)) state.save.weapons.push(it.itemId); if(it.kind==='skill'){if(!state.save.skills.includes(it.itemId)) state.save.skills.push(it.itemId); else v46().skillCopies[it.itemId]=(v46().skillCopies[it.itemId]||0)+1;} a.splice(idx,1); marketSave(a); saveGame(); renderMenu();}
  function registerMaterial(){const sel=document.getElementById('v46MatSelect'); const price=Math.max(1,Math.floor(Number(document.getElementById('v46MatPrice')?.value||0))); const name=sel&&sel.value; if(!name||!price){toast('부산물과 가격을 입력하세요.');return;} const qty=Math.min(10,matCount(name)); if(qty<=0){toast('등록할 부산물이 없습니다.');return;} takeMat(name,qty); const a=marketLoad(); a.push({id:'m'+Date.now()+Math.random(),kind:'material',name,qty,price,seller:state.save.playerName||'Player'}); marketSave(a); saveGame(); renderMenu();}
  function enhanceWeapon(id){const c=enhanceCost(id); const m=weaponMeta(id); if(m.enh>=10){toast('이미 최대 강화입니다.');return;} if((state.save.coins||0)<c.coins){toast('코인이 부족합니다.');return;} if(!takeMat(c.mat,c.qty)){toast(`부산물이 부족합니다: ${c.mat} ${matCount(c.mat)}/${c.qty}`);return;} state.save.coins-=c.coins; v46().weaponEnhance[safeId(id)]=m.enh+1; toast(`무기 +${m.enh+1} 강화 성공`); saveGame(); renderMenu();}
  function enchantWeapon(id){const types=['speed','attack','luck','plunder','critDamage','critChance']; const labels={speed:'공속',attack:'공격',luck:'행운',plunder:'약탈',critDamage:'치명타 피해량',critChance:'치명타 확률'}; const cost=160; if((state.save.coins||0)<cost){toast('인챈트 코인이 부족합니다.');return;} state.save.coins-=cost; const count=1+(Math.random()<.28?1:0)+(Math.random()<.08?1:0); const ench=[]; for(let i=0;i<count;i++) ench.push({type:types[Math.floor(Math.random()*types.length)],lv:1+Math.floor(Math.random()*3)}); v46().weaponEnchant[safeId(id)]=ench; toast('인챈트: '+ench.map(e=>`${labels[e.type]} Lv.${e.lv}`).join(' / ')); saveGame(); renderMenu();}
  ...
  function upgradeSkill(id){const v=v46(); const lv=v.skillLevels[id]||1, cp=v.skillCopies[id]||0; if(lv>=7){toast('최대 레벨입니다.');return;} if(cp<=0){toast('중복 스킬 포인트가 필요합니다.');return;} v.skillCopies[id]=cp-1; v.skillLevels[id]=lv+1; toast(`${(getSkill(id)||{}).name||id} Lv.${lv+1}`); saveGame(); renderMenu();}
  ...
  function renderGrowth(){v46(); const mats=Object.entries(v46().materials).filter(x=>x[1]>0); const weapons=[...new Set(state.save.weapons||[])].map(id=>getWeapon(id)).filter(Boolean); const skills=[...new Set(state.save.skills||[])].map(id=>getSkill(id)).filter(Boolean); return `<h2 class="title" style="font-size:22px">성장/강화</h2><p class="sub">무기 강화, 인챈트, 스킬 레벨업, 판매/분해가 여기에서 바로 보입니다.</p><div class="grid"><div class="card"><h3>코인/부산물</h3><p>코인 ${Math.floor(state.save.coins||0)}</p><p class="muted">${mats.slice(0,10).map(([m,q])=>`${esc(m)} x${q}`).join('<br>')||'부산물 없음'}</p></div><div class="card"><h3>스킬 레벨업</h3>${skills.map(s=>`<div class="record" title="${esc(s.desc)}"><div><b>${esc(s.name)} Lv.${s.v46Level||1}</b><div class="muted">위력 x${s.power} · 포인트 ${v46().skillCopies[s.id]||0}</div></div><button class="btn secondary" data-v46-skill-up="${esc(s.id)}">레벨업</button></div>`).join('')||'<p class="muted">스킬 없음</p>'}</div></div><h3>무기 관리</h3><div class="grid">${weapons.map(w=>{const c=enhanceCost(w.id), mt=matCount(c.mat); return `<div class="card" title="${esc(w.desc)}"><h3>${esc(w.name)} +${w.v46Enhance||0}</h3><p>공격 x${w.atk} · 공속 ${Number(w.speed||0).toFixed(2)}<br>인챈트: ${esc(enchantSummary(w.v46Enchant))}<br>강화 재료: ${esc(c.mat)} ${mt}/${c.qty} · ${c.coins}코인</p><div class="row"><button class="btn secondary" data-v46-enhance="${esc(w.id)}">강화</button><button class="btn secondary" data-v46-enchant="${esc(w.id)}">인챈트</button></div><div class="row" style="margin-top:6px"><button class="btn secondary" data-v46-sell="${esc(w.id)}">평균가 판매</button><button class="btn secondary" data-v46-disassemble="${esc(w.id)}">분해</button></div><button class="btn secondary" style="margin-top:6px;width:100%" data-v46-market-weapon="${esc(w.id)}">판매소 등록</button></div>`;}).join('')||'<p class="muted">무기 없음</p>'}</div>`;}
  function renderMarket(){v46(); const mats=Object.entries(v46().materials).filter(x=>x[1]>0); const opts=mats.map(([m,q])=>`<option value="${esc(m)}">${esc(m)} x${q}</option>`).join(''); const list=marketLoad().slice().reverse(); return `<h2 class="title" style="font-size:22px">유저 판매소</h2><p class="sub">부산물은 최대 10개가 1묶음으로 등록됩니다. 가격은 직접 입력합니다.</p><div class="grid"><div class="card"><h3>부산물 등록</h3><select id="v46MatSelect" class="input">${opts}</select><input id="v46MatPrice" class="input" type="number" min="1" placeholder="가격 입력" style="margin-top:8px"><button id="v46MatRegister" class="btn" style="margin-top:8px;width:100%">부산물 최대 10개 묶음 등록</button></div><div class="card"><h3>판매 목록</h3>${list.map(x=>`<div class="record"><div class="rank">${x.kind==='material'?x.qty:'1'}</div><div><b>${esc(x.name)}</b><div class="muted">${esc(x.kind)} · ${esc(x.seller)}</div></div><button class="btn secondary" data-v46-buy="${esc(x.id)}">${x.price} 구매</button></div>`).join('')||'<p class="muted">등록된 물품이 없습니다.</p>'}</div></div>`;}
  function renderChat(){const chats=chatLoad(); return `<h2 class="title" style="font-size:22px">채팅</h2><p class="sub">작성된 지 30분이 넘은 채팅은 자동 삭제됩니다.</p><div class="card"><div style="height:270px;overflow:auto;background:#020617;border:1px solid rgba(148,163,184,.22);border-radius:14px;padding:10px">${chats.map(m=>`<div class="muted" style="margin-bottom:6px"><b style="color:#e5e7eb">${esc(m.name)}:</b> ${esc(m.text)}</div>`).join('')||'<span class="muted">아직 채팅이 없습니다.</span>'}</div><div class="row" style="margin-top:10px"><input id="v46ChatInput" class="input" placeholder="채팅 입력"><button id="v46ChatSend" class="btn">전송</button></div></div>`;}
  function bindV46(){const root=ui&&ui.menu; if(!root)return; root.querySelectorAll('[data-v46-tab]').forEach(b=>b.onclick=()=>{state.menuTab=b.dataset.v46Tab; renderMenu();}); const by=(sel,fn)=>root.querySelectorAll(sel).forEach(el=>el.onclick=()=>fn(el)); by('[data-v46-skill-up]',el=>upgradeSkill(el.dataset.v46SkillUp)); by('[data-v46-enhance]',el=>enhanceWeapon(el.dataset.v46Enhance)); by('[data-v46-enchant]',el=>enchantWeapon(el.dataset.v46Enchant)); by('[data-v46-sell]',el=>sellWeapon(el.dataset.v46Sell)); by('[data-v46-disassemble]',el=>disassembleWeapon(el.dataset.v46Disassemble)); by('[data-v46-market-weapon]',el=>registerWeapon(el.dataset.v46MarketWeapon)); by('[data-v46-buy]',el=>marketBuy(el.dataset.v46Buy)); const mr=root.querySelector('#v46MatRegister'); if(mr)mr.onclick=registerMaterial; const cs=root.querySelector('#v46ChatSend'); if(cs)cs.onclick=chatSend; const inp=root.querySelector('#v46ChatInput'); if(inp)inp.onkeydown=e=>{if(e.key==='Enter')chatSend();};}
  const oldRenderMenuV46=renderMenu;
  renderMenu=function(){
    v46();
    ...
    if(extra.includes(state.menuTab)){
      ...
      ui.menu.innerHTML=`<div class="row"><div><h1 class="title">Raid Dungeon</h1><p class="sub">V46 성장/강화, 전용 드롭, 판매소, 채팅이 실제로 보이는 버전입니다.</p></div><div style="text-align:right"><div class="chip">${V46_VERSION}</div><div class="chip">닉네임 ${esc(state.save.playerName||'Player')}</div><div class="chip">코인 ${Math.floor(state.save.coins||0)}</div><div class="chip">무기티켓 ${state.save.tickets.weapon||0}</div><div class="chip">방어구티켓 ${state.save.tickets.armor||0}</div><div class="chip">스킬티켓 ${state.save.tickets.skill||0}</div><div class="chip">패시브티켓 ${state.save.tickets.passive||0}</div></div></div><div class="nav">${tabs.map(([t,l])=>`<button class="tab ${state.menuTab===t?'active':''}" data-v46-tab="${t}">${l}</button>`).join('')}</div>${body}`;
      bindV46(); return;
    }
    oldRenderMenuV46();
    try{
      const right=ui.menu.querySelector('.row > div:last-child'); if(right&&!right.querySelector('[data-v46-coin-chip]')) right.insertAdjacentHTML('beforeend',`<div class="chip" data-v46-coin-chip="1">코인 ${Math.floor(state.save.coins||0)}</div><div class="chip">${V46_VERSION}</div>`);
      const nav=ui.menu.querySelector('.nav'); if(nav&&!nav.querySelector('[data-v46-tab="growth"]')) nav.insertAdjacentHTML('beforeend',`<button class="tab" data-v46-tab="growth">성장/강화</button><button class="tab" data-v46-tab="market">유저 판매소</button><button class="tab" data-v46-tab="chat">채팅</button>`);
      bindV46();
    }catch(e){}
  };
  ...
  try{ window.RaidDungeonV46={version:V46_VERSION, v46, enhanceWeapon, enchantWeapon, sellWeapon, disassembleWeapon, upgradeSkill, registerMaterial, marketBuy, chatSend}; v46(); renderMenu(); }catch(e){console.warn('[V46 init failed]',e);}
})();
