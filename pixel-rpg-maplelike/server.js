...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === '/api/register' && req.method === 'POST') {
      ...
      if (!/^[a-zA-Z0-9_가-힣]{2,16}$/.test(username)) return json(res, 400, { ok: false, error: '아이디는 2~16자, 한글/영문/숫자/_ 만 가능합니다.' });
      if (password.length < 4) return json(res, 400, { ok: false, error: '비밀번호는 4자 이상이어야 합니다.' });
      const users = readUsers();
      if (users[username]) return json(res, 409, { ok: false, error: '이미 존재하는 아이디입니다.' });
      const salt = crypto.randomBytes(16).toString('hex');
      users[username] = { username, salt, passHash: hashPassword(password, salt), createdAt: new Date().toISOString(), save: null };
      writeUsers(users);
      const token = crypto.randomBytes(32).toString('hex');
      sessions.set(token, username);
      return json(res, 200, { ok: true, token, user: safeUser(users[username]) });
    }
    ...
    if (url.pathname === '/api/login' && req.method === 'POST') {
      ...
      if (!user || user.passHash !== hashPassword(password, user.salt)) return json(res, 401, { ok: false, error: '아이디 또는 비밀번호가 맞지 않습니다.' });
      const token = crypto.randomBytes(32).toString('hex');
      sessions.set(token, username);
    ...
    if (url.pathname === '/api/profile' && req.method === 'GET') {
      ...
      if (!username) return json(res, 401, { ok: false, error: '로그인이 필요합니다.' });
      const users = readUsers();
      return json(res, 200, { ok: true, user: safeUser(users[username]) });
    }
    ...
    if (url.pathname === '/api/check-nickname' && req.method === 'POST') {
      ...
      const users = readUsers();
      if (!/^[a-zA-Z0-9_가-힣]{2,10}$/.test(nickname)) return json(res, 400, { ok: false, error: '닉네임은 2~10자, 한글/영문/숫자/_ 만 가능합니다.' });
      if (nicknameTaken(users, nickname, username)) return json(res, 409, { ok: false, error: '이미 사용 중인 닉네임입니다.' });
    ...
    if (url.pathname === '/api/create-character' && req.method === 'POST') {
      ...
      if (!/^[a-zA-Z0-9_가-힣]{2,10}$/.test(character.name)) return json(res, 400, { ok: false, error: '게임 닉네임은 2~10자, 한글/영문/숫자/_ 만 가능합니다.' });
      const users = readUsers();
      if (!users[username]) return json(res, 404, { ok: false, error: '사용자를 찾을 수 없습니다.' });
      if (nicknameTaken(users, character.name, username)) return json(res, 409, { ok: false, error: '이미 사용 중인 닉네임입니다.' });
    ...
    if (url.pathname === '/api/save' && req.method === 'POST') {
      ...
      users[username].savedAt = new Date().toISOString();
      writeUsers(users);
      return json(res, 200, { ok: true, savedAt: users[username].savedAt });
    }
...
server.listen(PORT, () => console.log(`Pixel RPG server running at http://localhost:${PORT}`));
