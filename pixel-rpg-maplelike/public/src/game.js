...
/* =========================================================
   V18 TOWN / NPC / DIALOG / EQUIPMENT STABILITY POLISH
   - Town NPCs stay on the ground and are spaced cleanly.
   - Town invisible/floating platforms are removed to stop "walking on air".
   - NPCs no longer float/bob or stand on upper phantom floors.
   - NPCs hold role props cleanly, not random weapons.
   - Dialog windows hide the multiplayer chat UI while open.
   - Equipment and item icons use more distinct pixel-art shapes.
========================================================= */
(function(){
  'use strict';
  if (window.__PIXEL_RPG_V18_STABILITY_POLISH__) return;
  window.__PIXEL_RPG_V18_STABILITY_POLISH__ = true;
  window.PIXEL_RPG_PATCH_VERSION = 'V18_STABILITY_POLISH';

  function A(v){ return Array.isArray(v) ? v : []; }
  function n(v,d){ v = Number(v); return Number.isFinite(v) ? v : d; }
  function clamp2(v,a,b){ return Math.max(a, Math.min(b, v)); }
  function rr(c,x,y,w,h,r){ if (typeof roundRect === 'function') { roundRect(c,x,y,w,h,r||6); return; } c.beginPath(); c.roundRect(x,y,w,h,r||6); c.fill(); }
  function strokeRR(c,x,y,w,h,r){ if (typeof roundRect === 'function') { c.beginPath(); c.roundRect ? c.roundRect(x,y,w,h,r||6) : c.rect(x,y,w,h); c.stroke(); return; } c.strokeRect(x,y,w,h); }
  function fillRR(c,x,y,w,h,r,col){ c.fillStyle = col; rr(c,x,y,w,h,r); }
  function circle(c,x,y,r,col){ c.fillStyle = col; c.beginPath(); c.arc(x,y,r,0,Math.PI*2); c.fill(); }
  function line(c,x1,y1,x2,y2,col,lw){ c.strokeStyle=col; c.lineWidth=lw||2; c.lineCap='round'; c.beginPath(); c.moveTo(x1,y1); c.lineTo(x2,y2); c.stroke(); }
  function poly(c, pts, col){ c.fillStyle=col; c.beginPath(); pts.forEach(function(p,i){ if(i)c.lineTo(p[0],p[1]); else c.moveTo(p[0],p[1]); }); c.closePath(); c.fill(); }
  function hash(s){ s=String(s||''); var h=0; for(var i=0;i<s.length;i++) h=((h<<5)-h+s.charCodeAt(i))|0; return Math.abs(h); }
  function idOf(ref){ try { return typeof itemRefId === 'function' ? itemRefId(ref) : (typeof ref === 'string' ? ref : ref && ref.id); } catch(_) { return typeof ref === 'string' ? ref : ref && ref.id; } }
  function itemOf(ref){ if(!ref) return null; if(typeof ref === 'object' && (ref.id || ref.name || ref.icon)) return Object.assign({}, (typeof ITEMS !== 'undefined' && ITEMS && ref.id && ITEMS[ref.id]) || {}, ref); var id=idOf(ref); return (typeof ITEMS !== 'undefined' && ITEMS && ITEMS[id]) ? Object.assign({id:id}, ITEMS[id]) : (id ? {id:id,name:id,icon:id} : null); }
  function slotOf(it){ try { return typeof getEquipSlotForItem === 'function' ? getEquipSlotForItem(it||{}) : ((it&&it.equipSlot)||(it&&it.type)); } catch(_) { return (it&&it.equipSlot)||(it&&it.type); } }

  function hideChatWhenModal(){
    try {
      var box = document.getElementById('pixel-rpg-chat-box');
      if (!box || typeof game === 'undefined' || !game) return;
      var modal = !!(game.dialog || game.taxiOpen || game.shopOpen || game.blacksmithOpen || inventory.open || stats.open || skills.open || quests.open);
      box.style.zIndex = modal ? '1' : '9999';
      box.style.opacity = modal ? '0' : '1';
      box.style.pointerEvents = modal ? 'none' : '';
      box.style.visibility = modal ? 'hidden' : 'visible';
    } catch (_) {}
  }

  function sanitizeTownLayout(){
    if (typeof game === 'undefined' || !game || game.mode !== 'town') return;
    game.ground = n(game.ground, 560);
    /* Remove old upper town platforms/ladders. They caused invisible/floating walkable air in towns.
       Hunting maps keep their platforms. */
    game.platforms = [];
    game.ladders = [];

    var p = game.player;
    if (p && p.y < game.ground - 10 && !p.climbing) {
      p.y = game.ground;
      p.vy = 0;
      p.grounded = true;
    }
  ...
  function npcRole(npc){
    var s = String((npc && npc.name) || '') + ' ' + String((npc && npc.type) || '') + ' ' + String((npc && npc.role) || '');
    if (/택시/.test(s)) return 'taxi';
    if (/장비/.test(s)) return 'weapon';
    if (/대장장이|브론|blacksmith/.test(s)) return 'blacksmith';
    if (/상인|merchant/.test(s)) return 'merchant';
    if (/성검|십자가|룬석|오브|균열|유물|비석/.test(s) || (npc && npc.type === 'job_object')) return 'object';
    if (/용혈|용기사|드래곤|아르딘/.test(s)) return 'dragon';
    if (/마법|현자|이렌|벨라|아스텔|mage|sage/.test(s)) return 'mage';
    if (/궁수|명궁|세리아|리아|archer|ranger/.test(s)) return 'archer';
    if (/도적|그림자|암살|사신|카론|란|모르가|rogue|assassin/.test(s)) return 'rogue';
    if (/기사|전사|로한|카일|warrior|knight/.test(s)) return 'warrior';
    if (/장로|퀘스트/.test(s) || (npc && npc.type === 'quest')) return 'quest';
    return 'villager';
  }

  function normalizeTownNpcs(){
    if (typeof game === 'undefined' || !game || game.mode !== 'town') return;
    sanitizeTownLayout();
    game.npcs = A(game.npcs).filter(Boolean);

    var seen = {};
    game.npcs = game.npcs.filter(function(npc){
      var key = String(npc.id || '') + '|' + String(npc.name || '') + '|' + String(npc.quest || '') + '|' + String(npc.role || '');
      if (seen[key]) return false;
      seen[key] = true;
      return true;
    });

    var baseByRole = { taxi:420, merchant:760, weapon:1100, blacksmith:1280, quest:1460 };
    var jobStart = 1680;
    var jobIndex = 0;
    ...
    game.npcs.forEach(function(npc){
      ...
      if (role === 'taxi') npc.x = baseByRole.taxi;
      else if (role === 'merchant') npc.x = baseByRole.merchant;
      else if (role === 'weapon') npc.x = baseByRole.weapon;
      else if (role === 'blacksmith') npc.x = baseByRole.blacksmith;
      else if (role === 'quest') npc.x = baseByRole.quest;
      else {
        npc.x = jobStart + jobIndex * 210;
        jobIndex += 1;
      }

      npc.x = clamp2(n(npc.x, 260), 80, Math.max(800, (game.width || 4300) - 120));
      npc.y = game.ground;
      npc.vx = 0;
      npc.vy = 0;
      npc.grounded = true;
      npc.__v18GroundLocked = true;
    });
  }

  function drawRoleProp(c, role){
    c.save();
    c.translate(32, -48);
    if (role === 'taxi') {
      fillRR(c,-18,-12,36,20,5,'#fde047'); fillRR(c,-10,-19,20,8,3,'#f59e0b');
      circle(c,-10,10,4,'#111827'); circle(c,10,10,4,'#111827');
    } else if (role === 'merchant') {
      fillRR(c,-12,-11,24,26,6,'#92400e'); line(c,-5,-12,5,-12,'#fbbf24',3); fillRR(c,-5,-1,10,5,2,'#facc15');
    } else if (role === 'weapon') {
      /* shield emblem, not a held sword */
      poly(c, [[0,-22],[18,-12],[13,16],[0,27],[-13,16],[-18,-12]], '#cbd5e1');
      poly(c, [[0,-15],[11,-9],[8,10],[0,17],[-8,10],[-11,-9]], '#64748b');
      line(c,0,-10,0,13,'#e5e7eb',2);
    } else if (role === 'blacksmith') {
      /* tiny anvil/forge icon rather than a huge hammer */
      poly(c, [[-18,0],[-8,-12],[13,-12],[19,-4],[6,0],[-2,9]], '#cbd5e1');
      fillRR(c,-4,8,8,8,2,'#64748b');
      circle(c,17,-17,5,'#fb923c');
    } else if (role === 'quest') {
      fillRR(c,-14,-20,28,32,5,'#fef3c7');
      line(c,-8,-10,8,-10,'#92400e',2); line(c,-8,-2,8,-2,'#92400e',2); line(c,-8,6,3,6,'#92400e',2);
    } else if (role === 'warrior') {
      poly(c, [[0,-22],[16,-12],[12,15],[0,24],[-12,15],[-16,-12]], '#94a3b8');
      circle(c,0,0,7,'#facc15');
    } else if (role === 'mage') {
      circle(c,0,-2,13,'#93c5fd');
      c.strokeStyle='#c4b5fd'; c.lineWidth=3; c.beginPath(); c.arc(0,-2,20,0,Math.PI*2); c.stroke();
      circle(c,-5,-7,3,'#fff');
    } else if (role === 'rogue') {
      poly(c, [[0,-20],[7,-4],[21,-4],[9,6],[13,22],[0,12],[-13,22],[-9,6],[-21,-4],[-7,-4]], '#a78bfa');
      circle(c,0,0,3,'#111827');
    } else if (role === 'archer') {
      poly(c, [[-9,18],[-2,-23],[10,-19],[4,-4],[16,9],[1,8]], '#bef264');
      line(c,-3,14,8,-12,'#365314',2);
    } else if (role === 'dragon') {
      poly(c, [[-18,5],[-9,-22],[5,-16],[22,-3],[10,16],[-5,22]], '#5eead4');
      line(c,-12,2,-22,-8,'#134e4a',3);
      line(c,-4,-16,3,-27,'#134e4a',3);
      circle(c,8,-4,2,'#111827');
    } else {
      circle(c,0,0,12,'#60a5fa');
    }
    c.restore();
  }

  function drawJobObjectClean(npc){
    var role = String((npc && (npc.role || npc.name)) || '').toLowerCase();
    ctx.save();
    ...
    if (/cross|십자가/.test(role)) {
      fillRR(ctx,-7,-72,14,66,3,'#fef3c7');
      fillRR(ctx,-28,-51,56,13,3,'#fef3c7');
      ctx.strokeStyle='#fde68a'; ctx.lineWidth=2; ctx.strokeRect(-7,-72,14,66);
    } else if (/rune|룬/.test(role)) {
      fillRR(ctx,-25,-62,50,56,10,'#64748b');
      ctx.strokeStyle='#a78bfa'; ctx.lineWidth=4; ctx.strokeRect(-14,-51,28,34);
      line(ctx,-8,-42,8,-35,'#d8b4fe',2); line(ctx,8,-35,-5,-22,'#d8b4fe',2); line(ctx,-5,-22,10,-14,'#d8b4fe',2);
    } else if (/orb|오브|star/.test(role)) {
      ctx.shadowColor='#f0abfc'; ctx.shadowBlur=18;
      circle(ctx,0,-42,25,'#c084fc'); circle(ctx,0,-42,11,'#fef3c7');
      ctx.shadowBlur=0;
    } else if (/crack|균열|abyss/.test(role)) {
      ctx.strokeStyle='#312e81'; ctx.lineWidth=9; ctx.beginPath(); ctx.arc(0,-38,30,0,Math.PI*2); ctx.stroke();
      line(ctx,-8,-64,5,-47,'#818cf8',4); line(ctx,5,-47,-4,-25,'#818cf8',4);
    } else {
      /* sacred sword object: planted in the ground, not held by NPC */
      line(ctx,0,-74,0,-9,'#f8fafc',9);
      line(ctx,-24,-33,24,-33,'#facc15',8);
      fillRR(ctx,-6,-27,12,22,3,'#1d4ed8');
      circle(ctx,0,-84,10,'#60a5fa');
    }

    ctx.fillStyle = '#ffffff';
    ctx.font = 'bold 14px sans-serif';
    ctx.textAlign = 'center';
    if (npc.name) ctx.fillText(npc.name, 0, -94);
    if (game.player && Math.abs(game.player.x - npc.x) < 90) {
      ctx.fillStyle = '#ffe066';
      ctx.fillText('E 조사', 0, -114);
    }
    ctx.restore();
  }

  drawNPCBody = window.drawNPCBody = function drawNPCBodyV18(npc){
    if (!npc) return;
    var role = npc.__v18Role || npcRole(npc);
    if (role === 'object') { drawJobObjectClean(npc); return; }

    /* No bobbing. Feet always touch npc.y. */
    var look = {
      taxi:['#facc15','#111827','#111827','#8d5524'],
      merchant:['#f97316','#fed7aa','#78350f','#f1c27d'],
      weapon:['#64748b','#cbd5e1','#111827','#f1c27d'],
      blacksmith:['#7c2d12','#f59e0b','#292524','#c68642'],
      quest:['#7c3aed','#fef08a','#e5e7eb','#f1c27d'],
      warrior:['#475569','#facc15','#78350f','#f1c27d'],
      mage:['#4f46e5','#93c5fd','#e5e7eb','#f1c27d'],
      rogue:['#111827','#c084fc','#0f172a','#f1c27d'],
      archer:['#15803d','#bbf7d0','#713f12','#f1c27d'],
      dragon:['#0f766e','#5eead4','#7c2d12','#f1c27d'],
      villager:['#60a5fa','#dbeafe','#6b3f22','#ffd6a6']
    }[role] || ['#60a5fa','#dbeafe','#6b3f22','#ffd6a6'];

    var main=look[0], trim=look[1], hair=look[2], skin=look[3];
    ...
    /* legs/feet */
    fillRR(ctx,-16,-31,12,25,5,'#171717');
    fillRR(ctx,4,-31,12,25,5,'#171717');
    fillRR(ctx,-13,-30,8,21,4,'#374151');
    fillRR(ctx,5,-30,8,21,4,'#374151');
    fillRR(ctx,-24,-9,22,7,2,'#171717');
    fillRR(ctx,2,-9,22,7,2,'#171717');

    /* body */
    fillRR(ctx,-22,-66,44,42,11,'#171717');
    fillRR(ctx,-18,-63,36,36,9,main);
    ctx.fillStyle=trim; ctx.fillRect(-11,-58,22,6); fillRR(ctx,-15,-36,30,8,4,trim);

    /* arms, small hands. No large held weapon. */
    line(ctx,-15,-56,-29,-42,'#171717',9);
    line(ctx,15,-56,29,-42,'#171717',9);
    line(ctx,-15,-56,-29,-42,main,6);
    line(ctx,15,-56,29,-42,main,6);
    circle(ctx,-29,-42,5,'#171717'); circle(ctx,29,-42,5,'#171717');
    circle(ctx,-29,-42,3.4,skin); circle(ctx,29,-42,3.4,skin);

    drawRoleProp(ctx, role);

    /* head */
    circle(ctx,-20,-90,5,'#171717'); circle(ctx,20,-90,5,'#171717');
    ctx.fillStyle='#171717'; ctx.beginPath(); ctx.ellipse(0,-92,22,23,0,0,Math.PI*2); ctx.fill();
    circle(ctx,-19,-90,3.7,skin); circle(ctx,19,-90,3.7,skin);
    ctx.fillStyle=skin; ctx.beginPath(); ctx.ellipse(0,-92,18.5,20.5,0,0,Math.PI*2); ctx.fill();

    /* hair / hat */
    fillRR(ctx,-16,-114,32,13,8,hair);
    if (role === 'taxi') { fillRR(ctx,-18,-122,36,9,4,'#1f2937'); ctx.fillRect(-13,-129,26,9); }
    else if (role === 'merchant') { ctx.fillStyle='#b45309'; ctx.beginPath(); ctx.ellipse(0,-113,25,7,0,0,Math.PI*2); ctx.fill(); fillRR(ctx,-12,-126,24,14,5,'#b45309'); }
    else if (role === 'mage') { poly(ctx,[[-22,-112],[0,-140],[22,-112]],'#312e81'); fillRR(ctx,-14,-117,28,6,3,'#93c5fd'); }
    else if (role === 'dragon') { poly(ctx,[[-18,-118],[-4,-132],[0,-118],[9,-132],[18,-118]],'#134e4a'); }
    else if (role === 'blacksmith') { fillRR(ctx,-18,-120,36,12,5,'#4b5563'); fillRR(ctx,-10,-126,20,7,3,'#ef4444'); }
    else { fillRR(ctx,-18,-119,36,10,5,'#334155'); }
  ...
  drawNPCs = window.drawNPCs = function drawNPCsV18(){
    normalizeTownNpcs();
    A(game.npcs).forEach(function(npc){
      if(!npc) return;
      drawNPCBody(npc);
      var name = String(npc.name || 'NPC');
      ctx.textAlign='center';
      ctx.font='bold 14px sans-serif';
      var tw = Math.min(190, Math.max(68, ctx.measureText(name).width + 26));
      ctx.fillStyle='rgba(15,23,42,.92)';
      rr(ctx,npc.x-tw/2,npc.y-118,tw,24,8);
      ctx.fillStyle='#fff';
      ctx.fillText(name,npc.x,npc.y-101);
      if(game.player && Math.abs(game.player.x-npc.x)<90 && Math.abs(game.player.y-npc.y)<110){
        ctx.fillStyle='#ffe066';
        ctx.font='bold 14px sans-serif';
        ctx.fillText(npc.__v18Role === 'object' ? 'E 조사' : 'E 대화', npc.x, npc.y-128);
      }
    });
  };

  var oldLoadTownV18 = typeof loadTown === 'function' ? loadTown : null;
  if (oldLoadTownV18 && !oldLoadTownV18.__v18Wrapped) {
    var wrappedLoadTown = function loadTownV18(id){
      var r = oldLoadTownV18.apply(this, arguments);
      try { sanitizeTownLayout(); normalizeTownNpcs(); setTimeout(function(){ sanitizeTownLayout(); normalizeTownNpcs(); },80); } catch(e){ console.warn('[V18 town cleanup]', e); }
      return r;
    };
    wrappedLoadTown.__v18Wrapped = true;
    loadTown = window.loadTown = wrappedLoadTown;
  }

  /* drawWorld final guard: if town mode, remove walkable air every frame before physics/draw confusion spreads. */
  var oldDrawWorldV18 = typeof drawWorld === 'function' ? drawWorld : null;
  if (oldDrawWorldV18 && !oldDrawWorldV18.__v18Wrapped) {
    var wrappedDrawWorld = function drawWorldV18(){
      if (game && game.mode === 'town') sanitizeTownLayout();
      oldDrawWorldV18.apply(this, arguments);
    };
    wrappedDrawWorld.__v18Wrapped = true;
    drawWorld = window.drawWorld = wrappedDrawWorld;
  }

  /* Dialog: higher, larger, and chat-hidden. Button coordinates are also updated. */
  var DLG = { x:110, y:390, w:1060, h:210, action:{x:180,y:525,w:240,h:48}, close:{x:980,y:525,w:140,h:48} };

  drawDialog = window.drawDialog = function drawDialogV18(){
    if (!game.dialog) return;
    hideChatWhenModal();
    var npc = game.dialog.npc || {};
    ctx.save();
    ctx.fillStyle='rgba(15,23,42,.985)';
    rr(ctx,DLG.x,DLG.y,DLG.w,DLG.h,18);
    ctx.strokeStyle='#93c5fd';
    ctx.lineWidth=3;
    ctx.strokeRect(DLG.x,DLG.y,DLG.w,DLG.h);

    ctx.fillStyle='#ffe066';
    ctx.font='bold 24px sans-serif';
    ctx.textAlign='left';
    ctx.fillText(npc.name || 'NPC', DLG.x+34, DLG.y+44);

    ctx.fillStyle='#e2e8f0';
    ctx.font='16px sans-serif';
    var msg = String(npc.text || '무엇을 도와드릴까요?');
    var max = 68;
    var lines = [];
    while (msg.length > max) { lines.push(msg.slice(0,max)); msg = msg.slice(max); }
    lines.push(msg);
    lines.slice(0,3).forEach(function(lineText,i){ ctx.fillText(lineText, DLG.x+34, DLG.y+78+i*24); });
    ...
    if(npc.type==='weapon') action='장비 보기';
    if(npc.type==='blacksmith') action='강화하기';
    if(npc.type==='job_object') action='조사하기';
    if(npc.type==='quest' || npc.type==='job') {
      var q = quests && quests.active ? quests.active.find(function(item){ return item.id === npc.quest; }) : null;
      if(quests && quests.completed && quests.completed.includes(npc.quest)) action='완료됨';
      else if(q) { try { syncQuestItems(q); } catch(_){} action = (typeof questComplete === 'function' && questComplete(q)) ? '완료하기' : '진행 중'; }
      else action = npc.type === 'job' ? '전직 퀘스트' : '퀘스트 수락';
    }

    ctx.fillStyle='#4dabf7';
    rr(ctx,DLG.action.x,DLG.action.y,DLG.action.w,DLG.action.h,10);
    ctx.fillStyle='#fff';
    ctx.font='bold 18px sans-serif';
    ctx.textAlign='center';
    ctx.fillText(action,DLG.action.x+DLG.action.w/2,DLG.action.y+31);

    ctx.fillStyle='#334155';
    rr(ctx,DLG.close.x,DLG.close.y,DLG.close.w,DLG.close.h,10);
    ctx.fillStyle='#fff';
    ctx.fillText('닫기',DLG.close.x+DLG.close.w/2,DLG.close.y+31);
    ctx.restore();
  };

  handleDialogClick = window.handleDialogClick = function handleDialogClickV18(x,y){
    if(!game.dialog) return;
    var npc = game.dialog.npc || {};
    if (x>=DLG.close.x && x<=DLG.close.x+DLG.close.w && y>=DLG.close.y && y<=DLG.close.y+DLG.close.h) {
      game.dialog=null; hideChatWhenModal(); return;
    }
    if (x>=DLG.action.x && x<=DLG.action.x+DLG.action.w && y>=DLG.action.y && y<=DLG.action.y+DLG.action.h) {
      if(npc.type==='taxi') { game.dialog=null; game.taxiOpen=true; hideChatWhenModal(); return; }
      if(npc.type==='merchant' || npc.type==='weapon') { game.dialog=null; game.shopOpen=npc.type; game.shopScroll=0; hideChatWhenModal(); return; }
      if(npc.type==='blacksmith') { game.dialog=null; game.blacksmithOpen=true; inventory.open=true; hideChatWhenModal(); return; }
      if(npc.type==='quest' || npc.type==='job' || npc.type==='job_object') { if(npc.quest && typeof acceptOrCompleteQuest === 'function') acceptOrCompleteQuest(npc.quest); return; }
      game.dialog=null; hideChatWhenModal(); return;
    }
  };

  /* Distinct equipment icon system. */
  function kind(it){
    it=itemOf(it)||{};
    var s=String([it.weaponType,it.equipSlot,it.slot,it.type,it.id,it.name,it.icon].join(' ')).toLowerCase();
    if (/dagger|단검|knife|비수/.test(s)) return 'dagger';
    if (/bow|활|궁|arrow/.test(s)) return 'bow';
    if (/staff|wand|rod|스태프|지팡/.test(s)) return 'staff';
    if (/axe|도끼/.test(s)) return 'axe';
    if (/spear|lance|창/.test(s)) return 'spear';
    if (/hammer|mace|club|망치|철퇴/.test(s)) return 'hammer';
    if (/gun|총|rifle|pistol/.test(s)) return 'gun';
    if (/helmet|helm|투구|헬멧/.test(s)) return 'helmet';
    if (/armor|robe|갑옷|로브|흉갑/.test(s)) return 'armor';
    if (/knee|boot|boots|부츠|무릎|신발/.test(s)) return 'boots';
    if (/accessory|ring|charm|amulet|necklace|반지|부적|목걸이|악세/.test(s)) return 'accessory';
    if (/hp_potion|체력/.test(s)) return 'hp';
    if (/mp_potion|마나/.test(s)) return 'mp';
    if (/ore|crystal|core|jelly|spore|shard|scale|feather|hide|drop|광석|결정|조각|젤리/.test(s)) return 'material';
    if (/sword|blade|검|소드|칼/.test(s)) return 'sword';
    return 'material';
  }
  function rarityColor(it){
    var s=String([it&&it.rarity,it&&it.grade,it&&it.tier,it&&it.name,it&&it.id].join(' ')).toLowerCase();
    if(/legend|전설|레전|myth|신화/.test(s)) return '#facc15';
    if(/epic|에픽|영웅/.test(s)) return '#a855f7';
    if(/ultra|unique|유니크|초희귀/.test(s)) return '#22d3ee';
    if(/rare|희귀/.test(s)) return '#3b82f6';
    return '#9ca3af';
  }
  function itemPalette(it){
    var h=hash((it&&it.id)||it&&it.name||'item');
    var packs=[
      ['#e5e7eb','#ffffff','#1d4ed8','#f59e0b'],
      ['#84cc16','#d9f99d','#7c3aed','#92400e'],
      ['#cbd5e1','#ffffff','#111827','#fb923c'],
      ['#60a5fa','#dbeafe','#1e3a8a','#f59e0b'],
      ['#f97316','#fde68a','#7c2d12','#111827'],
      ['#ec4899','#fbcfe8','#064e3b','#22c55e'],
      ['#a78bfa','#f5f3ff','#581c87','#facc15'],
      ['#14b8a6','#ccfbf1','#164e63','#eab308']
    ];
    return packs[h % packs.length];
  }

  function drawIconFrame(s,it){
    var rc=rarityColor(it);
    fillRR(ctx,-s*.47,-s*.47,s*.94,s*.94,6,'#171321');
    ctx.strokeStyle=rc; ctx.lineWidth=Math.max(1.2,s*.045); ctx.strokeRect(-s*.47,-s*.47,s*.94,s*.94);
    ctx.strokeStyle='rgba(251,146,60,.52)'; ctx.lineWidth=1; ctx.strokeRect(-s*.38,-s*.38,s*.76,s*.76);
  }

  drawItemIcon = window.drawItemIcon = function drawItemIconV18(itemOrIcon,x,y,size){
    var it = itemOf(itemOrIcon) || (typeof itemOrIcon === 'object' ? itemOrIcon : {id:String(itemOrIcon||''),icon:String(itemOrIcon||'')});
    var k = kind(it), s = size || 32, p = itemPalette(it), v = hash(it.id||it.name||k)%12;
    ctx.save(); ctx.translate(x,y); ctx.imageSmoothingEnabled=false;
    drawIconFrame(s,it);

    if(k==='hp'){
      /* original-style heart potion */
      circle(ctx,-s*.12,-s*.09,s*.15,'#ff4d4f'); circle(ctx,s*.12,-s*.09,s*.15,'#ff4d4f');
      poly(ctx,[[-s*.28,-s*.02],[0,s*.30],[s*.28,-s*.02]],'#ff4d4f'); circle(ctx,-s*.08,-s*.12,s*.04,'rgba(255,255,255,.9)');
    } else if(k==='mp'){
      ctx.fillStyle='#4dabf7'; ctx.beginPath(); ctx.moveTo(0,-s*.34); ctx.quadraticCurveTo(s*.30,0,0,s*.34); ctx.quadraticCurveTo(-s*.30,0,0,-s*.34); ctx.fill(); circle(ctx,-s*.07,-s*.08,s*.045,'rgba(255,255,255,.85)');
    } else if(k==='sword'){
      ctx.rotate(-.75 + (v%3-1)*.08);
      poly(ctx,[[0,-s*.40],[s*.10,-s*.05],[s*.07,s*.28],[-s*.07,s*.28],[-s*.10,-s*.05]],p[0]);
      line(ctx,-s*.02,-s*.34,s*.02,s*.18,p[1],Math.max(1,s*.035));
      fillRR(ctx,-s*.20,s*.16,s*.40,s*.08,2,p[3]); fillRR(ctx,-s*.06,s*.23,s*.12,s*.18,2,p[2]);
      if(v%5===0) poly(ctx,[[0,-s*.47],[s*.12,-s*.27],[s*.04,-s*.1],[s*.16,.02],[-s*.06,.04]],'#fde68a');
    } else if(k==='dagger'){
      ctx.rotate(-.66 + (v%4)*.05);
      poly(ctx,[[0,-s*.31],[s*.11,-s*.07],[s*.04,s*.18],[-s*.05,s*.18],[-s*.11,-s*.07]],p[0]);
      fillRR(ctx,-s*.15,s*.08,s*.30,s*.06,2,p[3]); fillRR(ctx,-s*.05,s*.14,s*.10,s*.19,2,p[2]);
    } else if(k==='bow'){
      ctx.strokeStyle=p[3]; ctx.lineWidth=s*.085; ctx.beginPath(); ctx.arc(0,0,s*.29,-1.23,1.23); ctx.stroke();
      line(ctx,s*.07,-s*.27,s*.07,s*.27,p[1],s*.035); fillRR(ctx,-s*.04,-s*.07,s*.08,s*.14,1,p[2]);
      if(v%2) line(ctx,-s*.22,0,s*.28,0,'#e5e7eb',s*.035);
    } else if(k==='staff'){
      ctx.rotate(-.55);
      line(ctx,0,s*.30,0,-s*.27,p[3],s*.085);
      circle(ctx,0,-s*.36,s*(v%2?.11:.14),p[0]);
      ctx.strokeStyle=p[1]; ctx.lineWidth=s*.045; ctx.beginPath(); ctx.arc(0,-s*.36,s*.19,0,Math.PI*2); ctx.stroke();
    } else if(k==='axe'){
      ctx.rotate(-.48);
      line(ctx,0,s*.31,0,-s*.27,p[3],s*.09);
      poly(ctx,[[0,-s*.28],[s*.24,-s*.20],[s*.15,-s*.01],[0,-s*.05]],p[0]);
      if(v%3!==0) poly(ctx,[[0,-s*.27],[-s*.20,-s*.17],[-s*.11,-s*.02],[0,-s*.06]],p[1]);
    } else if(k==='hammer'){
      ctx.rotate(-.48);
      line(ctx,0,s*.30,0,-s*.13,p[3],s*.09);
      if(v%2) poly(ctx,[[0,-s*.35],[s*.20,-s*.24],[s*.08,-s*.08],[-s*.08,-s*.08],[-s*.20,-s*.24]],p[0]);
      else fillRR(ctx,-s*.20,-s*.29,s*.40,s*.17,2,p[0]);
      if(v%3===0){ circle(ctx,-s*.12,-s*.23,s*.04,p[1]); circle(ctx,s*.12,-s*.23,s*.04,p[1]); }
    } else if(k==='spear'){
      ctx.rotate(-.60);
      line(ctx,0,s*.34,0,-s*.28,p[3],s*.07);
      poly(ctx,[[0,-s*.44],[s*.13,-s*.22],[0,-s*.16],[-s*.13,-s*.22]],p[0]);
      fillRR(ctx,-s*.11,-s*.18,s*.22,s*.05,2,p[2]);
    } else if(k==='gun'){
      fillRR(ctx,-s*.25,-s*.12,s*.45,s*.17,2,p[0]); fillRR(ctx,s*.15,-s*.08,s*.22,s*.07,1,p[1]); fillRR(ctx,-s*.05,s*.04,s*.11,s*.20,2,p[3]); circle(ctx,s*.38,-s*.05,s*.035,'#fde68a');
    } else if(k==='helmet'){
      poly(ctx,[[-s*.26,-s*.02],[-s*.12,-s*.24],[s*.12,-s*.24],[s*.26,-s*.02],[s*.19,s*.16],[-s*.19,s*.16]],p[0]);
      fillRR(ctx,-s*.14,-s*.03,s*.28,s*.08,2,p[2]);
      if(v%2) line(ctx,0,-s*.24,0,-s*.36,p[3],s*.045);
    } else if(k==='armor'){
      poly(ctx,[[0,-s*.32],[s*.28,-s*.14],[s*.23,s*.31],[-s*.23,s*.31],[-s*.28,-s*.14]],p[0]);
      fillRR(ctx,-s*.14,-s*.10,s*.28,s*.10,2,p[1]); fillRR(ctx,-s*.18,s*.08,s*.36,s*.08,2,p[2]);
    } else if(k==='boots'){
      fillRR(ctx,-s*.25,-s*.05,s*.21,s*.19,3,p[0]); fillRR(ctx,s*.05,-s*.05,s*.21,s*.19,3,p[2]);
      fillRR(ctx,-s*.28,s*.09,s*.25,s*.08,2,p[3]); fillRR(ctx,s*.03,s*.09,s*.25,s*.08,2,p[3]);
    } else if(k==='accessory'){
      if(v%3===0){ ctx.strokeStyle=p[3]; ctx.lineWidth=s*.10; ctx.beginPath(); ctx.arc(0,s*.04,s*.18,0,Math.PI*2); ctx.stroke(); circle(ctx,0,-s*.16,s*.08,p[0]); }
      else if(v%3===1){ ctx.strokeStyle=p[3]; ctx.lineWidth=s*.045; ctx.beginPath(); ctx.arc(0,-s*.04,s*.22,Math.PI*.15,Math.PI*.85,true); ctx.stroke(); circle(ctx,0,s*.13,s*.11,p[0]); fillRR(ctx,-s*.05,s*.03,s*.10,s*.22,2,p[1]); }
      else { poly(ctx,[[0,-s*.25],[s*.16,0],[0,s*.25],[-s*.16,0]],p[0]); ctx.strokeStyle=p[1]; ctx.lineWidth=s*.04; ctx.stroke(); }
    } else {
      /* materials: distinct by item id */
      var vv=v%5;
      if(vv===0) { poly(ctx,[[0,-s*.30],[s*.24,0],[0,s*.30],[-s*.24,0]],p[0]); circle(ctx,-s*.06,-s*.08,s*.05,p[1]); }
      else if(vv===1) { circle(ctx,0,0,s*.23,p[0]); circle(ctx,s*.08,-s*.09,s*.06,p[1]); }
      else if(vv===2) { poly(ctx,[[-s*.25,s*.18],[-s*.12,-s*.23],[s*.25,-s*.06],[s*.10,s*.25]],p[0]); }
      else if(vv===3) { fillRR(ctx,-s*.20,-s*.18,s*.40,s*.36,5,p[0]); line(ctx,-s*.12,0,s*.12,0,p[1],2); }
      else { poly(ctx,[[0,-s*.32],[s*.29,-s*.07],[s*.18,s*.28],[-s*.18,s*.28],[-s*.29,-s*.07]],p[0]); }
    }
    ctx.restore();
  };

  drawWeapon = window.drawWeapon = function drawWeaponV18(c, weaponRef, handX, handY, attacking, attackKind, animTime){
    var it=itemOf(weaponRef || (typeof equipment !== 'undefined' && equipment && equipment.weapon));
    if(!it) return;
    var k=kind(it), p=itemPalette(it), v=hash(it.id||it.name||k)%12;
    var face=(game&&game.player&&game.player.face)||1;
    var swing=attacking ? Math.sin(Math.min(1, n(animTime,0)*10)*Math.PI) : 0;

    c.save();
    c.translate(handX,handY);
    c.scale(face,1);
    c.rotate(attacking ? (-.78+swing*1.25) : -.42);
    c.imageSmoothingEnabled=false;
    c.shadowColor=rarityColor(it);
    c.shadowBlur=/전설|legend/i.test(String(it.name||it.id||'')) ? 14 : 0;

    if(k==='bow'){
      c.strokeStyle=p[3]; c.lineWidth=5; c.beginPath(); c.arc(10,-14,34,-1.35,1.35); c.stroke();
      line(c,18,-48,18,20,p[1],1.5); fillRR(c,6,-18,7,12,2,p[2]);
      if(attacking){ line(c,14,-14,82,-14,'#f8fafc',2); poly(c,[[88,-14],[72,-21],[72,-7]],'#facc15'); }
    } else if(k==='staff'){
      line(c,0,22,0,-60,p[3],5); circle(c,0,-69,v%2?10:13,p[0]);
      c.strokeStyle=p[1]; c.lineWidth=2; c.beginPath(); c.arc(0,-69,19,0,Math.PI*2); c.stroke();
      if(attacking){ c.globalAlpha=.65; c.beginPath(); c.arc(0,-69,30+swing*18,0,Math.PI*2); c.stroke(); c.globalAlpha=1; }
    } else if(k==='dagger'){
      c.rotate(.45); poly(c,[[0,-39],[9,-6],[3,13],[-5,13],[-9,-6]],p[0]); fillRR(c,-12,9,24,7,2,p[3]); fillRR(c,-4,15,8,19,2,p[2]);
    } else if(k==='axe'){
      line(c,0,22,0,-58,p[3],5); poly(c,[[0,-57],[26,-46],[18,-18],[0,-28]],p[0]); if(v%3) poly(c,[[0,-55],[-24,-45],[-14,-20],[0,-29]],p[1]);
    } else if(k==='hammer'){
      line(c,0,20,0,-40,p[3],5); if(v%2) poly(c,[[0,-62],[20,-50],[10,-30],[-10,-30],[-20,-50]],p[0]); else fillRR(c,-22,-58,44,18,3,p[0]);
    } else if(k==='spear'){
      line(c,0,22,0,-76,p[3],4); poly(c,[[0,-96],[15,-72],[0,-63],[-15,-72]],p[0]);
    } else if(k==='gun'){
      fillRR(c,-5,-28,52,16,3,p[0]); fillRR(c,35,-24,24,7,2,p[1]); fillRR(c,5,-12,11,24,2,p[3]);
    } else {
      var len = 58 + (v%3)*6;
      poly(c,[[0,-len],[12,-20],[7,18],[-7,18],[-12,-20]],p[0]);
      line(c,-2,-len+8,2,10,p[1],1.5);
      fillRR(c,-18,8,36,8,2,p[3]); fillRR(c,-5,14,10,22,2,p[2]);
      if(v%5===0){ c.globalAlpha=.9; poly(c,[[0,-len-9],[12,-len+9],[4,-len+28],[16,-len+44],[-2,-len+40],[4,-len+18]],'#fde68a'); c.globalAlpha=1; }
    }
    c.restore();
  };

  var oldDrawHUDV18 = typeof drawHUD === 'function' ? drawHUD : null;
  if (oldDrawHUDV18 && !oldDrawHUDV18.__v18Wrapped) {
    var wrappedHUD = function drawHUDV18(){
      hideChatWhenModal();
      if (game && game.mode === 'town') { try { sanitizeTownLayout(); normalizeTownNpcs(); } catch(_){} }
      oldDrawHUDV18.apply(this, arguments);
      try {
        ctx.save();
        ctx.textAlign='right';
        ctx.font='bold 16px sans-serif';
        ...
        ctx.lineWidth=5;
        ctx.strokeText('V18 안정화 적용됨', W-22, 372);
        ctx.fillText('V18 안정화 적용됨', W-22, 372);
        ctx.fillStyle='#fcd34d';
        ctx.strokeText('NPC 지면고정 · 대화창 우선 · 장비외형 개선', W-22, 394);
        ctx.fillText('NPC 지면고정 · 대화창 우선 · 장비외형 개선', W-22, 394);
        ctx.restore();
      } catch(_){}
    };
    wrappedHUD.__v18Wrapped = true;
    drawHUD = window.drawHUD = wrappedHUD;
  }

  setInterval(function(){
    try {
      hideChatWhenModal();
      if (typeof game !== 'undefined' && game && game.mode === 'town') {
        sanitizeTownLayout();
        normalizeTownNpcs();
      }
    } catch(_) {}
  ...
  console.log('[PixelRPG] V18 stability polish installed');
})();


/* =========================================================
   V19 MOVEMENT / NPC PLACEMENT / EQUIPMENT QUALITY PATCH
   - NPCs are arranged in natural clusters instead of a straight line.
   - Jump height restored and slightly improved.
   - Held weapons are moved upward/forward so the character clearly holds them.
   - Weapon swing motion and attack trails are now different by weapon type.
   - Item borders and weapon effects scale by grade/quality.
========================================================= */
(function(){
  'use strict';
  if (window.__PIXEL_RPG_V19_EQUIPMENT_MOTION_POLISH__) return;
  window.__PIXEL_RPG_V19_EQUIPMENT_MOTION_POLISH__ = true;
  window.PIXEL_RPG_PATCH_VERSION = 'V19_EQUIPMENT_MOTION_POLISH';

  function A(v){ return Array.isArray(v) ? v : []; }
  function num(v,d){ v=Number(v); return Number.isFinite(v)?v:d; }
  function clamp2(v,a,b){ return Math.max(a, Math.min(b,v)); }
  function hsh(s){ s=String(s||''); var h=0; for(var i=0;i<s.length;i++) h=((h<<5)-h+s.charCodeAt(i))|0; return Math.abs(h); }
  function rr(c,x,y,w,h,r){ if(typeof roundRect === 'function') return roundRect(c,x,y,w,h,r||6); c.beginPath(); if(c.roundRect)c.roundRect(x,y,w,h,r||6); else c.rect(x,y,w,h); c.fill(); }
  function fillRR(c,x,y,w,h,r,col){ c.fillStyle=col; rr(c,x,y,w,h,r); }
  function strokeRR(c,x,y,w,h,r,col,lw){ c.strokeStyle=col; c.lineWidth=lw||1; c.beginPath(); if(c.roundRect)c.roundRect(x,y,w,h,r||6); else c.rect(x,y,w,h); c.stroke(); }
  function circle(c,x,y,r,col){ c.fillStyle=col; c.beginPath(); c.arc(x,y,r,0,Math.PI*2); c.fill(); }
  function line(c,x1,y1,x2,y2,col,lw){ c.strokeStyle=col; c.lineWidth=lw||2; c.lineCap='round'; c.beginPath(); c.moveTo(x1,y1); c.lineTo(x2,y2); c.stroke(); }
  function poly(c,pts,col){ c.fillStyle=col; c.beginPath(); pts.forEach(function(p,i){ if(i)c.lineTo(p[0],p[1]); else c.moveTo(p[0],p[1]); }); c.closePath(); c.fill(); }
  function idOf(ref){ try { return typeof itemRefId === 'function' ? itemRefId(ref) : (typeof ref === 'string' ? ref : ref && ref.id); } catch(_) { return typeof ref === 'string' ? ref : ref && ref.id; } }
  function itemOf(ref){
    if(!ref) return null;
    if(typeof ref === 'object' && (ref.id || ref.name || ref.icon)) return Object.assign({}, (typeof ITEMS !== 'undefined' && ITEMS && ref.id && ITEMS[ref.id]) || {}, ref);
    var id=idOf(ref);
    return (typeof ITEMS !== 'undefined' && ITEMS && ITEMS[id]) ? Object.assign({id:id}, ITEMS[id]) : (id ? {id:id,name:id,icon:id} : null);
  }
  function slotOf(it){ try { return typeof getEquipSlotForItem === 'function' ? getEquipSlotForItem(it||{}) : ((it&&it.equipSlot)||(it&&it.type)); } catch(_) { return (it&&it.equipSlot)||(it&&it.type); } }

  function gradeInfo(it){
    it = itemOf(it) || {};
    var s = String([it.rarity,it.grade,it.tier,it.name,it.id].join(' ')).toLowerCase();
    var req = num(it.reqLevel || it.level || it.lv, 1);
    var score = 0, key='common', ko='일반', color='#9ca3af', glow=0;
    if(/legend|전설|레전|myth|신화/.test(s) || req >= 55){ score=4; key='legendary'; ko='전설'; color='#facc15'; glow=22; }
    else if(/epic|에픽|영웅/.test(s) || req >= 38){ score=3; key='epic'; ko='에픽'; color='#a855f7'; glow=16; }
    else if(/ultra|unique|유니크|초희귀/.test(s) || req >= 25){ score=2; key='unique'; ko='유니크'; color='#22d3ee'; glow=11; }
    else if(/rare|희귀/.test(s) || req >= 12){ score=1; key='rare'; ko='희귀'; color='#3b82f6'; glow=7; }
    return {score:score,key:key,ko:ko,color:color,glow:glow};
  }
  function kind(it){
    it=itemOf(it)||{};
    var s=String([it.weaponType,it.equipSlot,it.slot,it.type,it.id,it.name,it.icon].join(' ')).toLowerCase();
  ...
  function palette(it){
    var g=gradeInfo(it), v=hsh((it&&it.id)||it&&it.name||'x')%10;
    var base=[
      ['#e5e7eb','#ffffff','#1d4ed8','#f59e0b'],
      ['#84cc16','#d9f99d','#7c3aed','#92400e'],
      ['#60a5fa','#dbeafe','#1e3a8a','#f59e0b'],
      ...
      ['#a78bfa','#f5f3ff','#581c87','#facc15'],
      ['#14b8a6','#ccfbf1','#164e63','#eab308'],
      ['#94a3b8','#f8fafc','#334155','#fb923c'],
      ['#fca5a5','#fee2e2','#7f1d1d','#fbbf24'],
      ['#c4b5fd','#faf5ff','#312e81','#38bdf8']
    ][v];
    if(g.score>=3) base=[base[0], '#ffffff', base[2], g.color];
    if(g.score>=4) base=['#fef3c7','#ffffff','#7c2d12','#facc15'];
    return base;
  }

  /* 1) Jump restoration: boost only at jump-start, so normal gravity/physics remain stable. */
  var oldUpdatePlayerV19 = typeof updatePlayer === 'function' ? updatePlayer : null;
  if(oldUpdatePlayerV19 && !oldUpdatePlayerV19.__v19Wrapped){
    var wrappedUpdatePlayer = function updatePlayerV19(dt){
      var p = game && game.player;
      var jumpPressed = keys && (keys.has(' ') || keys.has('arrowup'));
      var wasGrounded = !!(p && p.grounded);
      oldUpdatePlayerV19.apply(this, arguments);
      if(p && jumpPressed && wasGrounded && p.vy < -300 && p.vy > -760){
        p.vy = -700;          // V18 felt too low; this is high enough without breaking ladders/platforms.
        p.grounded = false;
        p.__v19JumpBoosted = true;
      }
    };
    wrappedUpdatePlayer.__v19Wrapped = true;
    updatePlayer = window.updatePlayer = wrappedUpdatePlayer;
  }

  /* 2) Natural NPC clusters. Still ground-locked, but no more ruler-straight spacing. */
  ...
  function clusterTownNpcs(){
    if(!game || game.mode!=='town') return;
    game.ground = num(game.ground,560);
    game.platforms = [];
    game.ladders = [];
    var basic = { taxi:390, merchant:720, weapon:1040, blacksmith:1285, quest:1525 };
    var townSeed = hsh(game.townId || 'lumina');
    var jobSlots = [
      1780, 1985, 2255, 2470, 2785, 3020, 3320, 3545, 3860
    ].map(function(x,i){
      var drift = ((townSeed + i*37) % 55) - 27;
      return x + drift;
    });
    var jobI=0;
    var used=[];
    A(game.npcs).forEach(function(npc){
      if(!npc) return;
      var role = roleOfNpc(npc);
      npc.__v19Role = role;
      if(basic[role]) npc.x = basic[role] + (((townSeed + hsh(role)) % 31) - 15);
      else {
        npc.x = jobSlots[jobI % jobSlots.length] + Math.floor(jobI / jobSlots.length) * 155;
        jobI++;
      }
      /* prevent accidental overlap while keeping "clustered" feeling */
      used.forEach(function(x){
        if(Math.abs(npc.x - x) < 82) npc.x = x + 92 + ((townSeed % 23) - 11);
      });
      npc.x = clamp2(npc.x, 80, Math.max(900,(game.width||4300)-120));
      npc.y = game.ground;
      npc.vx=0; npc.vy=0; npc.grounded=true;
      used.push(npc.x);
    });
    var p=game.player;
    if(p && game.mode==='town' && p.y < game.ground-10 && !p.climbing){ p.y=game.ground; p.vy=0; p.grounded=true; }
  }
  var oldLoadTownV19 = typeof loadTown === 'function' ? loadTown : null;
  if(oldLoadTownV19 && !oldLoadTownV19.__v19Wrapped){
    var wrappedLoadTown = function loadTownV19(){
      var r=oldLoadTownV19.apply(this, arguments);
      try{ clusterTownNpcs(); setTimeout(clusterTownNpcs,80); setTimeout(clusterTownNpcs,350); }catch(e){ console.warn('[V19 npc cluster]',e); }
      return r;
    };
    wrappedLoadTown.__v19Wrapped=true;
    loadTown = window.loadTown = wrappedLoadTown;
  }
  var oldDrawNPCsV19 = typeof drawNPCs === 'function' ? drawNPCs : null;
  if(oldDrawNPCsV19 && !oldDrawNPCsV19.__v19Wrapped){
    var wrappedDrawNPCs = function drawNPCsV19(){
      try{ clusterTownNpcs(); }catch(_){}
      oldDrawNPCsV19.apply(this, arguments);
    };
    wrappedDrawNPCs.__v19Wrapped=true;
    drawNPCs = window.drawNPCs = wrappedDrawNPCs;
  }

  /* 3) Higher-quality inventory/equipment icon frame and distinct silhouettes. */
  function iconFrame(s,it){
    var g=gradeInfo(it), col=g.color;
    fillRR(ctx,-s*.49,-s*.49,s*.98,s*.98,7,'#171321');
    ctx.shadowColor=col; ctx.shadowBlur=g.glow;
    strokeRR(ctx,-s*.49,-s*.49,s*.98,s*.98,7,col,Math.max(1.4,s*.05));
    ctx.shadowBlur=0;
    if(g.score>=2){ strokeRR(ctx,-s*.39,-s*.39,s*.78,s*.78,5,'rgba(255,255,255,.48)',1); }
    if(g.score>=3){
      ctx.fillStyle=col;
      for(var i=0;i<4;i++){
        var sx=(i<2?-1:1)*s*.39, sy=(i%2?-1:1)*s*.39;
        circle(ctx,sx,sy,s*.045,col);
      }
    }
    if(g.score>=4){
      ctx.strokeStyle='rgba(255,255,255,.85)';
      ctx.lineWidth=1.2;
      ctx.beginPath(); ctx.arc(0,0,s*.54,0,Math.PI*2); ctx.stroke();
    }
  }
  drawItemIcon = window.drawItemIcon = function drawItemIconV19(itemOrIcon,x,y,size){
    var it=itemOf(itemOrIcon) || (typeof itemOrIcon==='object' ? itemOrIcon : {id:String(itemOrIcon||''),icon:String(itemOrIcon||'')});
    var k=kind(it), s=size||32, p=palette(it), v=hsh((it.id||it.name||k))%15, g=gradeInfo(it);
    ctx.save(); ctx.translate(x,y); ctx.imageSmoothingEnabled=false;
    iconFrame(s,it);
    ctx.shadowColor=g.color; ctx.shadowBlur=g.score>=3?g.glow*.65:0;

    if(k==='hp'){
      circle(ctx,-s*.12,-s*.09,s*.15,'#ff4d4f'); circle(ctx,s*.12,-s*.09,s*.15,'#ff4d4f'); poly(ctx,[[-s*.28,-s*.02],[0,s*.30],[s*.28,-s*.02]],'#ff4d4f'); circle(ctx,-s*.08,-s*.12,s*.04,'rgba(255,255,255,.9)');
    } else if(k==='mp'){
      ctx.fillStyle='#4dabf7'; ctx.beginPath(); ctx.moveTo(0,-s*.34); ctx.quadraticCurveTo(s*.30,0,0,s*.34); ctx.quadraticCurveTo(-s*.30,0,0,-s*.34); ctx.fill(); circle(ctx,-s*.07,-s*.08,s*.045,'rgba(255,255,255,.85)');
    } else if(k==='sword'){
      ctx.rotate(-.74+(v%3-1)*.08); poly(ctx,[[0,-s*.42],[s*.11,-s*.06],[s*.08,s*.29],[-s*.08,s*.29],[-s*.11,-s*.06]],p[0]); line(ctx,-s*.02,-s*.34,s*.02,s*.19,p[1],Math.max(1,s*.035)); fillRR(ctx,-s*.22,s*.16,s*.44,s*.08,2,p[3]); fillRR(ctx,-s*.06,s*.23,s*.12,s*.19,2,p[2]); if(g.score>=3||v%5===0) poly(ctx,[[0,-s*.50],[s*.12,-s*.29],[s*.04,-s*.10],[s*.16,.02],[-s*.06,.04]],g.color);
    } else if(k==='dagger'){
      ctx.rotate(-.62+(v%4)*.06); poly(ctx,[[0,-s*.33],[s*.12,-s*.07],[s*.04,s*.19],[-s*.05,s*.19],[-s*.12,-s*.07]],p[0]); fillRR(ctx,-s*.16,s*.08,s*.32,s*.06,2,p[3]); fillRR(ctx,-s*.05,s*.14,s*.10,s*.20,2,p[2]); if(g.score>=2) line(ctx,-s*.10,-s*.24,s*.10,s*.04,g.color,1.5);
    } else if(k==='bow'){
      ctx.strokeStyle=p[3]; ctx.lineWidth=s*.09; ctx.beginPath(); ctx.arc(0,0,s*.30,-1.25,1.25); ctx.stroke(); line(ctx,s*.08,-s*.28,s*.08,s*.28,p[1],s*.035); fillRR(ctx,-s*.045,-s*.075,s*.09,s*.15,1,p[2]); if(g.score>=2||v%2) line(ctx,-s*.25,0,s*.30,0,'#e5e7eb',s*.035);
    } else if(k==='staff'){
      ctx.rotate(-.55); line(ctx,0,s*.31,0,-s*.28,p[3],s*.09); circle(ctx,0,-s*.37,s*(g.score>=3?.15:.11),p[0]); ctx.strokeStyle=g.color; ctx.lineWidth=s*.045; ctx.beginPath(); ctx.arc(0,-s*.37,s*(g.score>=3?.23:.18),0,Math.PI*2); ctx.stroke();
    } else if(k==='axe'){
      ctx.rotate(-.50); line(ctx,0,s*.32,0,-s*.28,p[3],s*.09); poly(ctx,[[0,-s*.30],[s*.25,-s*.21],[s*.16,-s*.01],[0,-s*.05]],p[0]); if(v%3!==0) poly(ctx,[[0,-s*.28],[-s*.22,-s*.18],[-s*.12,-s*.02],[0,-s*.06]],p[1]); if(g.score>=3) circle(ctx,s*.10,-s*.17,s*.045,g.color);
    } else if(k==='hammer'){
      ctx.rotate(-.48); line(ctx,0,s*.31,0,-s*.14,p[3],s*.09); if(v%2) poly(ctx,[[0,-s*.36],[s*.21,-s*.25],[s*.09,-s*.08],[-s*.09,-s*.08],[-s*.21,-s*.25]],p[0]); else fillRR(ctx,-s*.21,-s*.30,s*.42,s*.18,2,p[0]); if(g.score>=2){ circle(ctx,-s*.12,-s*.24,s*.04,p[1]); circle(ctx,s*.12,-s*.24,s*.04,p[1]); }
    } else if(k==='spear'){
      ctx.rotate(-.60); line(ctx,0,s*.34,0,-s*.29,p[3],s*.07); poly(ctx,[[0,-s*.45],[s*.14,-s*.23],[0,-s*.16],[-s*.14,-s*.23]],p[0]); fillRR(ctx,-s*.12,-s*.18,s*.24,s*.05,2,p[2]);
    } else if(k==='gun'){
      fillRR(ctx,-s*.27,-s*.13,s*.48,s*.18,2,p[0]); fillRR(ctx,s*.15,-s*.09,s*.24,s*.08,1,p[1]); fillRR(ctx,-s*.05,s*.04,s*.12,s*.21,2,p[3]); circle(ctx,s*.41,-s*.05,s*.035,g.color);
    } else if(k==='helmet'){
      poly(ctx,[[-s*.27,-s*.02],[-s*.12,-s*.25],[s*.12,-s*.25],[s*.27,-s*.02],[s*.20,s*.17],[-s*.20,s*.17]],p[0]); fillRR(ctx,-s*.15,-s*.03,s*.30,s*.08,2,p[2]); if(g.score>=2) line(ctx,0,-s*.25,0,-s*.39,g.color,s*.045);
    } else if(k==='armor'){
      poly(ctx,[[0,-s*.33],[s*.29,-s*.14],[s*.24,s*.32],[-s*.24,s*.32],[-s*.29,-s*.14]],p[0]); fillRR(ctx,-s*.15,-s*.11,s*.30,s*.10,2,p[1]); fillRR(ctx,-s*.19,s*.08,s*.38,s*.08,2,p[2]); if(g.score>=3) line(ctx,-s*.17,s*.22,s*.17,-s*.03,g.color,2);
    } else if(k==='boots'){
      fillRR(ctx,-s*.26,-s*.05,s*.22,s*.20,3,p[0]); fillRR(ctx,s*.05,-s*.05,s*.22,s*.20,3,p[2]); fillRR(ctx,-s*.29,s*.09,s*.26,s*.08,2,p[3]); fillRR(ctx,s*.03,s*.09,s*.26,s*.08,2,p[3]);
    } else if(k==='accessory'){
      if(v%3===0){ ctx.strokeStyle=p[3]; ctx.lineWidth=s*.10; ctx.beginPath(); ctx.arc(0,s*.04,s*.18,0,Math.PI*2); ctx.stroke(); circle(ctx,0,-s*.16,s*.08,g.color); }
      else if(v%3===1){ ctx.strokeStyle=p[3]; ctx.lineWidth=s*.045; ctx.beginPath(); ctx.arc(0,-s*.04,s*.23,Math.PI*.15,Math.PI*.85,true); ctx.stroke(); circle(ctx,0,s*.13,s*.115,g.color); fillRR(ctx,-s*.05,s*.03,s*.10,s*.22,2,p[1]); }
      else { poly(ctx,[[0,-s*.26],[s*.17,0],[0,s*.26],[-s*.17,0]],g.color); ctx.strokeStyle=p[1]; ctx.lineWidth=s*.04; ctx.stroke(); }
    } else {
      var vv=v%6; if(vv===0){ poly(ctx,[[0,-s*.31],[s*.25,0],[0,s*.31],[-s*.25,0]],p[0]); circle(ctx,-s*.06,-s*.08,s*.05,p[1]); }
      else if(vv===1){ circle(ctx,0,0,s*.23,p[0]); circle(ctx,s*.08,-s*.09,s*.06,p[1]); }
      else if(vv===2){ poly(ctx,[[-s*.25,s*.18],[-s*.12,-s*.23],[s*.25,-s*.06],[s*.10,s*.25]],p[0]); }
      else if(vv===3){ fillRR(ctx,-s*.20,-s*.18,s*.40,s*.36,5,p[0]); line(ctx,-s*.12,0,s*.12,0,p[1],2); }
      else if(vv===4){ poly(ctx,[[0,-s*.32],[s*.29,-s*.07],[s*.18,s*.28],[-s*.18,s*.28],[-s*.29,-s*.07]],p[0]); }
      else { line(ctx,-s*.22,s*.22,s*.22,-s*.22,p[0],s*.11); circle(ctx,s*.23,-s*.23,s*.08,p[1]); }
    }
    ctx.restore();
  };

  function drawTrail(c,k,g,sw,len){
    if(!sw || sw<=0) return;
    c.save();
    c.globalAlpha = .28 + g.score*.11;
    c.strokeStyle = g.color;
    c.lineWidth = 4 + g.score*1.6;
    c.lineCap = 'round';
    if(k==='sword' || k==='axe' || k==='hammer'){
      c.beginPath(); c.arc(0,-len*.45,48+g.score*9,-.95,1.05+sw*.35); c.stroke();
      if(g.score>=3){ c.globalAlpha=.22; c.beginPath(); c.arc(0,-len*.45,66+g.score*10,-.8,1.18+sw*.45); c.stroke(); }
    } else if(k==='dagger'){
      for(var i=0;i<2+g.score;i++){ c.beginPath(); c.moveTo(10+i*7,-24-i*10); c.lineTo(52+i*7,-28-i*10); c.stroke(); }
    } else if(k==='bow'){
      c.beginPath(); c.moveTo(20,-18); c.lineTo(92+g.score*18,-18); c.stroke();
      if(g.score>=2){ c.beginPath(); c.moveTo(10,-26); c.lineTo(80,-26); c.stroke(); }
    } else if(k==='staff'){
      c.beginPath(); c.arc(0,-len-12,28+g.score*10,0,Math.PI*2); c.stroke();
      if(g.score>=3){ c.beginPath(); c.arc(0,-len-12,48+g.score*10,0,Math.PI*2); c.stroke(); }
    } else if(k==='spear'){
      c.beginPath(); c.moveTo(0,-len-25); c.lineTo(105+g.score*22,-len-25); c.stroke();
    } else if(k==='gun'){
      c.beginPath(); c.moveTo(40,-20); c.lineTo(115+g.score*18,-20); c.stroke();
    }
    c.restore();
  }

  drawWeapon = window.drawWeapon = function drawWeaponV19(c, weaponRef, handX, handY, attacking, attackKind, animTime){
    var it=itemOf(weaponRef || (typeof equipment !== 'undefined' && equipment && equipment.weapon));
    if(!it) return;
    var k=kind(it), p=palette(it), g=gradeInfo(it), seed=hsh((it.id||'')+(it.name||'')), req=num(it.reqLevel||it.level||1,1);
    var face=(game&&game.player&&game.player.face)||1;
    var t=num(animTime,0);
    var sw=attacking?Math.sin(Math.min(1,t*12)*Math.PI):0;
    var len=50 + Math.min(34, req*.55) + g.score*7;

    c.save();
    /* Move weapon up and slightly forward so the hand clearly holds it. */
    c.translate(handX + face*11, handY - 24);
    c.scale(face,1);

    var baseRot = -.30;
    if(k==='bow') baseRot = -.10;
    else if(k==='staff') baseRot = -.22;
    else if(k==='dagger') baseRot = -.18;
    else if(k==='spear') baseRot = -.28;
    else if(k==='gun') baseRot = -.05;
    else if(k==='hammer' || k==='axe') baseRot = -.36;

    var attackRot = baseRot;
    if(attacking){
      if(k==='bow') attackRot = -.06 + sw*.10;
      else if(k==='staff') attackRot = -.35 + sw*.42;
      else if(k==='dagger') attackRot = -.08 + sw*.75;
      else if(k==='spear') attackRot = -.38 + sw*.55;
      else if(k==='gun') attackRot = -.02;
      else attackRot = -.95 + sw*1.48;
    }
    c.rotate(attacking ? attackRot : baseRot);
    c.imageSmoothingEnabled=false;
    c.shadowColor=g.color; c.shadowBlur=g.glow;

    if(k==='bow'){
      c.strokeStyle=p[3]; c.lineWidth=5; c.beginPath(); c.arc(4,-17,36,-1.35,1.35); c.stroke();
      line(c,17,-52,17,18,p[1],1.5);
      fillRR(c,3,-22,8,14,2,p[2]);
      line(c,-15,-17,44,-17,p[0],3);
      if(attacking){ line(c,12,-17,92+g.score*18,-17,'#f8fafc',2.2); poly(c,[[98+g.score*18,-17],[80+g.score*18,-25],[80+g.score*18,-9]],g.color); }
      drawTrail(c,k,g,sw,len);
    } else if(k==='staff'){
      line(c,0,24,0,-len,p[3],5);
      circle(c,0,-len-10,g.score>=3?14:10,p[0]);
      c.strokeStyle=g.color; c.lineWidth=2.2; c.beginPath(); c.arc(0,-len-10,20+g.score*4,0,Math.PI*2); c.stroke();
      if(g.score>=2){ circle(c,-7,-len-17,3,'#fff'); circle(c,8,-len-5,3,g.color); }
      drawTrail(c,k,g,sw,len);
    } else if(k==='dagger'){
      c.rotate(.40);
      poly(c,[[0,-len*.70],[10,-7],[3,14],[-5,14],[-10,-7]],p[0]);
      poly(c,[[0,-len*.58],[4,-7],[0,4],[-4,-7]],p[1]);
      fillRR(c,-13,9,26,7,2,p[3]); fillRR(c,-4,15,8,20,2,p[2]);
      drawTrail(c,k,g,sw,len);
    } else if(k==='axe'){
      line(c,0,24,0,-len,p[3],5);
      poly(c,[[0,-len+2],[28,-len+14],[19,-len+44],[0,-len+30]],p[0]);
      if(seed%3!==0) poly(c,[[0,-len+2],[-26,-len+15],[-16,-len+41],[0,-len+30]],p[1]);
      if(g.score>=3) circle(c,12,-len+18,5,g.color);
      drawTrail(c,k,g,sw,len);
    } else if(k==='hammer'){
      line(c,0,22,0,-len+18,p[3],5.5);
      if(seed%2) poly(c,[[0,-len-8],[22,-len+4],[10,-len+24],[-10,-len+24],[-22,-len+4]],p[0]);
      else fillRR(c,-24,-len,48,20,3,p[0]);
      if(g.score>=2){ circle(c,-14,-len+5,4,p[1]); circle(c,14,-len+5,4,p[1]); }
      drawTrail(c,k,g,sw,len);
    } else if(k==='spear'){
      line(c,0,25,0,-len-16,p[3],4.5);
      poly(c,[[0,-len-40],[16,-len-13],[0,-len-4],[-16,-len-13]],p[0]);
      fillRR(c,-13,-len-6,26,6,2,p[2]);
      drawTrail(c,k,g,sw,len);
    } else if(k==='gun'){
      fillRR(c,-7,-30,58,17,3,p[0]); fillRR(c,38,-26,28,8,2,p[1]); fillRR(c,5,-14,12,26,2,p[3]);
      if(attacking){ circle(c,72,-22,5,g.color); drawTrail(c,k,g,sw,len); }
    } else {
      /* sword default */
      poly(c,[[0,-len],[13,-21],[8,19],[-8,19],[-13,-21]],p[0]);
      line(c,-2,-len+8,2,11,p[1],1.7);
      fillRR(c,-19,8,38,8,2,p[3]); fillRR(c,-5,14,10,23,2,p[2]);
      circle(c,0,40,4,p[3]);
      if(g.score>=3 || seed%5===0){ c.globalAlpha=.95; poly(c,[[0,-len-10],[13,-len+8],[4,-len+30],[17,-len+45],[-2,-len+41],[4,-len+18]],g.color); c.globalAlpha=1; }
      drawTrail(c,k,g,sw,len);
    }
    c.shadowBlur=0;
    c.restore();
  ...
  var oldHudV19=typeof drawHUD==='function'?drawHUD:null;
  if(oldHudV19 && !oldHudV19.__v19Wrapped){
    var wrappedHud = function drawHUDV19(){
      try{ if(game && game.mode==='town') clusterTownNpcs(); }catch(_){}
      oldHudV19.apply(this, arguments);
      try{
        ctx.save();
        ctx.textAlign='right';
        ctx.font='bold 16px sans-serif';
        ...
        ctx.strokeStyle='rgba(15,23,42,.95)';
        ctx.lineWidth=5;
        ctx.strokeText('V19 모션/장비 퀄리티 적용됨', W-22, 418);
        ctx.fillText('V19 모션/장비 퀄리티 적용됨', W-22, 418);
        ctx.fillStyle='#fcd34d';
        ctx.strokeText('점프력 복구 · 무기 위치/휘두르기/등급 테두리 개선', W-22, 440);
        ctx.fillText('점프력 복구 · 무기 위치/휘두르기/등급 테두리 개선', W-22, 440);
        ctx.restore();
      }catch(_){}
    };
    wrappedHud.__v19Wrapped=true;
    drawHUD = window.drawHUD = wrappedHud;
  }

  setInterval(function(){ try{ if(game && game.mode==='town') clusterTownNpcs(); }catch(_){} }, 1200);
  try{ if(game && game.mode==='town') clusterTownNpcs(); }catch(_){}
  console.log('[PixelRPG] V19 equipment and motion polish installed');
})();


/* =========================================================
   V20 MAPLE-LIKE TOWN / PORTAL / CHARACTER / STAFF POLISH
   - Town is layered like a small hunting map: platforms + ladders.
   - NPCs are placed on proper floors with role buildings behind them.
   - Taxi NPC is visually replaced by an interactive portal. Cost/taxi logic remains.
   - Player/NPC body is more compact: head/body connected like classic pixel RPG.
   - Staff and all weapons are held correctly; no double-direction flip.
   - Staff casts forward with orb/ring effects instead of swinging from the air.
   - Enhancement success/destruction effects are visible and dramatic.
   - Monsters get a softer, more cohesive pixel shape pass.
========================================================= */
(function(){
  'use strict';
  if (window.__PIXEL_RPG_V20_MAPLELIKE_POLISH__) return;
  window.__PIXEL_RPG_V20_MAPLELIKE_POLISH__ = true;
  window.PIXEL_RPG_PATCH_VERSION = 'V20_MAPLELIKE_POLISH';
  ...
  function gradeInfo(it){
    it=itemOf(it)||{};
    var s=String([it.rarity,it.grade,it.tier,it.name,it.id].join(' ')).toLowerCase();
    var req=N(it.reqLevel||it.level||it.lv,1);
    if(/legend|전설|레전|myth|신화/.test(s)||req>=55) return {score:4,key:'legendary',color:'#facc15',glow:24};
    if(/epic|에픽|영웅/.test(s)||req>=38) return {score:3,key:'epic',color:'#a855f7',glow:17};
    if(/ultra|unique|유니크|초희귀/.test(s)||req>=25) return {score:2,key:'unique',color:'#22d3ee',glow:12};
    if(/rare|희귀/.test(s)||req>=12) return {score:1,key:'rare',color:'#3b82f6',glow:8};
    return {score:0,key:'common',color:'#9ca3af',glow:0};
  }
  function kind(it){
    it=itemOf(it)||{};
    var s=String([it.weaponType,it.equipSlot,it.slot,it.type,it.id,it.name,it.icon].join(' ')).toLowerCase();
  ...
  function pal(it){
    var g=gradeInfo(it), v=H((it&&it.id)||it&&it.name||'x')%9;
    var p=[
      ['#e5e7eb','#ffffff','#1d4ed8','#f59e0b'],
      ['#84cc16','#d9f99d','#7c3aed','#92400e'],
      ['#60a5fa','#dbeafe','#1e3a8a','#f59e0b'],
    ...
    if(g.score>=4) p=['#fef3c7','#ffffff','#7c2d12','#facc15'];
    else if(g.score>=3) p=[p[0],'#ffffff',p[2],g.color];
    return p;
  }

  /* ---------- Layered Town Map ---------- */
  function townPlatformsV20(){
    var ground=N(game&&game.ground,560);
    return [
      {x:120, y:ground-104, w:620, h:24, townFloor:true, layer:1},
      {x:900, y:ground-104, w:560, h:24, townFloor:true, layer:1},
      {x:1660, y:ground-104, w:760, h:24, townFloor:true, layer:1},
      {x:2700, y:ground-104, w:760, h:24, townFloor:true, layer:1},
      {x:3820, y:ground-104, w:610, h:24, townFloor:true, layer:1},

      {x:330, y:ground-218, w:520, h:24, townFloor:true, layer:2},
      {x:1180, y:ground-218, w:680, h:24, townFloor:true, layer:2},
      {x:2360, y:ground-218, w:690, h:24, townFloor:true, layer:2},
      {x:3400, y:ground-218, w:560, h:24, townFloor:true, layer:2},

      {x:760, y:ground-332, w:520, h:24, townFloor:true, layer:3},
      {x:2020, y:ground-332, w:620, h:24, townFloor:true, layer:3},
      {x:3180, y:ground-332, w:610, h:24, townFloor:true, layer:3}
    ];
  }
  function townLaddersV20(){
    var ground=N(game&&game.ground,560);
    return [
      {x:500, y1:ground-104, y2:ground},
      {x:1240, y1:ground-218, y2:ground-104},
      {x:1740, y1:ground-104, y2:ground},
      {x:2520, y1:ground-332, y2:ground-218},
      {x:2920, y1:ground-218, y2:ground-104},
      {x:3540, y1:ground-332, y2:ground-218},
      {x:4160, y1:ground-104, y2:ground}
    ];
  }
  function applyTownLayoutV20(){
    if(!game || game.mode!=='town') return;
    game.width=Math.max(game.width||0,4700);
    game.ground=N(game.ground,560);
    game.platforms=townPlatformsV20();
    game.ladders=townLaddersV20();
  }
  function floorAtRole(role, i){
    var ground=N(game&&game.ground,560);
    var table={
      portal: ground,
      merchant: ground-104,
      weapon: ground-104,
      blacksmith: ground-104,
      quest: ground-218,
      warrior: ground-218,
      mage: ground-332,
      rogue: ground-218,
    ...
    return table[role] || (i%3===0?ground-104:i%3===1?ground-218:ground-332);
  }
  function roleOfNpc(npc){
    var s=String((npc&&npc.name)||'')+' '+String((npc&&npc.type)||'')+' '+String((npc&&npc.role)||'');
    if(/택시|차원 포탈|포탈/.test(s) || (npc&&npc.type==='taxi')) return 'portal';
    if(/장비/.test(s) || (npc&&npc.type==='weapon')) return 'weapon';
    if(/대장장이|브론|blacksmith/.test(s)) return 'blacksmith';
    if(/상인|merchant/.test(s) || (npc&&npc.type==='merchant')) return 'merchant';
    if(/성검|십자가|룬석|오브|균열|유물|비석/.test(s) || (npc&&npc.type==='job_object')) return 'object';
    if(/용혈|용기사|드래곤|아르딘/.test(s)) return 'dragon';
    if(/마법|현자|이렌|벨라|아스텔|mage|sage/.test(s)) return 'mage';
    if(/궁수|명궁|세리아|리아|archer|ranger/.test(s)) return 'archer';
  ...
  function placeTownNpcsV20(){
    ...
    var used={};
    A(game.npcs).forEach(function(npc,idx){
      var role=roleOfNpc(npc);
      npc.__v20Role=role;
      if(role==='portal'){
        npc.type='taxi';                 // keep old taxi/taxi-cost behavior
        npc.name='차원 이동 포탈';
        npc.text='원을 지불하고 다른 도시로 이동합니다.';
      }
      var list=roleSlots[role]||roleSlots.villager;
      var u=used[role]||0; used[role]=u+1;
      var x=list[u%list.length] + Math.floor(u/list.length)*155 + ((H((game.townId||'')+role+u)%39)-19);
      npc.x=C(x,80,Math.max(900,(game.width||4700)-120));
      npc.y=floorAtRole(role,u);
      npc.vx=0; npc.vy=0; npc.grounded=true;
    });
    var p=game.player;
    if(p && p.y > game.ground) { p.y=game.ground; p.vy=0; p.grounded=true; }
  }

  var oldLoadTownV20=typeof loadTown==='function'?loadTown:null;
  if(oldLoadTownV20 && !oldLoadTownV20.__v20Wrapped){
    var loadTownWrap=function loadTownV20(){
      var r=oldLoadTownV20.apply(this,arguments);
      try{ applyTownLayoutV20(); placeTownNpcsV20(); setTimeout(function(){applyTownLayoutV20(); placeTownNpcsV20();},120); }catch(e){ console.warn('[V20 town layout]',e); }
      return r;
    };
    loadTownWrap.__v20Wrapped=true;
    loadTown=window.loadTown=loadTownWrap;
  }

  var oldUpdatePlayerV20=typeof updatePlayer==='function'?updatePlayer:null;
  if(oldUpdatePlayerV20 && !oldUpdatePlayerV20.__v20Wrapped){
    var updatePlayerWrap=function updatePlayerV20(dt){
      if(game && game.mode==='town') applyTownLayoutV20();
      oldUpdatePlayerV20.apply(this,arguments);
      var p=game&&game.player;
      if(p && (keys.has(' ')||keys.has('arrowup')) && p.__v19JumpBoosted && p.vy>-780){
        p.vy=-760;      // V20: tall enough for layered town jump spots.
        p.__v19JumpBoosted=false;
      }
    };
    updatePlayerWrap.__v20Wrapped=true;
    updatePlayer=window.updatePlayer=updatePlayerWrap;
  }

  /* ---------- Town Visuals: buildings, platforms, ladders, portal ---------- */
  function drawLadderV20(l){
    var top=Math.min(l.y1,l.y2), bottom=Math.max(l.y1,l.y2);
    ctx.save();
    ctx.strokeStyle='#6b3f19'; ctx.lineWidth=7;
    line(ctx,l.x-16,top,l.x-16,bottom,'#6b3f19',7);
    line(ctx,l.x+16,top,l.x+16,bottom,'#6b3f19',7);
    for(var y=top+16;y<bottom;y+=24) line(ctx,l.x-18,y,l.x+18,y,'#b7793b',4);
    ctx.restore();
  }
  function drawTreehouse(x,y,kind,label){
    ctx.save();
    ctx.translate(x,y);
    ctx.fillStyle='rgba(0,0,0,.18)'; ctx.beginPath(); ctx.ellipse(0,10,90,16,0,0,Math.PI*2); ctx.fill();

    if(kind==='blacksmith'){
      fillRR(ctx,-80,-95,160,85,10,'#7c2d12');
      fillRR(ctx,-62,-118,124,32,8,'#4b5563');
      fillRR(ctx,44,-142,28,50,5,'#374151');
      ctx.fillStyle='#fb923c'; ctx.beginPath(); ctx.arc(58,-150,13,0,Math.PI*2); ctx.fill();
      fillRR(ctx,-42,-58,84,48,6,'#1f2937');
      ctx.fillStyle='#facc15'; ctx.fillRect(-35,-50,70,9);
    } else if(kind==='weapon'){
      fillRR(ctx,-80,-88,160,78,10,'#334155');
      fillRR(ctx,-70,-115,140,32,7,'#94a3b8');
      fillRR(ctx,-44,-54,88,44,5,'#111827');
      poly(ctx,[[-18,-75],[0,-95],[18,-75],[0,-65]],'#f8fafc');
      fillRR(ctx,-55,-24,110,10,3,'#64748b');
    } else if(kind==='merchant'){
      fillRR(ctx,-74,-84,148,74,10,'#92400e');
      fillRR(ctx,-82,-116,164,34,8,'#f97316');
      fillRR(ctx,-44,-49,88,39,5,'#3f1f0f');
      fillRR(ctx,42,-70,28,26,5,'#facc15');
    } else if(kind==='mage'){
      fillRR(ctx,-58,-128,116,118,18,'#312e81');
      poly(ctx,[[-72,-128],[0,-190],[72,-128]],'#4f46e5');
      circle(ctx,0,-200,18,'#93c5fd');
      fillRR(ctx,-32,-67,64,57,8,'#111827');
    } else if(kind==='archer'){
      fillRR(ctx,-90,-70,180,60,18,'#14532d');
      poly(ctx,[[-95,-70],[0,-126],[95,-70]],'#16a34a');
      fillRR(ctx,-35,-45,70,35,6,'#3f2917');
      line(ctx,64,-70,84,-108,'#fef3c7',4);
    } else if(kind==='rogue'){
      fillRR(ctx,-72,-88,144,78,12,'#111827');
      fillRR(ctx,-78,-116,156,34,7,'#312e81');
      fillRR(ctx,-38,-55,76,45,5,'#0f172a');
      circle(ctx,0,-93,10,'#a78bfa');
    } else if(kind==='portal'){
      ctx.shadowColor='#60a5fa'; ctx.shadowBlur=24;
      ctx.strokeStyle='#60a5fa'; ctx.lineWidth=12; ctx.beginPath(); ctx.ellipse(0,-58,44,62,0,0,Math.PI*2); ctx.stroke();
      ctx.strokeStyle='#c084fc'; ctx.lineWidth=7; ctx.beginPath(); ctx.ellipse(0,-58,27,44,0,0,Math.PI*2); ctx.stroke();
      ctx.shadowBlur=0;
      ctx.fillStyle='rgba(96,165,250,.28)'; ctx.beginPath(); ctx.ellipse(0,-58,27,44,0,0,Math.PI*2); ctx.fill();
      fillRR(ctx,-58,-10,116,15,7,'#334155');
    } else {
      fillRR(ctx,-68,-75,136,65,10,'#8b5e34');
      fillRR(ctx,-76,-102,152,30,8,'#84cc16');
      fillRR(ctx,-31,-45,62,35,5,'#3f2917');
    }

    ctx.fillStyle='rgba(15,23,42,.85)';
    fillRR(ctx,-54,-140,108,24,9,'rgba(15,23,42,.85)');
    ctx.fillStyle='#fff'; ctx.font='bold 12px sans-serif'; ctx.textAlign='center';
    if(label) ctx.fillText(label,0,-123);
    ctx.restore();
  }
  function drawDecorV20(town){
    /* background forest / sky-island mood based on user reference */
    for(var x=60; x<game.width; x+=330){
      ctx.fillStyle='rgba(20,83,45,.26)';
      ctx.beginPath(); ctx.ellipse(x,game.ground-35,120,36,0,0,Math.PI*2); ctx.fill();
    }
    drawTreehouse(245, game.ground, 'portal', '포탈');
    drawTreehouse(1010, game.ground-104, 'merchant', '소비 상점');
    drawTreehouse(1880, game.ground-104, 'weapon', '무기 상점');
    drawTreehouse(2200, game.ground-104, 'blacksmith', '대장간');
    drawTreehouse(930, game.ground-332, 'mage', '마법 전당');
    drawTreehouse(3340, game.ground-332, 'archer', '궁수 훈련장');
    drawTreehouse(2760, game.ground-218, 'rogue', '그림자 길드');
    drawTreehouse(1360, game.ground-218, 'villager', '기사단 초소');
    drawTreehouse(3600, game.ground-332, 'villager', '용기사 제단');
  }
  var oldDrawTownObjectsV20=typeof drawTownObjects==='function'?drawTownObjects:null;
  drawTownObjects=window.drawTownObjects=function drawTownObjectsV20(town){
    if(game && game.mode==='town'){
      applyTownLayoutV20();
      drawDecorV20(town||getTown(game.townId));
      return;
    }
    if(oldDrawTownObjectsV20) return oldDrawTownObjectsV20.apply(this,arguments);
  };
  var oldDrawWorldV20=typeof drawWorld==='function'?drawWorld:null;
  if(oldDrawWorldV20 && !oldDrawWorldV20.__v20Wrapped){
    var drawWorldWrap=function drawWorldV20(){
      if(game && game.mode==='town') applyTownLayoutV20();
      oldDrawWorldV20.apply(this,arguments);
      if(game && game.mode==='town' && Array.isArray(game.ladders)) game.ladders.forEach(drawLadderV20);
    };
    drawWorldWrap.__v20Wrapped=true;
    drawWorld=window.drawWorld=drawWorldWrap;
  }

  /* ---------- Maple-like body and NPCs ---------- */
  function outfitColors(player){
    var armorId=idOf(typeof equipment!=='undefined' && equipment && equipment.armor);
    var body='#4f93f5', dark='#2f6fbd', pants='#273449';
    if(armorId && ITEMS && ITEMS[armorId]){
      var it=ITEMS[armorId], pix=it.pixel||{};
      body=pix.a||body; dark=pix.b||dark;
      if(kind(it)==='armor' && /robe|로브|magic/.test(String(it.id+' '+it.name).toLowerCase())) pants='#4c1d95';
    }
    return {body:body,dark:dark,pants:pants};
  }
  drawPlayerBody=window.drawPlayerBody=function drawPlayerBodyV20(c,player){
    player=player||game.player;
    var ch=player.character||game.player.character||{};
    var walk=Math.sin((player.animTime||0)*13);
    var moving=player.anim==='walk';
    var leg=moving?walk*4:0;
    var arm=moving?-walk*3:0;
    var eq=outfitColors(player);
    var weaponRef=(typeof equipment!=='undefined' && equipment && equipment.weapon) || null;
    var it=itemOf(weaponRef);
    var wk=kind(it);
    var t=Math.sin(Math.min(1,(player.animTime||0)*13)*Math.PI);
    var attacking=player.anim==='attack';

    c.save();
    c.lineCap='round'; c.lineJoin='round';
    c.fillStyle='rgba(0,0,0,.26)'; c.beginPath(); c.ellipse(0,3,24,6,0,0,Math.PI*2); c.fill();

    /* Short, connected chibi legs/body */
    fillRR(c,-17+leg,-30,13,24,6,'#151515');
    fillRR(c,4-leg,-30,13,24,6,'#151515');
    fillRR(c,-14+leg,-28,9,20,5,eq.pants);
    fillRR(c,5-leg,-28,9,20,5,eq.pants);
    fillRR(c,-22+leg,-10,21,7,2,'#171717');
    fillRR(c,1-leg,-10,21,7,2,'#171717');

    fillRR(c,-22,-66,44,43,12,'#151515');
    fillRR(c,-18,-64,36,39,11,eq.body);
    fillRR(c,-16,-36,32,10,5,eq.dark);
    c.fillStyle='#f8f9ff'; c.fillRect(-11,-58,22,5);

    var lx=-22+arm, ly=-48, rx=23-arm, ry=-48;
    if(attacking){
      if(wk==='staff'){ rx=30; ry=-60; lx=-18; ly=-54; }
      else if(wk==='bow'){ rx=30; ry=-52; lx=-31-t*7; ly=-53; }
      else if(wk==='dagger'){ rx=28+t*24; ry=-47; lx=-18; ly=-50; }
      else if(wk==='spear'){ rx=31+t*10; ry=-54; lx=-16; ly=-50; }
      else if(wk==='gun'){ rx=31; ry=-52; lx=-17; ly=-49; }
      else { rx=27+t*28; ry=-58+t*8; lx=-18; ly=-49; }
    }
    if(player.anim==='jump'){ lx=-23; rx=23; ly=-54; ry=-54; }

    line(c,-15,-57,lx,ly,'#151515',9); line(c,15,-57,rx,ry,'#151515',9);
    line(c,-15,-57,lx,ly,eq.dark,6); line(c,15,-57,rx,ry,eq.dark,6);
    circ(c,lx,ly,5,'#151515'); circ(c,rx,ry,5,'#151515');
    circ(c,lx,ly,3.4,ch.skin||'#ffd6a6'); circ(c,rx,ry,3.4,ch.skin||'#ffd6a6');

    /* weapon before head for natural layering */
    drawWeapon(c, weaponRef, rx, ry, attacking, wk, player.animTime||0);

    /* Head attached to body: no long exposed neck */
    fillRR(c,-7,-73,14,12,4,'#151515');
    fillRR(c,-4,-72,8,11,3,ch.skin||'#ffd6a6');
    circ(c,-20,-89,5,'#151515'); circ(c,20,-89,5,'#151515');
    c.fillStyle='#151515'; c.beginPath(); c.ellipse(0,-91,23,24,0,0,Math.PI*2); c.fill();
    circ(c,-19,-89,3.7,ch.skin||'#ffd6a6'); circ(c,19,-89,3.7,ch.skin||'#ffd6a6');
    c.fillStyle=ch.skin||'#ffd6a6'; c.beginPath(); c.ellipse(0,-91,19,21,0,0,Math.PI*2); c.fill();

    if(typeof drawHair==='function') drawHair(c,ch,-91);
    if(typeof drawFace==='function') drawFace(c,ch,-91);
    var helmetId=idOf(typeof equipment!=='undefined' && equipment && equipment.helmet);
    if(typeof drawEquippedHelmet==='function') drawEquippedHelmet(c,helmetId,-91);

    c.restore();
  };

  function drawPortalNpc(npc){
    ctx.save(); ctx.translate(npc.x,npc.y); ctx.imageSmoothingEnabled=false;
    ctx.fillStyle='rgba(0,0,0,.26)'; ctx.beginPath(); ctx.ellipse(0,4,42,9,0,0,Math.PI*2); ctx.fill();
    var pulse=1+Math.sin(performance.now()/260)*.08;
    ctx.scale(pulse,pulse);
    ctx.shadowColor='#60a5fa'; ctx.shadowBlur=26;
    ctx.strokeStyle='#60a5fa'; ctx.lineWidth=10; ctx.beginPath(); ctx.ellipse(0,-54,34,50,0,0,Math.PI*2); ctx.stroke();
    ctx.strokeStyle='#c084fc'; ctx.lineWidth=5; ctx.beginPath(); ctx.ellipse(0,-54,21,34,0,0,Math.PI*2); ctx.stroke();
    ctx.fillStyle='rgba(96,165,250,.22)'; ctx.beginPath(); ctx.ellipse(0,-54,22,36,0,0,Math.PI*2); ctx.fill();
    ctx.shadowBlur=0;
    ctx.fillStyle='#fff'; ctx.font='bold 14px sans-serif'; ctx.textAlign='center'; ctx.fillText('차원 이동 포탈',0,-116);
    if(game.player&&Math.abs(game.player.x-npc.x)<100&&Math.abs(game.player.y-npc.y)<140){ ctx.fillStyle='#ffe066'; ctx.fillText('E 이동',0,-136); }
    ctx.restore();
  }
  var oldDrawNPCBodyV20=typeof drawNPCBody==='function'?drawNPCBody:null;
  drawNPCBody=window.drawNPCBody=function drawNPCBodyV20(npc){
    if(!npc) return;
    var role=roleOfNpc(npc);
    if(role==='portal') return drawPortalNpc(npc);
    if(oldDrawNPCBodyV20) return oldDrawNPCBodyV20.apply(this,arguments);
  };
  var oldDrawNPCsV20=typeof drawNPCs==='function'?drawNPCs:null;
  drawNPCs=window.drawNPCs=function drawNPCsV20(){
    try{ placeTownNpcsV20(); }catch(_){}
    oldDrawNPCsV20.apply(this,arguments);
  };

  /* ---------- Weapon motion fix: no double flip; staff casts forward ---------- */
  function drawTrail(c,k,g,sw,len){
    if(!sw) return;
    c.save(); c.globalAlpha=.30+g.score*.10; c.strokeStyle=g.color; c.lineWidth=4+g.score*1.5; c.lineCap='round';
    if(k==='staff'){ c.beginPath(); c.arc(72,-34,18+g.score*8,0,Math.PI*2); c.stroke(); if(g.score>=2){ c.beginPath(); c.arc(72,-34,36+g.score*8,0,Math.PI*2); c.stroke(); } }
    else if(k==='bow'||k==='gun'||k==='spear'){ c.beginPath(); c.moveTo(24,-18); c.lineTo(105+g.score*18,-18); c.stroke(); }
    else if(k==='dagger'){ for(var i=0;i<2+g.score;i++) line(c,8+i*8,-22-i*8,52+i*6,-28-i*8,g.color,3); }
    else { c.beginPath(); c.arc(0,-len*.45,50+g.score*8,-.95,1.10); c.stroke(); if(g.score>=3){ c.beginPath(); c.arc(0,-len*.45,70+g.score*7,-.85,1.25); c.stroke(); } }
    c.restore();
  }
  drawWeapon=window.drawWeapon=function drawWeaponV20(c,weaponRef,handX,handY,attacking,attackKind,animTime){
    var it=itemOf(weaponRef || (typeof equipment!=='undefined' && equipment && equipment.weapon));
    if(!it) return;
    var k=kind(it), p=pal(it), g=gradeInfo(it), seed=H((it.id||'')+(it.name||'')), req=N(it.reqLevel||it.level||1,1);
    var t=N(animTime,0), sw=attacking?Math.sin(Math.min(1,t*12)*Math.PI):0;
    var len=52+Math.min(34,req*.55)+g.score*7;

    c.save();
    /* local character space is already flipped by drawPlayer(), so do not scale by face again */
    c.translate(handX+12, handY-7);
    var baseRot=-.30, attackRot=baseRot;
    if(k==='staff'){ baseRot=-.12; attackRot=-.06; }
    else if(k==='bow'){ baseRot=-.10; attackRot=-.08; }
    else if(k==='dagger'){ baseRot=-.18; attackRot=-.08+sw*.70; }
    else if(k==='spear'){ baseRot=-.20; attackRot=-.18+sw*.25; }
    else if(k==='gun'){ baseRot=-.03; attackRot=-.02; }
    else if(k==='axe'||k==='hammer'){ baseRot=-.36; attackRot=-.95+sw*1.48; }
    else { attackRot=-.92+sw*1.48; }
    c.rotate(attacking?attackRot:baseRot);
    c.imageSmoothingEnabled=false;
    c.shadowColor=g.color; c.shadowBlur=g.glow;

    if(k==='staff'){
      /* staff is held upright-forward, and magic shoots to the character's facing side */
      line(c,0,24,0,-len,p[3],5);
      circ(c,0,-len-10,g.score>=3?14:10,p[0]);
      c.strokeStyle=g.color; c.lineWidth=2.2; c.beginPath(); c.arc(0,-len-10,20+g.score*4,0,Math.PI*2); c.stroke();
      if(attacking){
        var orbX=72+g.score*12, orbY=-34;
        c.save(); c.shadowColor=g.color; c.shadowBlur=24+g.score*6;
        circ(c,orbX,orbY,10+g.score*2,g.color);
        c.globalAlpha=.55; circ(c,orbX-18,orbY+6,5,p[1]); c.globalAlpha=1;
        c.restore();
        drawTrail(c,k,g,sw,len);
      }
    } else if(k==='bow'){
      c.strokeStyle=p[3]; c.lineWidth=5; c.beginPath(); c.arc(4,-17,36,-1.35,1.35); c.stroke();
      line(c,17,-52,17,18,p[1],1.5); fillRR(c,3,-22,8,14,2,p[2]); line(c,-15,-17,44,-17,p[0],3);
      if(attacking){ line(c,12,-17,100+g.score*18,-17,'#f8fafc',2.3); poly(c,[[106+g.score*18,-17],[88+g.score*18,-25],[88+g.score*18,-9]],g.color); drawTrail(c,k,g,sw,len); }
    } else if(k==='dagger'){
      c.rotate(.40); poly(c,[[0,-len*.70],[10,-7],[3,14],[-5,14],[-10,-7]],p[0]); poly(c,[[0,-len*.58],[4,-7],[0,4],[-4,-7]],p[1]); fillRR(c,-13,9,26,7,2,p[3]); fillRR(c,-4,15,8,20,2,p[2]); drawTrail(c,k,g,sw,len);
    } else if(k==='axe'){
      line(c,0,24,0,-len,p[3],5); poly(c,[[0,-len+2],[28,-len+14],[19,-len+44],[0,-len+30]],p[0]); if(seed%3!==0) poly(c,[[0,-len+2],[-26,-len+15],[-16,-len+41],[0,-len+30]],p[1]); drawTrail(c,k,g,sw,len);
    } else if(k==='hammer'){
      line(c,0,22,0,-len+18,p[3],5.5); if(seed%2) poly(c,[[0,-len-8],[22,-len+4],[10,-len+24],[-10,-len+24],[-22,-len+4]],p[0]); else fillRR(c,-24,-len,48,20,3,p[0]); drawTrail(c,k,g,sw,len);
    } else if(k==='spear'){
      line(c,0,25,0,-len-16,p[3],4.5); poly(c,[[0,-len-40],[16,-len-13],[0,-len-4],[-16,-len-13]],p[0]); fillRR(c,-13,-len-6,26,6,2,p[2]); if(attacking) drawTrail(c,k,g,sw,len);
    } else if(k==='gun'){
      fillRR(c,-7,-30,58,17,3,p[0]); fillRR(c,38,-26,28,8,2,p[1]); fillRR(c,5,-14,12,26,2,p[3]); if(attacking){ circ(c,72,-22,5,g.color); drawTrail(c,k,g,sw,len); }
    } else {
      poly(c,[[0,-len],[13,-21],[8,19],[-8,19],[-13,-21]],p[0]); line(c,-2,-len+8,2,11,p[1],1.7); fillRR(c,-19,8,38,8,2,p[3]); fillRR(c,-5,14,10,23,2,p[2]); circ(c,0,40,4,p[3]);
      if(g.score>=3||seed%5===0){ c.globalAlpha=.95; poly(c,[[0,-len-10],[13,-len+8],[4,-len+30],[17,-len+45],[-2,-len+41],[4,-len+18]],g.color); c.globalAlpha=1; }
      drawTrail(c,k,g,sw,len);
    }
    c.restore();
  ...
  /* ---------- Enhancement effects ---------- */
  function enhanceBurst(x,y,mode){
    if(!game || !game.particles) return;
    var success=mode==='success';
    var colors=success?['#facc15','#86efac','#ffffff','#60a5fa']:['#ef4444','#111827','#f97316','#fca5a5'];
    for(var i=0;i<(success?70:90);i++){
      var a=Math.random()*Math.PI*2, sp=(success?90:130)+Math.random()*(success?230:310);
      game.particles.push({x:x,y:y,vx:Math.cos(a)*sp,vy:Math.sin(a)*sp-80,life:.45+Math.random()*.75,color:colors[i%colors.length]});
    }
    if(typeof makeText==='function') makeText(success?'✦ 강화 성공 ✦':'장비 파괴!',x,y-90,success?'#facc15':'#ff6b6b');
  }
  enhanceEquippedItem=window.enhanceEquippedItem=function enhanceEquippedItemV20(slot){
    var ref=equipment[slot], item=(typeof getItemData==='function')?getItemData(ref):itemOf(ref);
    if(!item) return;
    var plus=(typeof itemEnhance==='function')?itemEnhance(ref):N(ref&&ref.enhance,0);
    if(plus>=10){ makeText('이미 최대 강화입니다.',game.player.x,game.player.y-90,'#ffe066'); return; }
    var cost=(typeof getEnhanceCost==='function')?getEnhanceCost(ref):100;
    if(wallet.gold<cost){ makeText('강화 비용 '+cost+'G 필요',game.player.x,game.player.y-90,'#ff8787'); return; }
    wallet.gold-=cost;
    if(Math.random()<((typeof getEnhanceRate==='function')?getEnhanceRate(plus):.7)){
      equipment[slot]={id:item.id,enhance:plus+1};
      if(typeof recalcStats==='function') recalcStats();
      if(typeof markAutoSaveSoon==='function') markAutoSaveSoon();
      enhanceBurst(game.player.x,game.player.y-55,'success');
      if(typeof circleEffect==='function') circleEffect(game.player.x,game.player.y-55,'#ffd43b');
      return;
    }
    if(plus>=5 && Math.random()<((typeof getDestroyRate==='function')?getDestroyRate(plus):.15)){
      equipment[slot]=null;
      if(typeof recalcStats==='function') recalcStats();
      if(typeof markAutoSaveSoon==='function') markAutoSaveSoon();
      enhanceBurst(game.player.x,game.player.y-55,'destroy');
      return;
    }
    if(typeof markAutoSaveSoon==='function') markAutoSaveSoon();
    makeText('강화 실패',game.player.x,game.player.y-90,'#cbd5e1');
    enhanceBurst(game.player.x,game.player.y-55,'fail');
  };

  /* ---------- Monster style pass ---------- */
  var oldDrawMonsterShapeV20=typeof drawMonsterShape==='function'?drawMonsterShape:null;
  drawMonsterShape=window.drawMonsterShape=function drawMonsterShapeV20(type){
    type=type||{}; var color=type.color||'#86efac', shape=type.shape||type.family||'slime';
    ctx.save(); ctx.imageSmoothingEnabled=false;
    if(/slime/.test(shape)){
      ctx.fillStyle='rgba(0,0,0,.20)'; ctx.beginPath(); ctx.ellipse(0,-12,26,8,0,0,Math.PI*2); ctx.fill();
      ctx.fillStyle='#111827'; ctx.beginPath(); ctx.ellipse(0,-22,28,19,0,0,Math.PI*2); ctx.fill();
      ctx.fillStyle=color; ctx.beginPath(); ctx.ellipse(0,-24,25,17,0,0,Math.PI*2); ctx.fill();
      circ(ctx,-8,-27,3,'#111827'); circ(ctx,8,-27,3,'#111827'); ctx.fillStyle='rgba(255,255,255,.45)'; ctx.beginPath(); ctx.ellipse(-8,-34,7,3,0,0,Math.PI*2); ctx.fill();
      ctx.restore(); return;
    }
    if(/mushroom/.test(shape)){
      ctx.fillStyle=color; ctx.beginPath(); ctx.ellipse(0,-43,30,19,0,Math.PI,0); ctx.fill();
      ctx.fillStyle='#fff1cc'; fillRR(ctx,-18,-42,36,27,8,'#fff1cc');
      fillRR(ctx,-8,-20,16,20,5,'#7a4d27');
      circ(ctx,-9,-33,3,'#111827'); circ(ctx,8,-33,3,'#111827');
      circ(ctx,-12,-48,4,'#fff8'); circ(ctx,12,-52,5,'#fff8');
      ctx.restore(); return;
    }
    if(/boar|wolf|rat|lizard/.test(shape)){
      fillRR(ctx,-38,-48,76,35,16,color);
      circ(ctx,29,-36,15,color); circ(ctx,-24,-48,8,color);
      circ(ctx,36,-39,3,'#111827'); line(ctx,42,-32,55,-36,'#fef3c7',4);
      fillRR(ctx,-26,-16,12,16,4,'#5b3a1d'); fillRR(ctx,16,-16,12,16,4,'#5b3a1d');
      ctx.restore(); return;
    }
    if(/spirit|wisp|orb|mana/.test(shape)){
      ctx.shadowColor=color; ctx.shadowBlur=16; circ(ctx,0,-40,24,color); ctx.shadowBlur=0;
      poly(ctx,[[-18,-26],[-7,-4],[0,-18],[8,-4],[18,-26]],color);
      circ(ctx,-7,-45,3,'#111827'); circ(ctx,8,-45,3,'#111827');
      ctx.restore(); return;
    }
    if(/golem|ore|armor|ruin/.test(shape)){
      fillRR(ctx,-31,-72,62,55,12,'#334155'); fillRR(ctx,-24,-66,48,43,10,color);
      fillRR(ctx,-43,-52,18,33,7,color); fillRR(ctx,25,-52,18,33,7,color);
      circ(ctx,-9,-50,4,'#111827'); circ(ctx,9,-50,4,'#111827'); fillRR(ctx,-14,-33,28,5,2,'#111827');
      ctx.restore(); return;
    }
    if(oldDrawMonsterShapeV20) { ctx.restore(); return oldDrawMonsterShapeV20.apply(this,arguments); }
    ctx.restore();
  };

  var oldHudV20=typeof drawHUD==='function'?drawHUD:null;
  if(oldHudV20 && !oldHudV20.__v20Wrapped){
    var hudWrap=function drawHUDV20(){
      try{ if(game&&game.mode==='town'){ applyTownLayoutV20(); placeTownNpcsV20(); } }catch(_){}
      oldHudV20.apply(this,arguments);
      try{
        ctx.save(); ctx.textAlign='right'; ctx.font='bold 16px sans-serif'; ctx.fillStyle='#86efac'; ctx.strokeStyle='rgba(15,23,42,.95)'; ctx.lineWidth=5;
        ctx.strokeText('V20 층형 도시/포탈/캐릭터 개선 적용됨',W-22,464); ctx.fillText('V20 층형 도시/포탈/캐릭터 개선 적용됨',W-22,464);
        ctx.fillStyle='#fcd34d'; ctx.strokeText('사다리 도시 · 포탈 이동 · 지팡이 방향/이펙트 보정',W-22,486); ctx.fillText('사다리 도시 · 포탈 이동 · 지팡이 방향/이펙트 보정',W-22,486);
        ctx.restore();
      }catch(_){}
    };
    hudWrap.__v20Wrapped=true; drawHUD=window.drawHUD=hudWrap;
  }
  setInterval(function(){ try{ if(game&&game.mode==='town'){ applyTownLayoutV20(); placeTownNpcsV20(); } }catch(_){} }, 500);
  try{ if(game&&game.mode==='town'){ applyTownLayoutV20(); placeTownNpcsV20(); } }catch(_){}
  console.log('[PixelRPG] V20 maple-like town polish installed');
})();


/* =========================================================
   V21 FINAL VISUAL STACK FIX
   - Completely replaces NPC drawing, no old nested drawNPCs call => no doubled labels.
   - Re-applies layered town platforms after old V18/V19/V20 wrappers clear them.
   - Higher town jump power.
   - Stronger Maple-like connected player/NPC body.
   - Equipment grade borders and attack effects are more obvious.
========================================================= */
(function(){
  'use strict';
  if (window.__PIXEL_RPG_V21_FINAL_VISUAL_FIX__) return;
  window.__PIXEL_RPG_V21_FINAL_VISUAL_FIX__ = true;
  window.PIXEL_RPG_PATCH_VERSION = 'V21_FINAL_VISUAL_FIX';
  ...
  function gradeInfo(it){
    ...
    var s=String([it.rarity,it.grade,it.tier,it.name,it.id].join(' ')).toLowerCase();
    var req=N(it.reqLevel||it.level||it.lv,1);
    if(/legend|전설|레전|myth|신화/.test(s)||req>=55) return {score:4,key:'legendary',color:'#facc15',glow:28,label:'전설'};
    if(/epic|에픽|영웅/.test(s)||req>=38) return {score:3,key:'epic',color:'#a855f7',glow:20,label:'에픽'};
    if(/ultra|unique|유니크|초희귀/.test(s)||req>=25) return {score:2,key:'unique',color:'#22d3ee',glow:14,label:'유니크'};
    if(/rare|희귀/.test(s)||req>=12) return {score:1,key:'rare',color:'#3b82f6',glow:9,label:'희귀'};
    return {score:0,key:'common',color:'#9ca3af',glow:0,label:'일반'};
  }
  function kind(it){
    it=itemOf(it)||{};
    var s=String([it.weaponType,it.equipSlot,it.slot,it.type,it.id,it.name,it.icon].join(' ')).toLowerCase();
  ...
  function palette(it){
    var g=gradeInfo(it), v=HSH((it&&it.id)||it&&it.name||'x')%9;
    var p=[
      ['#e5e7eb','#ffffff','#1d4ed8','#f59e0b'],
      ['#84cc16','#d9f99d','#7c3aed','#92400e'],
      ['#60a5fa','#dbeafe','#1e3a8a','#f59e0b'],
  ...
  /* Layered town must win over older V18 cleanup every frame. */
  function townPlatformsV21(){
    var ground=N(game&&game.ground,560);
    return [
      {x:90,y:ground-96,w:620,h:24,townFloor:true,layer:1},
      {x:820,y:ground-96,w:620,h:24,townFloor:true,layer:1},
      {x:1660,y:ground-96,w:760,h:24,townFloor:true,layer:1},
      {x:2700,y:ground-96,w:770,h:24,townFloor:true,layer:1},
      {x:3800,y:ground-96,w:650,h:24,townFloor:true,layer:1},

      {x:340,y:ground-206,w:560,h:24,townFloor:true,layer:2},
      {x:1160,y:ground-206,w:690,h:24,townFloor:true,layer:2},
      {x:2280,y:ground-206,w:740,h:24,townFloor:true,layer:2},
      {x:3380,y:ground-206,w:610,h:24,townFloor:true,layer:2},

      {x:700,y:ground-316,w:560,h:24,townFloor:true,layer:3},
      {x:1970,y:ground-316,w:680,h:24,townFloor:true,layer:3},
      {x:3150,y:ground-316,w:660,h:24,townFloor:true,layer:3}
    ];
  }
  function townLaddersV21(){
    var ground=N(game&&game.ground,560);
    return [
      {x:500,y1:ground-96,y2:ground},
      {x:1220,y1:ground-206,y2:ground-96},
      {x:1740,y1:ground-96,y2:ground},
      {x:2500,y1:ground-316,y2:ground-206},
      {x:2940,y1:ground-206,y2:ground-96},
      {x:3540,y1:ground-316,y2:ground-206},
      {x:4140,y1:ground-96,y2:ground}
    ];
  }
  function applyTownV21(){
    if(!game || game.mode!=='town') return;
    game.width=Math.max(game.width||0,4700);
    game.ground=N(game.ground,560);
    game.platforms=townPlatformsV21();
    game.ladders=townLaddersV21();
  }
  function roleOf(npc){
    var s=String((npc&&npc.name)||'')+' '+String((npc&&npc.type)||'')+' '+String((npc&&npc.role)||'');
    if(/택시|차원 이동|포탈/.test(s) || (npc&&npc.type==='taxi')) return 'portal';
    if(/장비/.test(s) || (npc&&npc.type==='weapon')) return 'weapon';
    if(/대장장이|브론|blacksmith/.test(s)) return 'blacksmith';
    if(/상인|merchant/.test(s) || (npc&&npc.type==='merchant')) return 'merchant';
    if(/성검|십자가|룬석|오브|균열|유물|비석/.test(s) || (npc&&npc.type==='job_object')) return 'object';
  ...
  function floorForRole(role,i){
    var g=N(game&&game.ground,560);
    return ({portal:g, merchant:g-96, weapon:g-96, blacksmith:g-96, quest:g-206, warrior:g-206, rogue:g-206, object:g-206, mage:g-316, archer:g-316, dragon:g-316, villager:g-96}[role] || g-96);
  }
  function placeNPCsV21(){
    if(!game || game.mode!=='town') return;
    applyTownV21();
    var slots={
      portal:[240],
    ...
    game.npcs.forEach(function(npc){
      ...
      var i=count[role]||0; count[role]=i+1;
      var xs=slots[role]||slots.villager;
      npc.x=C(xs[i%xs.length]+Math.floor(i/xs.length)*170+((HSH((game.townId||'')+role+i)%31)-15),80,Math.max(900,(game.width||4700)-120));
      npc.y=floorForRole(role,i);
      npc.vx=0; npc.vy=0; npc.grounded=true;
    });
  }

  var oldLoadTownV21=typeof loadTown==='function'?loadTown:null;
  if(oldLoadTownV21 && !oldLoadTownV21.__v21Wrapped){
    var loadWrap=function loadTownV21(){
      var r=oldLoadTownV21.apply(this,arguments);
      try{ applyTownV21(); placeNPCsV21(); setTimeout(function(){applyTownV21(); placeNPCsV21();},80); setTimeout(function(){applyTownV21(); placeNPCsV21();},500); }catch(e){ console.warn('[V21 town]',e); }
      return r;
    };
    loadWrap.__v21Wrapped=true;
    loadTown=window.loadTown=loadWrap;
  }

  /* Jump: do not rely on old weak jump; force clear high jump at first takeoff. */
  var oldUpdatePlayerV21=typeof updatePlayer==='function'?updatePlayer:null;
  if(oldUpdatePlayerV21 && !oldUpdatePlayerV21.__v21Wrapped){
    var updateWrap=function updatePlayerV21(dt){
      var p=game&&game.player;
      var jump=keys&&(keys.has(' ')||keys.has('arrowup'));
      var was=!!(p&&p.grounded);
      if(game&&game.mode==='town') applyTownV21();
      oldUpdatePlayerV21.apply(this,arguments);
      if(game&&game.mode==='town') applyTownV21();
      if(p && jump && was && p.vy<0){
        p.vy=-840;
        p.grounded=false;
      }
    };
    updateWrap.__v21Wrapped=true;
    updatePlayer=window.updatePlayer=updateWrap;
  }

  /* Draw world: older drawWorld may have cleared platforms, so draw layered town after it as visible overlay. */
  function drawLadder(l){
    var top=Math.min(l.y1,l.y2), bot=Math.max(l.y1,l.y2);
    line(ctx,l.x-15,top,l.x-15,bot,'#6b3f19',7);
    line(ctx,l.x+15,top,l.x+15,bot,'#6b3f19',7);
    for(var y=top+16;y<bot;y+=24) line(ctx,l.x-18,y,l.x+18,y,'#b7793b',4);
  }
  function drawPlat(p){
    fillRR(ctx,p.x,p.y,p.w,p.h,8,'#8bcf5a');
    ctx.fillStyle='#5b3b1f'; ctx.fillRect(p.x+10,p.y+18,p.w-20,9);
    ctx.fillStyle='rgba(255,255,255,.18)'; ctx.fillRect(p.x+24,p.y+4,Math.max(40,p.w*.22),4);
  }
  function drawBuilding(x,y,type,label){
    ctx.save(); ctx.translate(x,y);
    if(type==='portal'){
      ctx.shadowColor='#60a5fa'; ctx.shadowBlur=26;
      ctx.strokeStyle='#60a5fa'; ctx.lineWidth=11; ctx.beginPath(); ctx.ellipse(0,-55,42,58,0,0,Math.PI*2); ctx.stroke();
      ctx.strokeStyle='#c084fc'; ctx.lineWidth=6; ctx.beginPath(); ctx.ellipse(0,-55,25,37,0,0,Math.PI*2); ctx.stroke();
      ctx.fillStyle='rgba(96,165,250,.24)'; ctx.beginPath(); ctx.ellipse(0,-55,25,37,0,0,Math.PI*2); ctx.fill();
      ctx.shadowBlur=0;
    } else {
      var main={merchant:'#92400e',weapon:'#334155',blacksmith:'#7c2d12',mage:'#312e81',archer:'#14532d',rogue:'#111827',warrior:'#8b5e34',dragon:'#134e4a'}[type]||'#8b5e34';
      var roof={merchant:'#f97316',weapon:'#94a3b8',blacksmith:'#4b5563',mage:'#4f46e5',archer:'#16a34a',rogue:'#312e81',warrior:'#84cc16',dragon:'#0f766e'}[type]||'#84cc16';
      fillRR(ctx,-70,-82,140,72,10,main);
      if(type==='mage') poly(ctx,[[-82,-82],[0,-142],[82,-82]],roof);
      else if(type==='archer') poly(ctx,[[-86,-82],[0,-130],[86,-82]],roof);
      else fillRR(ctx,-78,-110,156,32,8,roof);
      fillRR(ctx,-34,-48,68,38,6,'rgba(15,23,42,.85)');
      if(type==='blacksmith'){ fillRR(ctx,45,-135,25,50,5,'#374151'); circ(ctx,56,-144,12,'#fb923c'); }
      if(type==='weapon'){ poly(ctx,[[-14,-70],[0,-91],[14,-70],[0,-60]],'#f8fafc'); }
      if(type==='mage'){ circ(ctx,0,-152,15,'#93c5fd'); }
      if(type==='dragon'){ poly(ctx,[[-45,-116],[-15,-158],[15,-116]],'#5eead4'); }
    }
    fillRR(ctx,-54,-138,108,24,9,'rgba(15,23,42,.86)');
    ctx.fillStyle='#fff'; ctx.font='bold 12px sans-serif'; ctx.textAlign='center'; ctx.fillText(label,0,-121);
    ctx.restore();
  }
  var oldDrawWorldV21=typeof drawWorld==='function'?drawWorld:null;
  if(oldDrawWorldV21 && !oldDrawWorldV21.__v21Wrapped){
    var worldWrap=function drawWorldV21(){
      if(game&&game.mode==='town') applyTownV21();
      oldDrawWorldV21.apply(this,arguments);
      if(game&&game.mode==='town'){
        applyTownV21();
        drawBuilding(240,game.ground,'portal','포탈');
        drawBuilding(1010,game.ground-96,'merchant','소비 상점');
        drawBuilding(1885,game.ground-96,'weapon','무기 상점');
        drawBuilding(2215,game.ground-96,'blacksmith','대장간');
        drawBuilding(930,game.ground-316,'mage','마법 전당');
        drawBuilding(3350,game.ground-316,'archer','궁수 훈련장');
        drawBuilding(2770,game.ground-206,'rogue','그림자 길드');
        drawBuilding(1370,game.ground-206,'warrior','기사단 초소');
        drawBuilding(3610,game.ground-316,'dragon','용기사 제단');
        A(game.platforms).forEach(drawPlat);
        A(game.ladders).forEach(drawLadder);
      }
    };
    worldWrap.__v21Wrapped=true;
    drawWorld=window.drawWorld=worldWrap;
  }

  /* Stronger connected body. This is final drawPlayerBody and is visibly different. */
  function outfit(){
    var armor=itemOf(typeof equipment!=='undefined'&&equipment&&equipment.armor), g=gradeInfo(armor);
    var body='#4f93f5', dark='#2f6fbd', pants='#233044';
    if(armor){ var k=kind(armor); if(k==='armor'){ body=palette(armor)[0]; dark=palette(armor)[2]; pants=gradeInfo(armor).score>=2?'#111827':'#233044'; } }
    return {body:body,dark:dark,pants:pants,glow:g.color,score:g.score};
  }
  drawPlayerBody=window.drawPlayerBody=function drawPlayerBodyV21(c,player){
    player=player||game.player; var ch=player.character||game.player.character||{};
    var moving=player.anim==='walk', walk=Math.sin((player.animTime||0)*13), leg=moving?walk*4:0, arm=moving?-walk*3:0;
    var eq=outfit(), weaponRef=(typeof equipment!=='undefined'&&equipment&&equipment.weapon)||null, wk=kind(itemOf(weaponRef)), attacking=player.anim==='attack';
    var t=Math.sin(Math.min(1,(player.animTime||0)*13)*Math.PI);
    c.save(); c.lineCap='round'; c.lineJoin='round';
    c.fillStyle='rgba(0,0,0,.28)'; c.beginPath(); c.ellipse(0,3,25,6,0,0,Math.PI*2); c.fill();

    fillRR(c,-18+leg,-29,13,24,6,'#151515'); fillRR(c,5-leg,-29,13,24,6,'#151515');
    fillRR(c,-15+leg,-27,9,20,5,eq.pants); fillRR(c,6-leg,-27,9,20,5,eq.pants);
    fillRR(c,-23+leg,-9,22,7,2,'#171717'); fillRR(c,2-leg,-9,22,7,2,'#171717');

    fillRR(c,-24,-70,48,50,14,'#151515');
    fillRR(c,-20,-68,40,45,13,eq.body);
    fillRR(c,-17,-39,34,11,5,eq.dark);
    c.fillStyle='#f8f9ff'; c.fillRect(-11,-62,22,6);
    if(eq.score>=2){ c.globalAlpha=.8; line(c,-15,-48,15,-48,eq.glow,2); c.globalAlpha=1; }

    var lx=-24+arm, ly=-51, rx=26-arm, ry=-51;
    if(attacking){
      if(wk==='staff'){ rx=33; ry=-63; lx=-20; ly=-55; }
      else if(wk==='bow'){ rx=33; ry=-54; lx=-35-t*8; ly=-54; }
      else if(wk==='dagger'){ rx=31+t*25; ry=-48; lx=-18; ly=-51; }
      else if(wk==='spear'){ rx=34+t*10; ry=-56; lx=-17; ly=-52; }
      else if(wk==='gun'){ rx=34; ry=-54; lx=-18; ly=-51; }
      else { rx=31+t*29; ry=-61+t*8; lx=-19; ly=-51; }
    }
    if(player.anim==='jump'){ lx=-25; ly=-57; rx=25; ry=-57; }

    line(c,-17,-61,lx,ly,'#151515',9); line(c,17,-61,rx,ry,'#151515',9);
    line(c,-17,-61,lx,ly,eq.dark,6); line(c,17,-61,rx,ry,eq.dark,6);
    circ(c,lx,ly,5,'#151515'); circ(c,rx,ry,5,'#151515');
    circ(c,lx,ly,3.4,ch.skin||'#ffd6a6'); circ(c,rx,ry,3.4,ch.skin||'#ffd6a6');

    drawWeapon(c,weaponRef,rx,ry,attacking,wk,player.animTime||0);

    fillRR(c,-8,-76,16,11,4,'#151515');
    fillRR(c,-5,-75,10,10,3,ch.skin||'#ffd6a6');
    circ(c,-21,-93,5,'#151515'); circ(c,21,-93,5,'#151515');
    c.fillStyle='#151515'; c.beginPath(); c.ellipse(0,-95,24,25,0,0,Math.PI*2); c.fill();
    circ(c,-20,-93,3.8,ch.skin||'#ffd6a6'); circ(c,20,-93,3.8,ch.skin||'#ffd6a6');
    c.fillStyle=ch.skin||'#ffd6a6'; c.beginPath(); c.ellipse(0,-95,20,22,0,0,Math.PI*2); c.fill();

    /* visible hair even if old drawHair is subtle */
    var hair=ch.hair||'#2b160e';
    fillRR(c,-18,-119,36,15,8,hair);
    c.beginPath(); c.fillStyle=hair; c.arc(0,-108,22,Math.PI,0); c.fill();
    if(typeof drawHair==='function') { try{ drawHair(c,ch,-95); }catch(_){} }
    c.fillStyle='#111827'; c.fillRect(-8,-101,4,5); c.fillRect(5,-101,4,5); c.fillRect(-3,-91,7,2);
    if(typeof drawEquippedHelmet==='function') { try{ drawEquippedHelmet(c,idOf(equipment&&equipment.helmet),-95); }catch(_){} }
    c.restore();
  };
  ...
  function drawPortal(npc){
    ...
    ctx.fillStyle='rgba(0,0,0,.26)'; ctx.beginPath(); ctx.ellipse(0,4,42,9,0,0,Math.PI*2); ctx.fill();
    var pulse=1+Math.sin(performance.now()/260)*.08; ctx.scale(pulse,pulse);
    ctx.shadowColor='#60a5fa'; ctx.shadowBlur=26; ctx.strokeStyle='#60a5fa'; ctx.lineWidth=10; ctx.beginPath(); ctx.ellipse(0,-54,34,50,0,0,Math.PI*2); ctx.stroke();
    ctx.strokeStyle='#c084fc'; ctx.lineWidth=5; ctx.beginPath(); ctx.ellipse(0,-54,21,34,0,0,Math.PI*2); ctx.stroke();
    ctx.fillStyle='rgba(96,165,250,.22)'; ctx.beginPath(); ctx.ellipse(0,-54,22,36,0,0,Math.PI*2); ctx.fill();
    ctx.shadowBlur=0; ctx.restore();
  }
  function drawLabelOnce(text,x,y,near,action){
    var bw=Math.max(76,Math.min(170,ctx.measureText(text).width+28));
    fillRR(ctx,x-bw/2,y-17,bw,24,9,'rgba(15,23,42,.92)');
    ctx.fillStyle='#fff'; ctx.font='bold 14px sans-serif'; ctx.textAlign='center'; ctx.fillText(text,x,y);
    if(near){ ctx.fillStyle='#ffe066'; ctx.font='bold 13px sans-serif'; ctx.fillText(action||'E 대화',x,y-24); }
  }

  /* Complete final NPC draw. Never call older drawNPCs, so doubled text disappears. */
  drawNPCs=window.drawNPCs=function drawNPCsV21(){
    if(!game) return;
    placeNPCsV21();
    A(game.npcs).forEach(function(npc){
      if(!npc) return;
      var role=roleOf(npc), near=game.player&&Math.abs(game.player.x-npc.x)<95&&Math.abs(game.player.y-npc.y)<125;
      if(role==='portal'){
        drawPortal(npc);
        ctx.font='bold 14px sans-serif';
        drawLabelOnce('차원 이동 포탈',npc.x,npc.y-112,near,'E 이동');
      } else {
        drawNPCBody(npc);
        ctx.font='bold 14px sans-serif';
        drawLabelOnce(String(npc.name||'NPC'),npc.x,npc.y-112,near,role==='object'?'E 조사':'E 대화');
      }
    });
  };

  /* Equipment icon and attack effect 강화 */
  function iconFrame(s,it){
    var g=gradeInfo(it);
    fillRR(ctx,-s*.50,-s*.50,s,s,7,'#171321');
    ctx.shadowColor=g.color; ctx.shadowBlur=g.glow;
    strokeRR(ctx,-s*.50,-s*.50,s,s,7,g.color,Math.max(1.6,s*.06));
    ctx.shadowBlur=0;
    if(g.score>=2) strokeRR(ctx,-s*.39,-s*.39,s*.78,s*.78,5,'rgba(255,255,255,.52)',1.1);
    if(g.score>=3){ for(var i=0;i<4;i++) circ(ctx,(i<2?-1:1)*s*.40,(i%2?-1:1)*s*.40,s*.045,g.color); }
    if(g.score>=4){ ctx.strokeStyle='rgba(255,255,255,.90)'; ctx.lineWidth=1.4; ctx.beginPath(); ctx.arc(0,0,s*.55,0,Math.PI*2); ctx.stroke(); }
  }
  drawItemIcon=window.drawItemIcon=function drawItemIconV21(itemOrIcon,x,y,size){
    var it=itemOf(itemOrIcon)||(typeof itemOrIcon==='object'?itemOrIcon:{id:String(itemOrIcon||''),icon:String(itemOrIcon||'')});
    var k=kind(it), s=size||32, p=palette(it), g=gradeInfo(it), v=HSH((it.id||it.name||k))%16;
    ctx.save(); ctx.translate(x,y); ctx.imageSmoothingEnabled=false; iconFrame(s,it); ctx.shadowColor=g.color; ctx.shadowBlur=g.score>=2?g.glow*.7:0;
    if(k==='hp'){ circ(ctx,-s*.12,-s*.09,s*.15,'#ff4d4f'); circ(ctx,s*.12,-s*.09,s*.15,'#ff4d4f'); poly(ctx,[[-s*.28,-s*.02],[0,s*.30],[s*.28,-s*.02]],'#ff4d4f'); circ(ctx,-s*.08,-s*.12,s*.04,'rgba(255,255,255,.9)'); }
    else if(k==='mp'){ ctx.fillStyle='#4dabf7'; ctx.beginPath(); ctx.moveTo(0,-s*.34); ctx.quadraticCurveTo(s*.30,0,0,s*.34); ctx.quadraticCurveTo(-s*.30,0,0,-s*.34); ctx.fill(); circ(ctx,-s*.07,-s*.08,s*.045,'rgba(255,255,255,.85)'); }
    else if(k==='staff'){ ctx.rotate(-.55); line(ctx,0,s*.32,0,-s*.29,p[3],s*.09); circ(ctx,0,-s*.38,s*(g.score>=3?.16:.12),p[0]); ctx.strokeStyle=g.color; ctx.lineWidth=s*.05; ctx.beginPath(); ctx.arc(0,-s*.38,s*(g.score>=3?.25:.19),0,Math.PI*2); ctx.stroke(); if(g.score>=4)circ(ctx,0,-s*.38,s*.055,'#fff'); }
    else if(k==='sword'){ ctx.rotate(-.74+(v%3-1)*.08); poly(ctx,[[0,-s*.43],[s*.11,-s*.06],[s*.08,s*.30],[-s*.08,s*.30],[-s*.11,-s*.06]],p[0]); line(ctx,-s*.02,-s*.35,s*.02,s*.20,p[1],Math.max(1,s*.04)); fillRR(ctx,-s*.22,s*.16,s*.44,s*.08,2,p[3]); fillRR(ctx,-s*.06,s*.23,s*.12,s*.20,2,p[2]); if(g.score>=3) poly(ctx,[[0,-s*.52],[s*.12,-s*.29],[s*.04,-s*.10],[s*.16,.02],[-s*.06,.04]],g.color); }
    else if(k==='bow'){ ctx.strokeStyle=p[3]; ctx.lineWidth=s*.095; ctx.beginPath(); ctx.arc(0,0,s*.31,-1.25,1.25); ctx.stroke(); line(ctx,s*.08,-s*.29,s*.08,s*.29,p[1],s*.035); fillRR(ctx,-s*.045,-s*.075,s*.09,s*.15,1,p[2]); if(g.score>=2) line(ctx,-s*.26,0,s*.31,0,'#e5e7eb',s*.04); }
    else if(k==='dagger'){ ctx.rotate(-.62+(v%4)*.06); poly(ctx,[[0,-s*.34],[s*.12,-s*.07],[s*.04,s*.19],[-s*.05,s*.19],[-s*.12,-s*.07]],p[0]); fillRR(ctx,-s*.16,s*.08,s*.32,s*.06,2,p[3]); fillRR(ctx,-s*.05,s*.14,s*.10,s*.20,2,p[2]); line(ctx,-s*.10,-s*.24,s*.10,s*.04,g.color,1.4+g.score*.3); }
    else if(k==='axe'){ ctx.rotate(-.50); line(ctx,0,s*.32,0,-s*.28,p[3],s*.09); poly(ctx,[[0,-s*.30],[s*.25,-s*.21],[s*.16,-s*.01],[0,-s*.05]],p[0]); if(v%3!==0) poly(ctx,[[0,-s*.28],[-s*.22,-s*.18],[-s*.12,-s*.02],[0,-s*.06]],p[1]); if(g.score>=2)circ(ctx,s*.10,-s*.17,s*.045,g.color); }
    else if(k==='hammer'){ ctx.rotate(-.48); line(ctx,0,s*.31,0,-s*.14,p[3],s*.09); if(v%2) poly(ctx,[[0,-s*.36],[s*.21,-s*.25],[s*.09,-s*.08],[-s*.09,-s*.08],[-s*.21,-s*.25]],p[0]); else fillRR(ctx,-s*.21,-s*.30,s*.42,s*.18,2,p[0]); if(g.score>=2){ circ(ctx,-s*.12,-s*.24,s*.04,p[1]); circ(ctx,s*.12,-s*.24,s*.04,p[1]); } }
    else if(k==='spear'){ ctx.rotate(-.60); line(ctx,0,s*.34,0,-s*.29,p[3],s*.07); poly(ctx,[[0,-s*.45],[s*.14,-s*.23],[0,-s*.16],[-s*.14,-s*.23]],p[0]); fillRR(ctx,-s*.12,-s*.18,s*.24,s*.05,2,p[2]); }
    else { var vv=v%6; if(k==='armor'){ poly(ctx,[[0,-s*.33],[s*.29,-s*.14],[s*.24,s*.32],[-s*.24,s*.32],[-s*.29,-s*.14]],p[0]); fillRR(ctx,-s*.15,-s*.11,s*.30,s*.10,2,p[1]); fillRR(ctx,-s*.19,s*.08,s*.38,s*.08,2,p[2]); } else if(k==='helmet'){ poly(ctx,[[-s*.27,-s*.02],[-s*.12,-s*.25],[s*.12,-s*.25],[s*.27,-s*.02],[s*.20,s*.17],[-s*.20,s*.17]],p[0]); fillRR(ctx,-s*.15,-s*.03,s*.30,s*.08,2,p[2]); } else if(k==='accessory'){ ctx.strokeStyle=p[3]; ctx.lineWidth=s*.10; ctx.beginPath(); ctx.arc(0,s*.04,s*.18,0,Math.PI*2); ctx.stroke(); circ(ctx,0,-s*.16,s*.08,g.color); } else if(vv===0){ poly(ctx,[[0,-s*.31],[s*.25,0],[0,s*.31],[-s*.25,0]],p[0]); circ(ctx,-s*.06,-s*.08,s*.05,p[1]); } else if(vv===1){ circ(ctx,0,0,s*.23,p[0]); circ(ctx,s*.08,-s*.09,s*.06,p[1]); } else if(vv===2){ poly(ctx,[[-s*.25,s*.18],[-s*.12,-s*.23],[s*.25,-s*.06],[s*.10,s*.25]],p[0]); } else { fillRR(ctx,-s*.20,-s*.18,s*.40,s*.36,5,p[0]); line(ctx,-s*.12,0,s*.12,0,p[1],2); } }
    ctx.restore();
  };

  function trail(c,k,g,sw,len){
    if(!sw) return; c.save(); c.globalAlpha=.34+g.score*.12; c.strokeStyle=g.color; c.lineWidth=5+g.score*1.8; c.lineCap='round';
    if(k==='staff'){ c.beginPath(); c.arc(82,-34,20+g.score*8,0,Math.PI*2); c.stroke(); if(g.score>=2){ c.beginPath(); c.arc(82,-34,40+g.score*9,0,Math.PI*2); c.stroke(); } }
    else if(k==='bow'||k==='gun'||k==='spear'){ c.beginPath(); c.moveTo(24,-18); c.lineTo(120+g.score*20,-18); c.stroke(); }
    else if(k==='dagger'){ for(var i=0;i<2+g.score;i++) line(c,8+i*8,-22-i*8,58+i*8,-29-i*8,g.color,3.2); }
    else { c.beginPath(); c.arc(0,-len*.45,55+g.score*10,-.98,1.16); c.stroke(); if(g.score>=3){ c.beginPath(); c.arc(0,-len*.45,78+g.score*8,-.85,1.35); c.stroke(); } }
    c.restore();
  }
  drawWeapon=window.drawWeapon=function drawWeaponV21(c,weaponRef,handX,handY,attacking,attackKind,animTime){
    var it=itemOf(weaponRef || (typeof equipment!=='undefined'&&equipment&&equipment.weapon)); if(!it) return;
    var k=kind(it), p=palette(it), g=gradeInfo(it), seed=HSH((it.id||'')+(it.name||'')), req=N(it.reqLevel||it.level||1,1);
    var t=N(animTime,0), sw=attacking?Math.sin(Math.min(1,t*12)*Math.PI):0, len=54+Math.min(38,req*.6)+g.score*8;
    c.save(); c.translate(handX+12,handY-6); var rot=-.28;
    if(k==='staff') rot=attacking?-.06:-.10; else if(k==='bow') rot=-.08; else if(k==='dagger') rot=attacking?(-.08+sw*.70):-.18; else if(k==='spear') rot=attacking?(-.18+sw*.25):-.20; else if(k==='gun') rot=-.03; else rot=attacking?(-.92+sw*1.50):-.34;
    c.rotate(rot); c.shadowColor=g.color; c.shadowBlur=g.glow; c.imageSmoothingEnabled=false;
    if(k==='staff'){ line(c,0,25,0,-len,p[3],5.2); circ(c,0,-len-10,g.score>=3?15:11,p[0]); c.strokeStyle=g.color; c.lineWidth=2.4; c.beginPath(); c.arc(0,-len-10,22+g.score*4,0,Math.PI*2); c.stroke(); if(attacking){ var ox=82+g.score*12,oy=-34; circ(c,ox,oy,11+g.score*2,g.color); circ(c,ox-17,oy+5,5,p[1]); trail(c,k,g,sw,len); } }
    else if(k==='bow'){ c.strokeStyle=p[3]; c.lineWidth=5; c.beginPath(); c.arc(4,-17,36,-1.35,1.35); c.stroke(); line(c,17,-52,17,18,p[1],1.5); fillRR(c,3,-22,8,14,2,p[2]); line(c,-15,-17,44,-17,p[0],3); if(attacking){ line(c,12,-17,104+g.score*20,-17,'#f8fafc',2.3); poly(c,[[110+g.score*20,-17],[92+g.score*20,-25],[92+g.score*20,-9]],g.color); trail(c,k,g,sw,len); } }
    else if(k==='dagger'){ c.rotate(.40); poly(c,[[0,-len*.70],[10,-7],[3,14],[-5,14],[-10,-7]],p[0]); poly(c,[[0,-len*.58],[4,-7],[0,4],[-4,-7]],p[1]); fillRR(c,-13,9,26,7,2,p[3]); fillRR(c,-4,15,8,20,2,p[2]); trail(c,k,g,sw,len); }
    else if(k==='axe'){ line(c,0,24,0,-len,p[3],5); poly(c,[[0,-len+2],[28,-len+14],[19,-len+44],[0,-len+30]],p[0]); if(seed%3!==0) poly(c,[[0,-len+2],[-26,-len+15],[-16,-len+41],[0,-len+30]],p[1]); trail(c,k,g,sw,len); }
    else if(k==='hammer'){ line(c,0,22,0,-len+18,p[3],5.5); if(seed%2) poly(c,[[0,-len-8],[22,-len+4],[10,-len+24],[-10,-len+24],[-22,-len+4]],p[0]); else fillRR(c,-24,-len,48,20,3,p[0]); trail(c,k,g,sw,len); }
    else if(k==='spear'){ line(c,0,25,0,-len-16,p[3],4.5); poly(c,[[0,-len-40],[16,-len-13],[0,-len-4],[-16,-len-13]],p[0]); fillRR(c,-13,-len-6,26,6,2,p[2]); if(attacking)trail(c,k,g,sw,len); }
    else { poly(c,[[0,-len],[13,-21],[8,19],[-8,19],[-13,-21]],p[0]); line(c,-2,-len+8,2,11,p[1],1.8); fillRR(c,-20,8,40,8,2,p[3]); fillRR(c,-5,14,10,23,2,p[2]); circ(c,0,40,4,p[3]); if(g.score>=3||seed%5===0){ poly(c,[[0,-len-10],[13,-len+8],[4,-len+30],[17,-len+45],[-2,-len+41],[4,-len+18]],g.color); } trail(c,k,g,sw,len); }
    c.restore();
  };

  var oldHudV21=typeof drawHUD==='function'?drawHUD:null;
  if(oldHudV21 && !oldHudV21.__v21Wrapped){
    var hudWrap=function drawHUDV21(){
      if(game&&game.mode==='town'){ applyTownV21(); placeNPCsV21(); }
      oldHudV21.apply(this,arguments);
      if(game&&game.mode==='town'){ applyTownV21(); placeNPCsV21(); }
      try{ ctx.save(); ctx.textAlign='right'; ctx.font='bold 16px sans-serif'; ctx.fillStyle='#86efac'; ctx.strokeStyle='rgba(15,23,42,.95)'; ctx.lineWidth=5; ctx.strokeText('V21 최종 시각 안정화 적용됨',W-22,508); ctx.fillText('V21 최종 시각 안정화 적용됨',W-22,508); ctx.fillStyle='#fcd34d'; ctx.strokeText('글씨 더블링 제거 · 층형 도시 강제 · 점프/캐릭터/장비 강화',W-22,530); ctx.fillText('글씨 더블링 제거 · 층형 도시 강제 · 점프/캐릭터/장비 강화',W-22,530); ctx.restore(); }catch(_){}
    };
    hudWrap.__v21Wrapped=true; drawHUD=window.drawHUD=hudWrap;
  }

  setInterval(function(){ try{ if(game&&game.mode==='town'){ applyTownV21(); placeNPCsV21(); } }catch(_){} }, 120);
  try{ if(game&&game.mode==='town'){ applyTownV21(); placeNPCsV21(); } }catch(_){}
  console.log('[PixelRPG] V21 final visual stack fix installed');
})();


/* =========================================================
   V22 SPRITE / INVENTORY / NAMING PATCH
   - 캐릭터/NPC를 업로드 레퍼런스에 맞춘 연결형 치비 스타일로 재정의
   - 장비창에서 캐릭터 제거, 악세사리 슬롯 노출
   - 인벤토리 50칸 제한 + 스크롤 + 클릭 좌표 보정
   - 장비 이름 재정비 (LV 표기 제거, 상위 등급 고유명)
========================================================= */
(function(){
  'use strict';
  if (window.__PIXEL_RPG_V22_SPRITE_INVENTORY_PATCH__) return;
  window.__PIXEL_RPG_V22_SPRITE_INVENTORY_PATCH__ = true;
  window.PIXEL_RPG_PATCH_VERSION = 'V22_SPRITE_INVENTORY_PATCH';

  inventory.maxSlots = 50;
  inventory.scroll = Number.isFinite(inventory.scroll) ? inventory.scroll : 0;
  inventory.tab = inventory.tab || 'equip';
  dragState.justDroppedAt = 0;

  function clampV22(v,a,b){ return Math.max(a, Math.min(b, v)); }
  function itemDataV22(ref){ return ITEMS[itemRefId(ref)] || null; }
  function itemKindV22(item){
    item = item || {};
    const id = String(item.id || '');
    if (item.weaponType) return item.weaponType;
    if (item.equipSlot) return item.equipSlot;
    if (item.type) return item.type;
    return id.split('_')[0] || 'etc';
  }
  function itemCategoryV22(item){
    if (!item) return 'etc';
    if (item.type === 'consumable') return 'use';
    if (item.type === 'etc') return 'etc';
    return 'equip';
  }
  function invMetricsV22(){
    return {
      x: 50 + 395 + 18,
      y: 96 + 62 + 88,
      cols: 6,
  ...
  function getFilteredInventoryEntriesV22(){
    const tab = inventory.tab || 'equip';
    const out = [];
    inventory.items.forEach(function(it, index){
      const data = itemDataV22(it);
      const cat = itemCategoryV22(data);
      if (tab === 'all' || tab === cat) out.push({ item: it, index: index, data: data });
    });
    return out;
  }
  function getVisibleInventoryEntriesV22(){
    const m = invMetricsV22();
    const list = getFilteredInventoryEntriesV22();
    const maxRows = Math.max(0, Math.ceil(list.length / m.cols) - m.visibleRows);
    inventory.scroll = clampV22(inventory.scroll || 0, 0, maxRows);
    const start = (inventory.scroll || 0) * m.cols;
    return { list: list, entries: list.slice(start, start + m.cols * m.visibleRows), start: start, maxRows: maxRows, metrics: m };
  }
  function getInventoryTabAtV22(x, y){
    const baseX = 50 + 395 + 14;
    const baseY = 96 + 62 + 42;
    const defs = [
      { key: 'equip', label: '장비', x: baseX },
      { key: 'use', label: '소비', x: baseX + 74 },
      { key: 'etc', label: '기타', x: baseX + 148 }
    ];
    for (const tab of defs) {
      if (hit(x, y, tab.x, baseY, 68, 32)) return tab.key;
    }
    return null;
  }

  const oldAddItemV22 = addItem;
  addItem = window.addItem = function addItemV22(ref, count){
    const id = itemRefId(ref);
    const enhance = itemEnhance(ref);
    const data = ITEMS[id];
    if (!id || !data) return;
    ...
    if (inventory.items.length >= (inventory.maxSlots || 50)) {
      makeText('인벤토리가 가득 찼습니다.', game.player.x, game.player.y - 92, '#ff8787');
      return;
    }
    inventory.items.push(stack(id, count, enhance));
  };

  getInventorySlotAt = window.getInventorySlotAt = function getInventorySlotAtV22(x, y){
    const info = getVisibleInventoryEntriesV22();
    const m = info.metrics;
    for (let i = 0; i < m.cols * m.visibleRows; i++) {
      const cx = m.x + (i % m.cols) * m.cell;
      const cy = m.y + Math.floor(i / m.cols) * m.cell;
      if (hit(x, y, cx, cy, m.box, m.box)) {
  ...
  const oldBeginDragV22 = beginInventoryDrag;
  beginInventoryDrag = window.beginInventoryDrag = function beginInventoryDragV22(x, y){
    const tab = getInventoryTabAtV22(x, y);
    if (tab) return;
    return oldBeginDragV22(x, y);
  };

  const oldFinishDragV22 = finishInventoryDrag;
  finishInventoryDrag = window.finishInventoryDrag = function finishInventoryDragV22(x, y){
    oldFinishDragV22(x, y);
    dragState.justDroppedAt = performance.now();
  };

  handleInventoryClick = window.handleInventoryClick = function handleInventoryClickV22(x, y){
    if (dragState.active) return;
    if (performance.now() - (dragState.justDroppedAt || 0) < 150) return;
    const tab = getInventoryTabAtV22(x, y);
    if (tab) {
      inventory.tab = tab;
      inventory.scroll = 0;
    ...
    const idx = getInventorySlotAt(x, y);
    if (idx >= 0 && inventory.items[idx]) useItem(inventory.items[idx]);
  };

  function handleInventoryWheelV22(e){
    if (!game.ready || !inventory.open) return;
    const pos = getMouse(e);
    if (!hit(pos.x, pos.y, 50, 96, 850, 560)) return;
    const info = getVisibleInventoryEntriesV22();
    if (info.maxRows <= 0) return;
    e.preventDefault();
    const dir = e.deltaY > 0 ? 1 : -1;
    inventory.scroll = clampV22((inventory.scroll || 0) + dir, 0, info.maxRows);
  }
  canvas.addEventListener('wheel', handleInventoryWheelV22, { passive: false });
  window.addEventListener('wheel', handleInventoryWheelV22, { passive: false });

  function drawPanelLabelV22(text, x, y){
    ctx.fillStyle = '#5579a6';
    ctx.font = 'bold 12px sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText(text, x, y);
  }

  drawEquipmentBox = window.drawEquipmentBox = function drawEquipmentBoxV22(x, y){
    const w = 340;
    const h = 455;
    ctx.fillStyle = '#f8fbff';
    ctx.fillRect(x, y, w, h);
    ...
    const slots = [
      { name: '헬멧', key: 'helmet', x: 134, y: 48 },
      { name: '무기', key: 'weapon', x: 28, y: 170 },
      { name: '갑옷', key: 'armor', x: 134, y: 170 },
      { name: '무릎', key: 'knee', x: 240, y: 170 },
      { name: '악세사리', key: 'accessory', x: 134, y: 292 }
    ];

    ctx.strokeStyle = '#bfd7ea';
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(x + 170, y + 130);
    ctx.lineTo(x + 64, y + 208);
    ctx.moveTo(x + 170, y + 130);
    ctx.lineTo(x + 170, y + 208);
    ctx.moveTo(x + 170, y + 130);
    ctx.lineTo(x + 276, y + 208);
    ctx.moveTo(x + 170, y + 244);
    ctx.lineTo(x + 170, y + 292);
    ctx.stroke();
    ...
    slots.forEach(function(slot){
      ...
      ctx.strokeStyle = '#9bb7d6';
      ctx.strokeRect(sx, sy, 72, 72);
      drawPanelLabelV22(slot.name, sx + 36, sy - 8);
      const ref = equipment[slot.key];
      const data = itemDataV22(ref);
      if (data) {
        drawItemIcon(data, sx + 36, sy + 36, 42);
        ctx.fillStyle = '#233';
        ctx.font = '11px sans-serif';
        ctx.textAlign = 'center';
        const nm = getItemDisplayName(ref);
        ctx.fillText(nm.length > 10 ? nm.slice(0, 10) + '…' : nm, sx + 36, sy + 88);
      } else {
        ctx.fillStyle = '#8aa5c6';
        ctx.font = '11px sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText('비어 있음', sx + 36, sy + 42);
      }
    });

    ctx.fillStyle = '#eef6ff';
    ctx.fillRect(x + 16, y + 382, 308, 52);
    ctx.strokeStyle = '#b8cfe6';
    ctx.strokeRect(x + 16, y + 382, 308, 52);
    ctx.fillStyle = '#516d92';
    ctx.font = '12px sans-serif';
    ctx.textAlign = 'left';
    ctx.fillText('※ 장비는 드래그로 장착/해제합니다.', x + 26, y + 405);
    ctx.fillText('※ 캐릭터 미리보기는 제거하여 슬롯을 가리지 않게 했습니다.', x + 26, y + 425);
  };

  drawItemBox = window.drawItemBox = function drawItemBoxV22(x, y){
    const w = 410;
    const h = 455;
    ctx.fillStyle = '#fff';
    ctx.fillRect(x, y, w, h);
    ...
    ctx.font = 'bold 17px sans-serif';
    ctx.textAlign = 'left';
    ctx.fillText('ITEM INVENTORY ' + inventory.items.length + '/' + (inventory.maxSlots || 50), x + 14, y + 28);

    tabs.forEach(function(tab, i){
      const tx = x + 14 + i * 74;
      const active = (inventory.tab || 'equip') === tab.key;
      ctx.fillStyle = active ? '#ff7aa2' : '#d9e6f4';
      ctx.fillRect(tx, y + 42, 68, 32);
      ctx.strokeStyle = '#8eb2d5';
      ctx.strokeRect(tx, y + 42, 68, 32);
      ctx.fillStyle = '#1f2937';
      ctx.font = 'bold 14px sans-serif';
      ctx.textAlign = 'center';
      ctx.fillText(tab.label, tx + 34, y + 64);
    });

    ctx.save();
    ctx.beginPath();
    ctx.rect(m.x - 1, m.y - 1, m.cols * m.cell - 8, m.visibleRows * m.cell - 8);
    ctx.clip();
    ...
    for (let i = 0; i < m.cols * m.visibleRows; i++) {
      ...
      const cy = m.y + Math.floor(i / m.cols) * m.cell;
      ctx.fillStyle = '#eef6ff';
      ctx.fillRect(cx, cy, m.box, m.box);
      ctx.strokeStyle = '#cad9e8';
      ctx.strokeRect(cx, cy, m.box, m.box);
      const entry = info.entries[i];
      if (entry && entry.item) {
        drawItemIcon(entry.data || entry.item, cx + 24, cy + 24, 34);
        ctx.fillStyle = '#1f2937';
        ctx.font = 'bold 11px sans-serif';
        ctx.textAlign = 'right';
        ctx.fillText(String(entry.item.count), cx + 43, cy + 43);
      }
    }
    ctx.restore();
    ...
    const barX = x + 356;
    const barY = y + 88;
    const barH = m.visibleRows * m.cell - 8;
    ctx.fillStyle = '#e5eef8';
    ctx.fillRect(barX, barY, 14, barH);
    ctx.strokeStyle = '#b5c8dd';
    ctx.strokeRect(barX, barY, 14, barH);
    const totalRows = Math.max(1, Math.ceil(info.list.length / m.cols));
    const thumbH = Math.max(34, barH * (m.visibleRows / totalRows));
    const thumbY = barY + (info.maxRows ? (barH - thumbH) * ((inventory.scroll || 0) / info.maxRows) : 0);
    ctx.fillStyle = '#8fb3d7';
    ctx.fillRect(barX + 2, thumbY + 2, 10, thumbH - 4);
    ...
    ctx.font = '12px sans-serif';
    ctx.textAlign = 'right';
    ctx.fillText('휠 스크롤로 아래 아이템 확인', x + 392, y + 428);
  };

  function outlineRoundRectV22(c, x, y, w, h, r, fill, stroke){
    c.fillStyle = fill;
  ...
  function pxDotV22(c, x, y, w, h, col){ c.fillStyle = col; c.fillRect(x, y, w, h); }
  function drawMiniLimbV22(c, x1, y1, x2, y2, outer, inner, lw1, lw2){
    c.lineCap = 'round';
    c.strokeStyle = outer; c.lineWidth = lw1; c.beginPath(); c.moveTo(x1, y1); c.lineTo(x2, y2); c.stroke();
    c.strokeStyle = inner; c.lineWidth = lw2; c.beginPath(); c.moveTo(x1, y1); c.lineTo(x2, y2); c.stroke();
  }
  function drawChibiFaceV22(c, skin, hair, eye, mouthY){
    c.fillStyle = '#7a5a35';
    c.beginPath(); c.ellipse(0, -44, 20.5, 19.5, 0, 0, Math.PI * 2); c.fill();
    c.fillStyle = skin;
    c.beginPath(); c.ellipse(0, -45, 19, 18, 0, 0, Math.PI * 2); c.fill();
    c.fillStyle = hair;
    c.beginPath();
    c.moveTo(-18, -53);
    c.quadraticCurveTo(-14, -68, 0, -69);
    c.quadraticCurveTo(14, -68, 18, -53);
    c.lineTo(18, -44);
    c.quadraticCurveTo(8, -49, 0, -50);
    c.quadraticCurveTo(-8, -49, -18, -44);
    c.closePath();
    c.fill();
    c.fillStyle = eye || '#1f2937';
    c.fillRect(-8, -46, 3, 3);
    c.fillRect(5, -46, 3, 3);
    c.fillStyle = '#b86b4f';
    c.fillRect(-1, mouthY || -38, 2, 2);
  }
  function drawReferenceChibiBodyV22(c, look, pose){
    pose = pose || 'idle';
    const skin = look.skin || '#ffe7c4';
    const hair = look.hair || '#6b4a2d';
    const body = look.body || '#f59e0b';
    const sub = look.sub || '#fce7b2';
    const pants = look.pants || '#6b7280';
    const shoe = look.shoe || '#1f2937';
    const moving = pose === 'walk';
    const jump = pose === 'jump';
    const t = performance.now() / 1000;
    const walk = Math.sin(t * 9);
    const leg = moving ? walk * 3.5 : 0;
    const arm = moving ? -walk * 2.5 : 0;

    c.fillStyle = 'rgba(0,0,0,0.20)';
    c.beginPath(); c.ellipse(0, 2, 16, 4, 0, 0, Math.PI * 2); c.fill();

    drawMiniLimbV22(c, -7, -14, -10 + arm, -2, '#7a5a35', skin, 7, 4.5);
    drawMiniLimbV22(c, 7, -14, 10 - arm, -2, '#7a5a35', skin, 7, 4.5);

    c.fillStyle = '#7a5a35';
    roundRect(c, -11, -27, 22, 22, 8);
    c.fillStyle = body;
    roundRect(c, -9, -25, 18, 18, 6);
    c.fillStyle = sub;
    c.fillRect(-6, -21, 12, 4);
    c.fillStyle = body;
    c.fillRect(-4, -7, 8, 4);

    drawMiniLimbV22(c, -4, -7, -6 - leg, 10, '#7a5a35', pants, 7.5, 5);
    drawMiniLimbV22(c, 4, -7, 6 + leg, 10, '#7a5a35', pants, 7.5, 5);
    pxDotV22(c, -10 - leg, 9, 8, 4, shoe);
    pxDotV22(c, 2 + leg, 9, 8, 4, shoe);

    if (jump) {
      drawMiniLimbV22(c, -7, -14, -14, -10, '#7a5a35', skin, 7, 4.5);
      drawMiniLimbV22(c, 7, -14, 14, -10, '#7a5a35', skin, 7, 4.5);
    }

    drawChibiFaceV22(c, skin, hair, look.eye || '#1f2937', look.mouthY || -38);
    if (look.band) {
      c.fillStyle = look.band;
      c.fillRect(-18, -56, 36, 4);
    }
    if (look.hat) {
      c.fillStyle = look.hat;
      roundRect(c, -16, -61, 32, 9, 4);
    }
  }

  drawPlayerBody = window.drawPlayerBody = function drawPlayerBodyV22(c, player){
    player = player || game.player;
    const ch = player.character || game.player.character || {};
    const armor = itemDataV22(equipment.armor) || {};
    const look = {
      skin: ch.skin || '#ffe7c4',
      hair: ch.hair || '#7b5632',
      body: armor.pixel ? armor.pixel.a : '#f3a35d',
      sub: armor.pixel ? armor.pixel.b : '#ffd28a',
      pants: equipment.knee ? '#4b5563' : '#5d6b84',
      shoe: '#1f2937',
      hat: ch.hat || null,
      band: null
    };
    const pose = player.anim === 'attack' ? 'walk' : (player.anim === 'jump' ? 'jump' : (player.anim === 'walk' ? 'walk' : 'idle'));
    c.save();
    c.lineJoin = 'round';
    drawReferenceChibiBodyV22(c, look, pose);
    ...
    drawWeapon(c, itemRefId(equipment.weapon), handX, handY, player.anim === 'attack', player.attackKind || ((itemDataV22(equipment.weapon) || {}).weaponType) || 'sword', player.animTime || 0);
    if (equipment.helmet && typeof drawEquippedHelmet === 'function') {
      try { drawEquippedHelmet(c, itemRefId(equipment.helmet), -45); } catch (_) {}
    }
    c.restore();
  };

  function npcPaletteV22(npc){
    const role = (npc.__v21Role || npc.type || '').toString();
    const map = {
      merchant:  { skin:'#ffe0bc', hair:'#7c4a21', body:'#9cd86d', sub:'#eef8da', pants:'#596b53', hat:'#8b5e34', prop:'bag' },
      weapon:    { skin:'#f2d1ab', hair:'#50433a', body:'#9aa4b8', sub:'#eff2f7', pants:'#405166', hat:'#5a6477', prop:'blade' },
      blacksmith:{ skin:'#d8b08a', hair:'#3e3129', body:'#7b8798', sub:'#dce5ef', pants:'#434343', hat:'#566170', prop:'hammer' },
      quest:     { skin:'#ffe0bc', hair:'#f3f0dd', body:'#cdb5ff', sub:'#f3edff', pants:'#7b6bc9', hat:'#7c68d8', prop:'scroll' },
      warrior:   { skin:'#f2d3b0', hair:'#2c2b30', body:'#8b93ab', sub:'#e2e8f0', pants:'#495264', hat:'#5d6678', prop:'blade' },
      rogue:     { skin:'#e8c299', hair:'#26212b', body:'#8096a8', sub:'#dce6f1', pants:'#3c4352', hat:'#252d40', prop:'dagger' },
      mage:      { skin:'#f7d9bb', hair:'#7158a5', body:'#ab93f7', sub:'#efe9ff', pants:'#6253a4', hat:'#6c5ddb', prop:'staff' },
      archer:    { skin:'#f3d0a8', hair:'#8c6332', body:'#94c97d', sub:'#edf8e3', pants:'#53684a', hat:'#6a8b4d', prop:'bow' },
      dragon:    { skin:'#efc7a0', hair:'#7c2d12', body:'#6dcac3', sub:'#dbfbf7', pants:'#3f6e74', hat:'#2aa198', prop:'spear' },
      villager:  { skin:'#ffe0bc', hair:'#6f4d33', body:'#92b3ff', sub:'#eef3ff', pants:'#5e6c87', hat:null, prop:null },
      object:    { skin:'#ffe0bc', hair:'#6b4a2d', body:'#f59e0b', sub:'#fff3c4', pants:'#6b7280', hat:null, prop:'star' }
    };
    return map[role] || map.villager;
  }
  function drawNpcPropV22(c, prop){
    if (!prop) return;
    c.save();
    c.translate(14, -18);
    if (prop === 'staff') {
      c.rotate(-0.05);
      c.fillStyle = '#7c3aed'; c.fillRect(-1, -28, 3, 25);
      c.fillStyle = '#7dd3fc'; c.beginPath(); c.arc(0, -31, 5, 0, Math.PI * 2); c.fill();
    } else if (prop === 'blade') {
      c.rotate(-0.65);
      c.fillStyle = '#bfe7ff'; c.beginPath(); c.moveTo(0, -30); c.lineTo(6, -16); c.lineTo(2, 6); c.lineTo(-2, 6); c.lineTo(-6, -16); c.closePath(); c.fill();
      c.fillStyle = '#facc15'; c.fillRect(-7, 2, 14, 3);
      c.fillStyle = '#334155'; c.fillRect(-2, 5, 4, 12);
    } else if (prop === 'dagger') {
      c.rotate(-0.55);
      c.fillStyle = '#dde6f6'; c.beginPath(); c.moveTo(0, -18); c.lineTo(5, -7); c.lineTo(-5, -7); c.closePath(); c.fill();
      c.fillStyle = '#f472b6'; c.fillRect(-5, -7, 10, 3);
      c.fillStyle = '#334155'; c.fillRect(-2, -4, 4, 10);
    } else if (prop === 'hammer') {
      c.rotate(-0.55);
      c.fillStyle = '#4b5563'; c.fillRect(-8, -24, 16, 8);
      c.fillStyle = '#c084fc'; c.fillRect(-1, -18, 3, 20);
    } else if (prop === 'bow') {
      c.strokeStyle = '#7c4a21'; c.lineWidth = 3; c.beginPath(); c.arc(0, -8, 12, -Math.PI/2, Math.PI/2); c.stroke();
      c.strokeStyle = '#d9dee8'; c.lineWidth = 1; c.beginPath(); c.moveTo(0, -20); c.lineTo(0, 4); c.stroke();
    } else if (prop === 'scroll') {
      c.fillStyle = '#fef3c7'; c.fillRect(-8, -18, 16, 12); c.strokeStyle = '#a16207'; c.strokeRect(-8, -18, 16, 12);
    } else if (prop === 'bag') {
      c.fillStyle = '#8b5a2b'; roundRect(c, -8, -18, 16, 16, 4); c.fillStyle = '#d9a15c'; c.fillRect(-3, -21, 6, 4);
    } else if (prop === 'spear') {
      c.rotate(-0.08); c.fillStyle = '#8b5a2b'; c.fillRect(-1, -32, 2, 30); c.fillStyle = '#e5e7eb'; c.beginPath(); c.moveTo(0, -38); c.lineTo(5, -30); c.lineTo(0, -24); c.lineTo(-5, -30); c.closePath(); c.fill();
    } else if (prop === 'star') {
      c.fillStyle = '#facc15'; c.beginPath(); c.moveTo(0,-18); c.lineTo(4,-8); c.lineTo(14,-8); c.lineTo(6,-1); c.lineTo(9,9); c.lineTo(0,3); c.lineTo(-9,9); c.lineTo(-6,-1); c.lineTo(-14,-8); c.lineTo(-4,-8); c.closePath(); c.fill();
    }
    c.restore();
  }

  drawNPCBody = window.drawNPCBody = function drawNPCBodyV22(npc){
    const t = performance.now() / 1000;
    const bob = Math.sin(t * 3.4 + npc.x * 0.01) * 1.4;
    const look = npcPaletteV22(npc);
    ctx.save();
    ctx.translate(npc.x, npc.y + bob);
    ctx.scale(0.84, 0.84);
    drawReferenceChibiBodyV22(ctx, look, 'idle');
    drawNpcPropV22(ctx, look.prop);
    ctx.restore();
  };

  const oldDrawHUDV22 = drawHUD;
  drawHUD = window.drawHUD = function drawHUDV22(){
    oldDrawHUDV22.apply(this, arguments);
    try {
      ctx.save();
      ctx.textAlign = 'right';
      ctx.font = 'bold 16px sans-serif';
      ...
      ctx.strokeStyle = 'rgba(15,23,42,.95)';
      ctx.lineWidth = 5;
      ctx.strokeText('V22 외형/인벤토리/이름 패치 적용됨', W - 22, 552);
      ctx.fillText('V22 외형/인벤토리/이름 패치 적용됨', W - 22, 552);
      ctx.fillStyle = '#fcd34d';
      ctx.strokeText('캐릭터/NPC 치비화 · 인벤토리 50칸+스크롤 · 장비 이름 재정비', W - 22, 574);
      ctx.fillText('캐릭터/NPC 치비화 · 인벤토리 50칸+스크롤 · 장비 이름 재정비', W - 22, 574);
      ctx.restore();
    } catch (_) {}
  };

  function setSeriesNamesV22(prefix, names){
    Object.keys(ITEMS).forEach(function(id){
      if (!id.startsWith(prefix + '_')) return;
      const m = id.match(/_(\d+)$/);
      if (!m) return;
      const n = Math.max(1, parseInt(m[1], 10));
      const idx = Math.min(names.length - 1, n - 1);
      ITEMS[id].name = names[idx];
      if (n >= 17) ITEMS[id].rarity = 'legendary';
      else if (n >= 13) ITEMS[id].rarity = 'epic';
      else if (n >= 9) ITEMS[id].rarity = 'rare';
      else ITEMS[id].rarity = 'common';
    });
  }
  setSeriesNamesV22('sword', [
    '낡은 검','수습 검','견습 검','훈련용 검','보병 검','정찰검','정기사 검','근위대 검','청은검','월광검','태양검','성기사검','알타이르','발뭉','아스칼론','미스틸테인','루멘 칼리버','라그나로크','네메시스','에테르 블레이드'
  ]);
  setSeriesNamesV22('dagger', [
    '낡은 단검','수습 단검','견습 단검','훈련용 단검','도적 단검','정찰 단검','암살 단검','그림자 비수','청월 비수','은결 비수','혈영 비수','질풍 단검','나이트폴','셰이드팽','팬텀엣지','모르가나','블러드문','레이븐클로','네크로스','오블리비언'
  ]);
  setSeriesNamesV22('bow', [
    '낡은 활','수습 활','견습 활','훈련용 활','정찰궁','사냥꾼의 활','장궁','강철 장궁','바람깃 활','은월궁','숲바람 활','천궁','실피드','페일노트','알테어 보우','그린미스트','하늘노래','아폴론','이클립스','펜릴'
  ]);
  setSeriesNamesV22('staff', [
    '낡은 지팡이','수습 지팡이','견습 마도봉','연습용 스태프','마도사의 지팡이','원소 지팡이','현자의 스태프','정령술사의 홀','월광의 지팡이','별빛 스태프','청명한 마도봉','룬각인 스태프','아르카나','세라핌','루나리스','아스트레아','네뷸라','에테르','오벨리스크','네메시스'
  ]);
  setSeriesNamesV22('helmet', [
    '낡은 두건','가죽 두건','견습 투구','훈련 투구','철제 투구','기사 투구','정기사 투구','근위 투구','청은 투구','성광 투구','월광 투구','수호 투구','오리온 헬름','아르곤 헬름','그리폰 헬름','솔라 헬름','발키리 헬름','티탄 헬름','드래곤 헬름','네메시스 헬름'
  ]);
  setSeriesNamesV22('armor', [
    '낡은 조끼','가죽 조끼','견습 갑옷','훈련 흉갑','철제 갑옷','기사 갑옷','정기사 갑옷','근위 흉갑','청은 갑옷','성광 갑옷','월광 갑옷','수호 흉갑','아르테미스 메일','오리온 메일','그리폰 아머','솔라 아머','발키리 아머','티탄 메일','드래곤 아머','네메시스 아머'
  ]);
  setSeriesNamesV22('knee', [
    '낡은 보호대','가죽 보호대','견습 보호대','훈련 보호대','철제 보호대','기사 보호대','정기사 그리브','근위 그리브','청은 그리브','성광 그리브','월광 그리브','수호 그리브','오리온 그리브','아르곤 그리브','그리폰 그리브','솔라 그리브','발키리 그리브','티탄 그리브','드래곤 그리브','네메시스 그리브'
  ]);
  setSeriesNamesV22('accessory', [
    '낡은 펜던트','수습 펜던트','견습 반지','훈련 부적','은빛 반지','행운의 펜던트','기사단 반지','정기사 문장','청은 부적','월광 펜던트','성광 반지','정령의 인장','루미나 펜던트','세라핌 링','아르카나 참','하이페리온','오블리비언 링','네뷸라 참','이그니스 하트','에테르 시길','네메시스 코어','별바람 문장','천공의 고리','드래곤 심장','아우라 펜던트','빛의 표식','암영의 반지','월식의 파편','새벽의 징표','영원의 인장'
  ]);

  Object.assign(ITEMS.wooden_sword, { name: '훈련용 목검', rarity: 'common' });
  Object.assign(ITEMS.iron_sword, { name: '철검', rarity: 'rare' });
  Object.assign(ITEMS.mage_staff, { name: '수습 마도봉', rarity: 'common' });
  Object.assign(ITEMS.hunter_bow, { name: '사냥꾼의 활', rarity: 'rare' });
  Object.assign(ITEMS.rogue_dagger, { name: '그림자 단검', rarity: 'rare' });
  Object.assign(ITEMS.cloth_armor, { name: '수습 갑옷', rarity: 'common' });
  Object.assign(ITEMS.iron_armor, { name: '철제 흉갑', rarity: 'rare' });
  Object.assign(ITEMS.magic_robe, { name: '푸른 로브', rarity: 'rare' });
  Object.assign(ITEMS.lucky_ring, { name: '행운의 반지', rarity: 'rare' });
  Object.assign(ITEMS.swift_charm, { name: '질주의 부적', rarity: 'rare' });

  console.log('[PixelRPG] V22 sprite/inventory/naming patch installed');
})();


/* =========================================================
   V22 EXTRA FX PATCH
   - 무기/장비 등급에 따른 광택 및 공격 이펙트 보강
========================================================= */
(function(){
  'use strict';
  if (window.__PIXEL_RPG_V22_WEAPON_FX_PATCH__) return;
  window.__PIXEL_RPG_V22_WEAPON_FX_PATCH__ = true;

  function rarityInfoV22(id){
    const item = ITEMS[id] || {};
    const rarity = item.rarity || 'common';
    if (rarity === 'legendary') return { rarity: rarity, glow: '#facc15', sub: '#fb7185', ring: '#fde68a', power: 1.8 };
    if (rarity === 'epic') return { rarity: rarity, glow: '#a78bfa', sub: '#7dd3fc', ring: '#ddd6fe', power: 1.4 };
    if (rarity === 'rare') return { rarity: rarity, glow: '#60a5fa', sub: '#93c5fd', ring: '#dbeafe', power: 1.0 };
    return { rarity: rarity, glow: '#9ca3af', sub: '#d1d5db', ring: '#f3f4f6', power: 0.55 };
  }

  const baseDrawWeaponV22Fx = drawWeapon;
  drawWeapon = window.drawWeapon = function drawWeaponV22Fx(c, weaponId, handX, handY, attacking, attackKind, animTime){
    if (!weaponId) return baseDrawWeaponV22Fx(c, weaponId, handX, handY, attacking, attackKind, animTime);
    const fx = rarityInfoV22(weaponId);
    c.save();
    c.translate(handX, handY);
    if (attacking) {
      const t = Math.sin(Math.min(1, (animTime || 0) * 14) * Math.PI);
      c.globalAlpha = 0.18 + fx.power * 0.10;
      c.strokeStyle = fx.glow;
      c.lineWidth = 6 + fx.power * 2;
      c.beginPath();
      c.arc(2 + t * 6, -22, 14 + fx.power * 4 + t * 8, -1.15, 0.7);
      c.stroke();
      c.globalAlpha = 0.12 + fx.power * 0.08;
      c.strokeStyle = fx.sub;
      c.lineWidth = 3 + fx.power * 1.5;
      c.beginPath();
      c.arc(4 + t * 9, -18, 18 + fx.power * 5 + t * 6, -1.2, 0.85);
      c.stroke();
      if (fx.rarity === 'epic' || fx.rarity === 'legendary') {
        c.globalAlpha = 0.28;
        c.fillStyle = fx.ring;
        for (let i = 0; i < (fx.rarity === 'legendary' ? 4 : 2); i++) {
          const px = -2 + i * 7 + t * 4;
          const py = -30 - i * 5;
          c.beginPath();
          c.arc(px, py, 2 + (fx.rarity === 'legendary' ? 1 : 0), 0, Math.PI * 2);
          c.fill();
        }
      }
    } else {
      c.globalAlpha = 0.10 + fx.power * 0.04;
      c.strokeStyle = fx.glow;
      c.lineWidth = 2.5 + fx.power;
      c.beginPath();
      c.arc(0, -20, 8 + fx.power * 3, 0, Math.PI * 2);
      c.stroke();
    }
    c.restore();
    baseDrawWeaponV22Fx(c, weaponId, handX, handY, attacking, attackKind, animTime);
  };

  const baseDrawPlayerBodyFx = drawPlayerBody;
  drawPlayerBody = window.drawPlayerBody = function drawPlayerBodyV22ArmorFx(c, player){
    baseDrawPlayerBodyFx(c, player);
    try {
      const ids = [itemRefId(equipment.armor), itemRefId(equipment.helmet), itemRefId(equipment.accessory)].filter(Boolean);
      let best = { power: 0, glow: null, ring: null };
      ids.forEach(function(id){
        const fx = rarityInfoV22(id);
        if (fx.power > best.power) best = fx;
      });
      if (best.power <= 0.9) return;
      c.save();
      c.globalAlpha = 0.10 + best.power * 0.03;
      c.strokeStyle = best.glow;
      c.lineWidth = 2 + best.power * 0.6;
      c.beginPath();
      c.ellipse(0, -18, 16 + best.power * 2, 24 + best.power * 2, 0, 0, Math.PI * 2);
      c.stroke();
      if (best.power >= 1.4) {
        c.fillStyle = best.ring;
        c.globalAlpha = 0.20;
        c.fillRect(-14, -38, 3, 3);
        c.fillRect(11, -32, 3, 3);
        c.fillRect(-6, -8, 3, 3);
      }
      c.restore();
    } catch (_) {}
  ...
  console.log('[PixelRPG] V22 extra fx patch installed');
})();

/* =========================================================
   V23 CHARACTER / INVENTORY / NAMING PATCH
   - 캐릭터/NPC를 업로드한 치비 레퍼런스 스타일로 통일
   - 장비창 캐릭터 미리보기 제거 (악세사리 슬롯 가림 방지)
   - 인벤토리 50칸 제한 + 스크롤 + 전체/장비/소비 탭
   - 아이템 이름을 레벨명 대신 고유/직관 명칭으로 정리
========================================================= */
(function(){
  'use strict';
  if (window.__PIXEL_RPG_V23_CHARACTER_INVENTORY_PATCH__) return;
  window.__PIXEL_RPG_V23_CHARACTER_INVENTORY_PATCH__ = true;

  /* -----------------------------
     기본 상태값
  ----------------------------- */
  inventory.maxSlots = 50;
  inventory.tab = inventory.tab || 'all';
  inventory.scroll = Number.isFinite(inventory.scroll) ? inventory.scroll : 0;

  function v23Clamp(v, a, b) { return Math.max(a, Math.min(b, v)); }
  function v23Hit(x, y, bx, by, bw, bh) { return x >= bx && x <= bx + bw && y >= by && y <= by + bh; }
  function v23ItemId(ref) { return typeof itemRefId === 'function' ? itemRefId(ref) : (typeof ref === 'string' ? ref : ref && ref.id); }
  function v23ItemEnh(ref) { return typeof itemEnhance === 'function' ? itemEnhance(ref) : Number(ref && ref.enhance || 0); }
  function v23ItemData(ref) { const id = v23ItemId(ref); return id && ITEMS[id] ? ITEMS[id] : null; }

  function v23Category(ref) {
    const item = v23ItemData(ref);
    if (!item) return 'etc';
    if (item.type === 'consumable') return 'use';
    if (item.type === 'weapon' || item.type === 'armor' || item.equipSlot || item.weaponType) return 'equip';
    return 'etc';
  }

  function v23InventoryMetrics() {
    return { panelX:445, panelY:158, x:463, y:246, cols:6, visibleRows:5, cell:56, box:48 };
  }

  function v23FilteredEntries() {
    const tab = inventory.tab || 'all';
    const out = [];
    inventory.items.forEach(function(itemRef, index) {
      const cat = v23Category(itemRef);
      if (tab === 'all' || tab === cat) out.push({ item:itemRef, index:index, data:v23ItemData(itemRef), category:cat });
    });
    return out;
  }

  function v23VisibleEntries() {
    const m = v23InventoryMetrics();
    const list = v23FilteredEntries();
    const maxRows = Math.max(0, Math.ceil(list.length / m.cols) - m.visibleRows);
    inventory.scroll = v23Clamp(inventory.scroll || 0, 0, maxRows);
    const start = (inventory.scroll || 0) * m.cols;
    return { list, entries:list.slice(start, start + m.cols * m.visibleRows), start, maxRows, metrics:m };
  }

  function v23InventoryTabAt(x, y) {
    const baseX = 459, baseY = 200;
    const tabs = [
      { key:'all', label:'전체', x:baseX },
      { key:'equip', label:'장비', x:baseX + 74 },
      { key:'use', label:'소비', x:baseX + 148 }
    ];
    for (const tab of tabs) { if (v23Hit(x, y, tab.x, baseY, 68, 32)) return tab.key; }
    return null;
  }

  getInventorySlotAt = window.getInventorySlotAt = function getInventorySlotAtV23(x, y) {
    const info = v23VisibleEntries();
    const m = info.metrics;
    for (let i = 0; i < m.cols * m.visibleRows; i++) {
      const cx = m.x + (i % m.cols) * m.cell;
      const cy = m.y + Math.floor(i / m.cols) * m.cell;
      if (v23Hit(x, y, cx, cy, m.box, m.box)) {
        const entry = info.entries[i];
        return entry ? entry.index : -1;
      }
    }
  ...
  getHoveredItemRef = window.getHoveredItemRef = function getHoveredItemRefV23() {
    if (!inventory.open || !game.mouse) return null;
    const i = getInventorySlotAt(game.mouse.x, game.mouse.y);
    if (i >= 0 && inventory.items[i]) return inventory.items[i];
    const slot = getEquipmentSlotAt(game.mouse.x, game.mouse.y);
    if (slot && equipment[slot]) return equipment[slot];
    return null;
  };

  handleInventoryClick = window.handleInventoryClick = function handleInventoryClickV23(x, y) {
    if (dragState.active) return;
    const tab = v23InventoryTabAt(x, y);
    if (tab) { inventory.tab = tab; inventory.scroll = 0; return; }
    const i = getInventorySlotAt(x, y);
    if (i >= 0 && inventory.items[i]) useItem(inventory.items[i]);
  };

  const __oldBeginInventoryDragV23 = beginInventoryDrag;
  beginInventoryDrag = window.beginInventoryDrag = function beginInventoryDragV23(x, y) {
    const tab = v23InventoryTabAt(x, y);
    if (tab) { inventory.tab = tab; inventory.scroll = 0; return; }
    return __oldBeginInventoryDragV23(x, y);
  };

  function v23HandleInventoryWheel(e) {
    if (!game.ready || !inventory.open) return;
    const pos = typeof getMouse === 'function' ? getMouse(e) : { x: e.clientX, y: e.clientY };
    if (!v23Hit(pos.x, pos.y, 445, 158, 410, 455)) return;
    const info = v23VisibleEntries();
    if (info.maxRows <= 0) return;
    e.preventDefault();
    inventory.scroll = v23Clamp((inventory.scroll || 0) + (e.deltaY > 0 ? 1 : -1), 0, info.maxRows);
  }
  try { canvas.addEventListener('wheel', v23HandleInventoryWheel, { passive:false }); } catch(_e) {}
  try { window.addEventListener('wheel', v23HandleInventoryWheel, { passive:false }); } catch(_e) {}

  /* -----------------------------
     인벤토리 최대 50칸
  ----------------------------- */
  const __oldAddItemV23 = addItem;
  addItem = window.addItem = function addItemV23(ref, count) {
    const before = inventory.items.length;
    const id = v23ItemId(ref);
    const enhance = v23ItemEnh(ref);
    const data = id ? ITEMS[id] : null;
    if (!id || !data) return;

    const shouldStack = data.type === 'consumable' || data.type === 'etc';
    const found = inventory.items.find(function (item) {
      return item.id === id && (shouldStack || v23ItemEnh(item) === enhance);
    });

    if (found) return __oldAddItemV23(ref, count);
    if (inventory.items.length >= (inventory.maxSlots || 50)) {
      if (typeof makeText === 'function') makeText('인벤토리가 가득 찼습니다. (50칸)', game.player.x, game.player.y - 80, '#ff8787');
      return false;
    }
    const result = __oldAddItemV23(ref, count);
    if (inventory.items.length > (inventory.maxSlots || 50)) inventory.items.length = inventory.maxSlots || 50;
    return result;
  };
  ...
  /* -----------------------------
     장비창: 캐릭터 제거
  ----------------------------- */
  function v23PanelLabel(text, x, y) {
    ctx.fillStyle = '#5579a6';
    ctx.font = 'bold 12px sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText(text, x, y);
  ...
  drawEquipmentBox = window.drawEquipmentBox = function drawEquipmentBoxV23(x, y) {
    const w = 340, h = 455;
    ctx.fillStyle = '#f8fbff';
    ctx.fillRect(x, y, w, h);
    ctx.strokeStyle = '#8eb2d5';
    ...
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(x + 170, y + 130); ctx.lineTo(x + 64, y + 208);
    ctx.moveTo(x + 170, y + 130); ctx.lineTo(x + 170, y + 208);
    ctx.moveTo(x + 170, y + 130); ctx.lineTo(x + 276, y + 208);
    ctx.moveTo(x + 170, y + 244); ctx.lineTo(x + 170, y + 292);
    ctx.stroke();

    slots.forEach(function(slot) {
      const sx = x + slot.x, sy = y + slot.y;
      ctx.fillStyle = '#dbeafe';
      ctx.fillRect(sx, sy, 72, 72);
      ctx.strokeStyle = '#9bb7d6';
      ctx.strokeRect(sx, sy, 72, 72);
      v23PanelLabel(slot.name, sx + 36, sy - 8);
      const ref = equipment[slot.key];
      const data = v23ItemData(ref);
      if (data) {
        drawItemIcon(data, sx + 36, sy + 36, 42);
        ctx.fillStyle = '#233';
        ctx.font = '11px sans-serif';
    ...
    ctx.font = '12px sans-serif';
    ctx.textAlign = 'left';
    ctx.fillText('※ 캐릭터 미리보기를 제거하여 악세사리 슬롯이 가려지지 않습니다.', x + 22, y + 405);
    ctx.fillText('※ 장비는 드래그로 장착/해제할 수 있습니다.', x + 22, y + 425);
  };

  /* -----------------------------
     아이템창: 전체/장비/소비
  ----------------------------- */
  drawItemBox = window.drawItemBox = function drawItemBoxV23(x, y) {
    const w = 410, h = 455;
    const info = v23VisibleEntries();
    const m = info.metrics;
    const tabs = [
    ...
    tabs.forEach(function(tab, i) {
      const tx = x + 14 + i * 74;
      const active = (inventory.tab || 'all') === tab.key;
      ctx.fillStyle = active ? '#ff7aa2' : '#d9e6f4';
      ctx.fillRect(tx, y + 42, 68, 32);
      ctx.strokeStyle = '#8eb2d5';
      ctx.strokeRect(tx, y + 42, 68, 32);
    ...
    for (let i = 0; i < m.cols * m.visibleRows; i++) {
      ...
      if (entry && entry.item) {
        ...
        ctx.font = 'bold 11px sans-serif';
        ctx.textAlign = 'right';
        ctx.fillText(String(entry.item.count || 1), cx + 43, cy + 43);
      }
    }
    ctx.restore();

    const barX = x + 356, barY = y + 88, barH = m.visibleRows * m.cell - 8;
    ctx.fillStyle = '#e5eef8';
    ctx.fillRect(barX, barY, 14, barH);
    ctx.strokeStyle = '#b5c8dd';
    ctx.strokeRect(barX, barY, 14, barH);
    ...
    ctx.font = 'bold 14px sans-serif';
    ctx.textAlign = 'left';
    ctx.fillText(String(wallet.gold) + ' 메소', x + 26, y + 428);
    ctx.fillStyle = '#5579a6';
    ctx.font = '12px sans-serif';
    ctx.textAlign = 'right';
    ctx.fillText('마우스 휠로 스크롤', x + 392, y + 428);
  };

  /* -----------------------------
     아이템 이름 정리
  ----------------------------- */
  function setSeriesNamesV23(prefix, names) {
    Object.keys(ITEMS).forEach(function(id) {
      if (!id.startsWith(prefix + '_')) return;
      const m = id.match(/_(\d+)$/);
      if (!m) return;
  ...
  setSeriesNamesV23('sword', ['낡은 검','수습 검','견습 검','훈련용 검','보병 검','정찰검','정기사 검','근위대 검','청은검','월광검','태양검','성기사검','알타이르','발뭉','아스칼론','미스틸테인','루멘 칼리버','라그나로크','네메시스','에테르 블레이드']);
  setSeriesNamesV23('dagger', ['낡은 단검','수습 단검','견습 단검','훈련용 단검','도적 단검','정찰 단검','암살 단검','그림자 비수','청월 비수','은결 비수','혈영 비수','질풍 단검','나이트폴','셰이드팽','팬텀엣지','모르가나','블러드문','레이븐클로','네크로스','오블리비언']);
  setSeriesNamesV23('bow', ['낡은 활','수습 활','견습 활','훈련용 활','정찰궁','사냥꾼의 활','장궁','강철 장궁','바람깃 활','은월궁','숲바람 활','천궁','실피드','페일노트','알테어 보우','그린미스트','하늘노래','아폴론','이클립스','펜릴']);
  setSeriesNamesV23('staff', ['낡은 지팡이','수습 지팡이','견습 마도봉','연습용 스태프','마도사의 지팡이','원소 지팡이','현자의 스태프','정령술사의 홀','월광의 지팡이','별빛 스태프','청명한 마도봉','룬각인 스태프','아르카나','세라핌','루나리스','아스트레아','네뷸라','에테르','오벨리스크','네메시스']);
  setSeriesNamesV23('helmet', ['낡은 두건','가죽 두건','견습 투구','훈련 투구','철제 투구','기사 투구','정기사 투구','근위 투구','청은 투구','성광 투구','월광 투구','수호 투구','오리온 헬름','아르곤 헬름','그리폰 헬름','솔라 헬름','발키리 헬름','티탄 헬름','드래곤 헬름','네메시스 헬름']);
  setSeriesNamesV23('armor', ['낡은 조끼','가죽 조끼','견습 갑옷','훈련 흉갑','철제 갑옷','기사 갑옷','정기사 갑옷','근위 흉갑','청은 갑옷','성광 갑옷','월광 갑옷','수호 흉갑','아르테미스 메일','오리온 메일','그리폰 아머','솔라 아머','발키리 아머','티탄 메일','드래곤 아머','네메시스 아머']);
  setSeriesNamesV23('knee', ['낡은 보호대','가죽 보호대','견습 보호대','훈련 보호대','철제 보호대','기사 보호대','정기사 그리브','근위 그리브','청은 그리브','성광 그리브','월광 그리브','수호 그리브','오리온 그리브','아르곤 그리브','그리폰 그리브','솔라 그리브','발키리 그리브','티탄 그리브','드래곤 그리브','네메시스 그리브']);
  setSeriesNamesV23('accessory', ['낡은 펜던트','수습 펜던트','견습 반지','훈련 부적','은빛 반지','행운의 펜던트','기사단 반지','정기사 문장','청은 부적','월광 펜던트','성광 반지','정령의 인장','루미나 펜던트','세라핌 링','아르카나 참','하이페리온','오블리비언 링','네뷸라 참','이그니스 하트','에테르 시길']);

  getItemDisplayName = window.getItemDisplayName = function getItemDisplayNameV23(ref) {
    const item = v23ItemData(ref);
    if (!item) return String(v23ItemId(ref) || '아이템');
    const plus = v23ItemEnh(ref);
    return (plus ? '+' + plus + ' ' : '') + (item.name || '아이템');
  };

  /* -----------------------------
     업로드 레퍼런스 캐릭터 스타일 적용
     - 업로드한 작은 치비 이미지를 그대로 사용
  ----------------------------- */
  const V23_CHIBI_SRC = 'data:image/png;base64,';
  const v23ChibiImg = new Image();
  let v23ChibiReady = false;
  v23ChibiImg.onload = function(){ v23ChibiReady = true; };
  v23ChibiImg.src = V23_CHIBI_SRC;

  function v23DrawSpriteBase(c, face, pose, t) {
    if (!v23ChibiReady) return false;
    const bob = pose === 'jump' ? -3 : Math.sin(t * 8) * (pose === 'walk' ? 1.4 : 0.5);
    c.save();
    c.scale(face < 0 ? -1 : 1, 1);
    c.globalAlpha = 0.20;
    c.fillStyle = '#000';
    c.beginPath();
    c.ellipse(0, 1, 10, 3, 0, 0, Math.PI * 2);
    c.fill();
    c.globalAlpha = 1;
    c.drawImage(v23ChibiImg, -13, -39 + bob, 27, 40);
    c.restore();
    return true;
  }

  function v23WeaponHand(player) {
    const face = player.face || 1;
    const weapon = v23ItemData(equipment.weapon) || {};
    const kind = weapon.weaponType || weapon.type || 'sword';
    const attack = player.anim === 'attack';
    const t = player.animTime || 0;
    let x = face > 0 ? 6 : -6;
    let y = -15;
    if (kind === 'staff') { x = face > 0 ? 8 : -8; y = -17; }
    if (kind === 'bow') { x = face > 0 ? 7 : -7; y = -17; }
    if (kind === 'dagger') { x = face > 0 ? 7 : -7; y = -14; }
    if (attack) {
      const p = Math.sin(Math.min(1, t * 14) * Math.PI);
      x += face * (5 + p * 4);
      y -= 5 + p * 2;
    }
    return { x, y, kind };
  ...
  drawPlayerBody = window.drawPlayerBody = function drawPlayerBodyV23(c, player) {
    player = player || game.player;
    const t = performance.now() / 1000;
    const pose = player.anim === 'jump' ? 'jump' : (player.anim === 'walk' ? 'walk' : 'idle');
    const face = player.face || 1;
    c.save();
    if (!v23DrawSpriteBase(c, face, pose, t)) {
      c.fillStyle = '#222';
      c.fillRect(-8, -28, 16, 24);
    }
    const hand = v23WeaponHand(player);
    drawWeapon(c, v23ItemId(equipment.weapon), hand.x, hand.y, player.anim === 'attack', hand.kind, player.animTime || 0);
    c.restore();
  };

  function v23NPCWeaponKind(prop) {
    if (prop === 'staff') return 'staff';
    if (prop === 'bow') return 'bow';
    if (prop === 'dagger') return 'dagger';
    return 'sword';
  }

  const v23NpcRoleWeapon = {
    merchant: null,
    weapon: 'iron_sword',
    blacksmith: null,
    quest: null,
  ...
  drawNPCBody = window.drawNPCBody = function drawNPCBodyV23(npc) {
    const t = performance.now() / 1000;
    const bob = Math.sin(t * 3.5 + npc.x * 0.02) * 1.1;
    const face = npc.face || 1;
    const role = (npc.__v21Role || npc.type || 'villager').toString();
    ctx.save();
    ctx.translate(npc.x, npc.y + bob);
    ctx.scale(0.92, 0.92);
    v23DrawSpriteBase(ctx, face, 'idle', t);
    const weaponId = v23NpcRoleWeapon[role];
    if (weaponId) {
      const kind = v23ItemData(weaponId)?.weaponType || v23NPCWeaponKind(role);
      drawWeapon(ctx, weaponId, face > 0 ? 6 : -6, -16, false, kind, 0);
    }
    ctx.restore();
  };
  ...
  /* -----------------------------
     HUD 안내
  ----------------------------- */
  const __oldDrawHUDV23 = drawHUD;
  drawHUD = window.drawHUD = function drawHUDV23() {
    __oldDrawHUDV23.apply(this, arguments);
    try {
      ctx.save();
      ctx.textAlign = 'right';
      ctx.font = 'bold 16px sans-serif';
      ...
      ctx.strokeStyle = 'rgba(15,23,42,.95)';
      ctx.lineWidth = 5;
      ctx.strokeText('V23 외형/인벤토리/이름 수정 적용됨', W - 22, 552);
      ctx.fillText('V23 외형/인벤토리/이름 수정 적용됨', W - 22, 552);
      ctx.fillStyle = '#fcd34d';
      ctx.strokeText('치비 외형 · 장비창 캐릭터 제거 · 50칸 스크롤 · 전체/장비/소비 탭', W - 22, 574);
      ctx.fillText('치비 외형 · 장비창 캐릭터 제거 · 50칸 스크롤 · 전체/장비/소비 탭', W - 22, 574);
      ctx.restore();
    } catch (_e) {}
  };

  console.log('[PixelRPG] V23 character/inventory/naming patch installed');
})();
