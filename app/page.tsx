...
function computeCardPowerProfile(card: CardDefinition): { score: number; tier: 'S' | 'A' | 'B' | 'C'; breakdown: Array<{ label: string; value: number }>; summary: string } {
  ...
  const effect = Math.round((card.onSummon ? 7 : 0) + (card.effect ? 8 : 0) + (card.uniqueTrait ? 9 + ((card.uniqueTrait.effects?.length ?? 0) * 2) : 0) + (card.trapEffect ? 9 : 0) + ((card.extraChoices?.length ?? 0) * 4));
...
function DeckBuilder({ hub, onHub }: { hub: HubData; onHub: (hub: HubData) => void }) {
  ...
  const filtered = useMemo(() => CARDS.filter((card) => {
    ...
    if (search && !`${card.name} ${card.text} ${card.subtitle} ${card.series ?? ''} ${card.comboTag ?? ''} ${card.uniqueTrait?.name ?? ''} ${card.uniqueTrait?.description ?? ''} ${card.unitType ? UNIT_TYPE_LABEL[card.unitType] : ''} ${(card.keywords ?? []).map((keyword) => KEYWORD_LABEL[keyword]).join(' ')}`.toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  }).sort((a, b) => {
...
function CollectionView({ hub }: { hub: HubData }) {
  ...
  const visible = ownedUnique
    ...
    .filter((card) => !search || `${card.name} ${card.text} ${card.series ?? ''} ${card.uniqueTrait?.name ?? ''} ${card.uniqueTrait?.description ?? ''} ${(card.keywords ?? []).map((keyword) => KEYWORD_LABEL[keyword]).join(' ')}`.toLowerCase().includes(search.toLowerCase()));
  return (
...
function UnitSlot({
  ...
  const defenseDeltaFromPrinted = unit && card ? unit.health - (card.health ?? 0) : 0;
    ...
    >
      {!unit ? <span className="slot-mark">{index + 1}</span> : (
        <>
          ...
          <span className="unit-stats" aria-label={`공격 ${unit.attack}${attackDeltaFromPrinted > 0 ? ' 강화됨' : attackDeltaFromPrinted < 0 ? ' 약화됨' : ''}, 방어 ${unit.health}${defenseDeltaFromPrinted > 0 ? ' 강화됨' : defenseDeltaFromPrinted < 0 ? ' 약화됨' : ''}${unit.shield > 0 ? `, 방어막 +${unit.shield}` : ''}`}>
            <span className={`v32n-stat attack ${attackDeltaFromPrinted > 0 ? 'buffed' : attackDeltaFromPrinted < 0 ? 'debuffed' : ''}`} title={attackDeltaFromPrinted > 0 ? `원래 ATK ${(card?.attack ?? unit.attack)} → 현재 ${unit.attack}` : attackDeltaFromPrinted < 0 ? `원래 ATK ${(card?.attack ?? unit.attack)} → 현재 ${unit.attack}` : `ATK ${unit.attack}`}><b>{unit.attack}</b><i>ATK</i></span>
            <span className={`v32n-stat defense ${defenseDeltaFromPrinted > 0 ? 'buffed' : defenseDeltaFromPrinted < 0 ? 'debuffed' : ''}`} title={defenseDeltaFromPrinted > 0 ? `원래 DEF ${(card?.health ?? unit.health)} → 현재 ${unit.health}` : defenseDeltaFromPrinted < 0 ? `원래 DEF ${(card?.health ?? unit.health)} → 현재 ${unit.health}` : `DEF ${unit.health}`}><b>{unit.health}</b><i>DEF</i></span>
...
function BossRaidDuel({ userId, hub, activeDeck, boss, runId, onHub, onEconomy, onExit }: { userId: string; hub: HubData; activeDeck: DeckRow; boss: BossRaidDefinition; runId: string; onHub: (hub: HubData) => void; onEconomy: (economy: EconomyCenterData) => void; onExit: () => void }) {
  ...
  return createPortal(
    <div className={`v19-client v23-client in-duel v35-practice-overlay v51-boss-raid-overlay threat-${boss.threat}`} data-ui-build="v32-retail" data-boss-build="v57-card-layout-fix" data-boss-raid="true">
